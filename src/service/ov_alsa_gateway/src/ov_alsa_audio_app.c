...
static bool device_info_from_extended_format(ov_json_value const *jval,
    ...
    if (0 == jval) {
        ...
        return set_string(device_str, ov_json_string_get(jdevice)) &&
               ...
               ((0 == jmixer) ||
                set_string(mixer, ov_json_string_get(jmixer))) &&
...
ov_alsa_audio_app_config ov_alsa_audio_app_config_from_json(
    ...
    if (0 != channels) {
        ...
        cfg.max_num_streams =
            ov_config_u32_or_default(jcfg, "/" OV_KEY_STREAMS, 1);
...
static ov_alsa_rtp_mixer *create_mixer_for(OutChannel *channel,
    ...
    if (ov_ptr_valid(channel,
                     "Cannot create mixer for channel - channel pointer "
                     "invalid")) {
...
static bool cb_rtp_recv(ov_rtp_frame *rtp_frame, void *userdata) {
    ...
    if (ov_ptr_valid(
        ...
        if (rtp_frame->expanded.ssrc == channel->ssid_to_cancel) {
            ...
            ov_log_debug("Received RTP frame for own SSRC %" PRIu32
                         " - ignoring",
...
ov_alsa_audio_app *ov_alsa_audio_app_create(ov_event_loop *loop,
    ...
    if (ov_ptr_valid(
        ...
        self->settings.default_codec = ov_json_value_from_string(
