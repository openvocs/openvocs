...
static ov_alsa_rtp_mixer const *as_mixer(void const *ptr) {

  ov_alsa_rtp_mixer const *mixer = ptr;
...
static ov_codec *get_codec_for_ssrc(ov_alsa_rtp_mixer *self, uint32_t ssrc) {

  intptr_t key = ssrc;
  codec_entry *codec = 0;

  if (ov_ptr_valid(self, "Cannot get Codec - invalid ALSA RTP mixer pointer")) {
    ...
    if (!codec) {
      ...
      codec->codec = ov_codec_factory_get_codec(0, ov_codec_opus_id(), key, 0);
...
static ov_codec *get_codec_for_frame(ov_alsa_rtp_mixer *mixer,
...
static ov_buffer *
create_comfort_noise_for_default_frame(ov_alsa_rtp_mixer *self) {
...
ov_alsa_rtp_mixer *ov_alsa_rtp_mixer_create(ov_alsa_rtp_mixer_config cfg) {
  ...
  ov_alsa_rtp_mixer *mixer = calloc(1, sizeof(ov_alsa_rtp_mixer));
...
ov_alsa_rtp_mixer *ov_alsa_rtp_mixer_free(ov_alsa_rtp_mixer *self) {
  ...
  if (ov_ptr_valid(as_mixer(self),
                   "Cannot free ALSA RTP mixer: Invalid pointer")) {
...
bool ov_alsa_rtp_mixer_add_frame(ov_alsa_rtp_mixer *self, ov_rtp_frame *frame) {
  ...
  if (ov_ptr_valid(self,
                   "Cannot add RTP frame to mixing buffer - invalid pointer")) {

    ov_rtp_frame_free(
        frame_buffer_add(&self->frame_buffer_lock, self->frame_buffer, frame));
    return true;
...
static bool decode_frame(ov_codec *codec, ov_rtp_frame *frame,
  ...
  if (ov_ptr_valid(codec, "Cannot decode frame - invalid codec") &&
    ...
    target->length = ov_codec_decode(
        codec, frame->expanded.sequence_number, frame->expanded.payload.data,
        frame->expanded.payload.length, target->start, target->capacity);
...
static ov_buffer *mix_frames(ov_alsa_rtp_mixer *self, ov_list *frames,
                             size_t frame_length_samples) {
  ...
  for (size_t i = 0; i < num_frames; ++i) {
    ...
    if (decode_frame(get_codec_for_frame(self, frame), frame, decoded_16bit) &&
        (ov_ptr_valid(decoded_16bit, "Could not decode RTP frame") &&
         ...
         (ov_cond_valid(ov_pcm_16_scale_to_32(frame_length_samples,
...
static bool process_frames(ov_alsa_rtp_mixer *self, ov_list *frames,
