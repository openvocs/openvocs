...
static bool cb_io(void *userdata, int socket, const char *domain, const ov_memory_pointer data){
    ...
    ov_io_send(io, socket, (ov_memory_pointer){
        .start = (uint8_t*)str, 
        .length = strlen(str)
    });
...
int main(int argc, char **argv) {
    ...
    ov_socket_configuration socket_config = ov_socket_configuration_from_json(ov_json_get(json_config,"/echo/socket"),
                (ov_socket_configuration){0});

    if (ov_json_is_true(ov_json_get(json_config, "/echo/client"))){

        ov_log_debug("Trying to open connection to %s:%i", socket_config.host, socket_config.port);

        socket = ov_io_open_connection(io, (ov_io_socket_config){
            .auto_reconnect = true,
        ...
        ov_log_debug("Trying to open %s:%i", socket_config.host, socket_config.port);

        socket = ov_io_open_listener(io, (ov_io_socket_config){
            .socket = socket_config,
            .callbacks.userdata = io,
            .callbacks.io = cb_io
