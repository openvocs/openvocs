...
static void ov_value_sample() {
  ...
  for (size_t i = 0; i < NUM_RUNS; ++i) {
    ...
    vlist = ov_value_list(ov_value_null(), ov_value_string("Cthulhu naftagn"),
                          ov_value_true());
...
static void ov_json_value_sample() {
  ...
  for (size_t i = 0; i < NUM_RUNS; ++i) {
    ...
    vlist = get_json_array(ov_json_null(), ov_json_string("Cthulhu naftagn"),
                           ov_json_true());
...
static void ov_json_value_from_string_sample() {
  ...
  for (size_t i = 0; i < NUM_PARSE_RUNS; ++i) {
    ...
    ov_json_value *v = ov_json_value_from_string((char *)buf.start, buf.length);
    OV_ASSERT(0 != v);
...
size_t get_NUM_RUNS(int argc, char **argv) {
  ...
  if ((0 == endptr) || (0 != *endptr)) {
    ...
    fprintf(stderr, "Require number of runs to be a positive number, got %s\n",
            argv[1]);
    exit(EXIT_FAILURE);
  }
