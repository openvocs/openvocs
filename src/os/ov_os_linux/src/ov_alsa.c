...
static bool set_volume(snd_mixer_elem_t *elem,
    ...
    if (0.0 > volume) {
        ...
        ov_log_error(
            "Cannot set ALSA volume to %f: Volume out of range - must be "
            "between 0 and 1",
            volume);
        return false;
...
ov_alsa *ov_alsa_create(char const *device_name,
    ...
    ov_log_debug("Using ALSA device %s - device string to connect:%s \n",
                 ov_string_sanitize(device_name),
                 ov_string_sanitize(alsa_name));
...
size_t ov_alsa_capture_period(ov_alsa *self,
    ...
    snd_pcm_sframes_t err = snd_pcm_readi_wrapper(
        get_handle(self), target_buffer, get_samples_per_period(self));
