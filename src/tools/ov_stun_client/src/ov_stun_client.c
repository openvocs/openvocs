...
static bool check_server(ov_event_loop *loop, int client,
  ...
  if (!ov_stun_generate_binding_request_plain(buffer, size, &next,
                                              transaction_id, NULL, 0, true)) {
  ...
  ssize_t bytes =
      sendto(client, buffer, next - buffer, 0, (struct sockaddr *)&sa, socklen);
...
static bool check_ports(ov_event_loop *loop, ov_json_value *jconfig) {
  ...
  if (value) {
    ...
    if (ov_json_get(stun, "/server/range")) {
      ...
      uint64_t end = ov_json_number_get(ov_json_get(stun, "/server/range/end"));
