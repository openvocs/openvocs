...
static ov_codec *get_codec(char const *codec_json_str) {
  ...
  if (0 == codec) {
    ...
    fprintf(stderr, "Could not create codec for %s\n",
...
static bool print_rtp_characteristics(ov_rtp_frame const *frame, FILE *out) {
  ...
  if (0 == frame) {
    ...
    fprintf(out, "SSRC %" PRIu32 "  SEQ  %" PRIu16, frame->expanded.ssrc,
...
static bool decode_to_pcm(ov_rtp_frame const *frame, ov_codec *codec,
                          uint8_t *pcm, ssize_t *pcm_octets) {
  ...
  if (0 == frame) {
    ...
    *pcm_octets = ov_codec_decode(
        codec, frame->expanded.sequence_number, frame->expanded.payload.data,
        frame->expanded.payload.length, pcm, *pcm_octets);
...
static bool check__plausibility(FILE *out, ov_vad_parameters params,
  ...
  double zero_crossings_diff = fabs(zero_crossings_hz - zero_crossings_hz_ref);

  // Powerlevel density
  double powerlevel_density =
      20.0 / log(10.0) * log(params.powerlevel_density_per_sample);

  double powerlevel_diff = fabs(powerlevel_density - powerlevel_density_db_ref);
...
static bool print_audio_characteristics(uint8_t *pcm, size_t pcm_octets,
                                        ov_vad_config vad, FILE *out) {
  ...
  fprintf(out, "   Powerlevel density %f   zero crossing rate %f Hz",
          powerlevel_density_db, zero_crossings_hz);
...
static void sockaddr_address(struct sockaddr *addr, char *target,
  ...
  case AF_INET:
    ...
    inet_ntop(AF_INET, &(((struct sockaddr_in *)addr)->sin_addr), target,
              target_len);
    break;

  case AF_INET6:

    inet_ntop(AF_INET6, &(((struct sockaddr_in6 *)addr)->sin6_addr), target,
              target_len);
    break;
...
static void receive_from(int fd, char const *mc_ip, char const *mc_port,
  ...
  while (true) {
    ssize_t read_octets = recvfrom(fd, buf, sizeof(buf) - 1, 0,
    ...
    if (read_octets >= 0) {
      ...
      fprintf(out, "%s:%s  From %s:%" PRIu16 " ", ov_string_sanitize(mc_ip),
              ov_string_sanitize(mc_port), srcaddr_str,
              sockaddr_port((struct sockaddr *)&srcaddr));
