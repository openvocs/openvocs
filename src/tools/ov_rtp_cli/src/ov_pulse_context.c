...
ov_json_value *ov_pulse_get_state(ov_pulse_context *restrict context) {
  ...
  ov_json_object_set(stream_json, OV_KEY_STREAM_STATE,
...
static void pulse_setup_stream(ov_pulse_context *context,
  ...
  pa_stream_set_underflow_callback(stream, stream_underflow_callback, context);
...
static void stream_state_callback(pa_stream *stream, void *userdata) {
  ...
  case PA_STREAM_READY:
    ...
    if (!(a = pa_stream_get_buffer_attr(stream)))
      ...
      ov_log_info("Using sample spec '%s', channel "
                  "map '%s'.",
...
static void stream_write_callback(pa_stream *stream, size_t nbytes,
  ...
  int retval = pa_stream_write(stream, pcm_data.data, pcm_data.nbytes, free, 0,
                               PA_SEEK_RELATIVE);
...
static Buffer get_next_chunk_from_queue(AudioIo *audio_io,
  ...
  while (buffer.nbytes < nbytes_requested) {
    ...
    if (current->nbytes > bytes_to_write) {
      ...
      memcpy(remainder->data, current->data + bytes_to_write, remaining_bytes);
...
static void audio_io_init(AudioIo *audio_io, size_t lock_timeout_usecs,
  ...
  audio_io->queue = ov_ringbuffer_create(num_frames_to_buffer, free_buffer, 0);
}
