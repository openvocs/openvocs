...
static void analyse_audio(FILE *out, ov_buffer encoded_audio, ov_format *fmt) {
  ...
  if ((0 == pcap) || (!ov_format_pcap_get_current_packet_header(pcap, &phdr))) {
  ...
  fprintf(out,
          ...
          phdr.timestamp_secs, phdr.timestamp_usecs, rhdr.timestamp, rhdr.ssrc,
          uhdr.source_port, uhdr.destination_port, rhdr.sequence_number,
          num_samples, num_channels, encoded_audio.length);
}
