...
static size_t read_line(FILE *input) {
  ...
  while (c != 0) {
    ...
    if (i >= max_line_len) {
      printerr("Line way too long %zu chars - surpasing maximum chars of %zu",
               i, max_line_len);
      abort();
    }
  };
...
static State line_to_state() {
  ...
  if (((i + 5) > line_len) || (line[i] != '<')) {
    printerr("After digits: Line too short or '<' missing: %s  - %c should be "
             "'<'\n",
             line, line[i]);
    return INVALID;
  }
  ...
  if (line[i + 1] == 'S') {
    ...
    if ((line[i + 2] == 'N') && (line[i + 3] == 'D') && (line[i + 4] == '>')) {
      return SND;
    } else {
      return INVALID;
    }
    ...
    if ((line[i + 2] == 'C') && (line[i + 3] == 'V') && (line[i + 4] == '>')) {
      return RCV;
    } else {
      return INVALID;
    }
