...
static bool split_prob_span_destructively(char *str, float *prob,
  ...
  if (ov_ptr_valid(str, "Cannot parse probability & span: No string") &&
      ov_ptr_valid(prob,
                   ...
                   "to store result to") &&
      ov_ptr_valid(span, "Cannot parse probability & span: No span variable to "
                         "store result to") &&
...
static uint16_t port_from_string(char const *str) {
  ...
  if ((port > UINT16_MAX) || (port < 0) || (errno == ERANGE) || (endptr == 0) ||
      (*endptr != '\0')) {
...
struct configuration_t parse_arguments(int argc, char const **argv) {
  ...
  while ((c = getopt(argc, (char **)argv, "tmr:d:D:")) != -1) {
    ...
    case 'd':
      ...
      if (!split_prob_span(optarg, &args.delay.probability, &args.delay.span)) {
        fprintf(stderr, "Invalid arguments to 'reorder' option\n");
        usage(argv[0]);
      }
  ...
  if (optind + 4 > argc) {
    ...
    fprintf(stderr, "Missing LISTENIF, LISTENPORT, TARGETIP or TARGETOPRT\n");
    usage(argv[0]);
  }
...
static int open_listen_socket(ov_socket_configuration scfg, bool multicast) {
  ...
  if (UDP == scfg.type) {
    ...
    printf("RECV: Using TCP - Not supported yet - need to listen and accept "
           "- ");
    exit(1);
  }
...
static void senddata(struct jitter_app_t app, uint8_t const *data,
  ...
  if (TCP == app.send.sd) {
    ...
    bytes_sent = sendto(app.send.sd, data, num_octets, 0,
...
static bool trigger_sending() {
  ...
  while (0 != buffer) {
    ...
    if (ov_ptr_valid(buffer, "Cannot send data - no data")) {
      ...
      uint32_t rel_prob = configuration.reorder.probability * (float)UINT32_MAX;
      if (ov_random_uint32() < rel_prob) {
        ...
        insert_past(reordered, reordered_capacity, (size_t)reorder_index,
                    buffer);
...
static bool register_socket(int s, ov_event_loop *loop) {
  ...
  return ov_event_loop_set(
      loop, s, OV_EVENT_IO_IN | OV_EVENT_IO_ERR | OV_EVENT_IO_CLOSE, 0, cb_io);
}
