...
static void print_usage() {
  ...
  fprintf(stdout, "\n");
  fprintf(stdout, "               -w,     --workfactor    scrypt workfactor to "
                  "use\n");
  fprintf(stdout,
...
bool read_user_input(int argc, char *argv[], ov_password_hash_parameter *params,
  ...
  while (1) {
    ...
    case 'p':
      ov_convert_string_to_uint16(optarg, strlen(optarg), &params->parallel);
      break;
    ...
    case 'w':
      ov_convert_string_to_uint16(optarg, strlen(optarg), &params->workfactor);
      break;

    case 'b':
      ov_convert_string_to_uint16(optarg, strlen(optarg), &params->blocksize);
      break;
