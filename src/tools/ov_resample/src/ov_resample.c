...
static void resample(double samplerate_in_hz, double samplerate_out_hz,
  ...
  while (next_sample_chunk(&args, sig_in, num_samples)) {
    ...
    case NO_PRECALC:
      ...
      out_samples = ov_pcm_16_resample_uncached(resampler, sig_in, num_samples,
    ...
    case PRECALC:
      ...
      out_samples = ov_pcm_16_resample(resampler, sig_in, num_samples, out,
                                       out_length_samples);
...
static bool parse_args(size_t argc, char const **argv, struct args *args) {
  ...
  while (i < argc) {
    ...
    case 'p':
      ...
      i = parse_periods(argc, argv, ++i, &args->periods, &args->num_periods);
      break;
