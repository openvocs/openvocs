...
static ov_codec *get_codec_ssrc(ov_vad_core *self, uint32_t ssrc) {
  ...
  if (ov_ptr_valid(self, "Cannot get Codec - invalid vad pointer")) {
    ...
    if (!codec) {
      ...
      codec->codec = ov_codec_factory_get_codec(self->codec.factory,
                                                ov_codec_opus_id(), key, 0);

      if (codec && (!ov_dict_set(self->codec.codecs, (void *)key, codec, 0))) {
...
static bool handle_loop_io(ov_vad_core *self, Loops *loop, uint8_t *buf,
  ...
  if (stream_codec)
    length_bytes =
        ov_codec_decode(stream_codec, frame->expanded.sequence_number,
                        frame->expanded.payload.data,
                        frame->expanded.payload.length, (uint8_t *)pcm16, 2048);
  ...
  if (!counter) {
    ...
    ov_dict_set(loop->ssrcs, (void *)(intptr_t)frame->expanded.ssrc, counter,
                NULL);
  }
  ...
  if (ov_pcm_vad_detected(48000, vad_params, self->config.vad)) {
    ...
    if (!counter->active) {
      ...
      if (counter->on >= self->config.limits.frames_activate) {
        ...
        // voice switch on
        ov_log_debug("VAD on %s SSRC %i", loop->name, frame->expanded.ssrc);
    ...
    if (counter->active) {
      ...
      if (counter->off >= self->config.limits.frames_deactivate) {
        ...
        ov_log_debug("VAD off %s SSRC %i", loop->name, frame->expanded.ssrc);
  ...
  if (switch_on) {
    ...
    if (!loop->on) {
      ...
      loop->on = true;
      self->config.callbacks.vad(self->config.callbacks.userdata, loop->name,
                                 loop->on);
    }
  }
...
ov_vad_core *ov_vad_core_create(ov_vad_core_config config) {
  ...
  vad->idle_check =
      ov_event_loop_timer_set(vad->config.loop, 500000, vad, check_ilde_loops);
