  ...
  *val = parameter;
  ...
  for (size_t i = 0; i < file_extension_size; i++) {
    ...
    if (name[0] == '.') {
      ...
      ov_log_debug("%s starts with . (dot) - ignoring as extension", name);
    ...
    if (ov_dict_is_set(reg->extensions, key)) {
      ...
      ov_log_debug("%s already registered as extension - overriding", key);
    }
...
ov_file_format_get(const ov_file_format_registry *registry, const char *name) {
  ...
  return (const ov_file_format_parameter *)ov_dict_get(registry->format, name);
}
...
static bool check_mime_object(void *val) {
  ...
  if (!value || !ov_json_is_array(value)) {
    ov_log_error("... pair content %s not array or not set", OV_KEY_EXTENSION);
    goto error;
  }
...
bool ov_file_format_register_values_from_json(
  ...
  const char *mime = ov_json_string_get(ov_json_object_get(data, OV_KEY_MIME));
  if (!mime)
    goto error;
  ...
  if (name) {
    ...
    if (!snprintf(parameter->name, OV_FILE_FORMAT_TYPE_NAME_MAX, "%s", name))
      goto error;
  }
