...
static bool get_packet_header_unsafe(ov_format_pcap_packet_header *out,
  ...
  ok =
      ok && ov_file_get_32(&hdr.length_stored_bytes, rd_ptr, length, byteorder);
  ok =
      ok && ov_file_get_32(&hdr.length_origin_bytes, rd_ptr, length, byteorder);
...
int ov_format_pcap_print_global_header(FILE *out,
  ...
  return fprintf(out,
                 ...
                 hdr->version_minor, hdr->thiszone, hdr->sigfigs, hdr->snaplen,
                 hdr->data_link_type);
}
...
ov_format *ov_format_pcap_create_network_layer_format(ov_format *pcap_fmt) {
  ...
  default:
    ...
    ov_log_error("Unsupported link layer format: %" PRIu32, hdr.data_link_type);
  }
