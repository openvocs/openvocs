...
char *ov_format_ethernet_mac_to_string(uint8_t *mac, char *out_string,
  ...
  size_t octets_written = snprintf(out_string, out_len, "%x:%x:%x:%x:%x:%x",
...
impl_dispatcher_next_chunk(ov_format *f, size_t requested_bytes, void *data) {
  ...
  ov_buffer new_data = ov_format_payload_read_chunk_nocopy(ddata->ethernet, 0);
  ...
  case IPV4:
    ...
    buffer = ov_format_payload_read_chunk_nocopy(ddata->ipv4, requested_bytes);
  ...
  case IPV6:
    ...
    buffer = ov_format_payload_read_chunk_nocopy(ddata->ipv6, requested_bytes);
