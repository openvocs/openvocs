...
bool ov_stun_attribute_frame_is_error_code(const uint8_t *buffer,
  ...
  if (type != STUN_ERROR_CODE)
    goto error;

  if (size > 763 || size < 1)
    goto error;

  if (length < (size_t)size + 8)
    goto error;
...
bool ov_stun_error_code_decode_phrase(const uint8_t *buffer, size_t length,
  ...
  if (!buffer || length < 9 || !phrase || !size)
    goto error;

  if (ov_stun_attribute_get_type(buffer, 4) != STUN_ERROR_CODE)
    goto error;
...
bool ov_stun_error_code_encode(uint8_t *buffer, size_t length, uint8_t **next,
  ...
  if (!buffer || length < 8 || !phrase || phrase_length < 1)
    goto error;

  if (code < 300 || code >= 700)
    goto error;

  if (phrase_length > 763)
    goto error;

  if (length < phrase_length + 8)
    goto error;

  if (!ov_utf8_validate_sequence(phrase, phrase_length))
    goto error;
  ...
  content[3] = code % 100;
  if (!memcpy(content + 4, phrase, phrase_length))
    goto error;

  return ov_stun_attribute_encode(buffer, length, next, STUN_ERROR_CODE,
                                  content, phrase_length + 4);
error:
  return false;
}
...
bool ov_stun_error_code_set_try_alternate(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 300, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_bad_request(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 400, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_unauthorized(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 401, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_unknown_attribute(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 420, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_stale_nonce(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 438, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_server_error(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 500, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_allocation_mismatch(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 437, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_family_mismatch(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 443, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_unsupported_transport(uint8_t *buffer,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 442, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_address_family_not_supported(uint8_t *buffer,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 440, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
bool ov_stun_error_code_set_forbidden(uint8_t *buffer, size_t length,
  ...
  return ov_stun_error_code_encode(buffer, length, next, 403, (uint8_t *)phrase,
                                   strlen(phrase));
}
...
size_t ov_stun_error_code_generate_response(
  ...
  if (!frame || !response || frame_size < 20 || response_size < 20 || !function)
    goto error;
  ...
  if (!ov_stun_frame_set_error_response(response, response_size))
    goto error;

  if (!ov_stun_frame_set_magic_cookie(response, response_size))
    goto error;

  // copy method
  if (!ov_stun_frame_set_method(response, response_size,
  ...
  if (!function(response + 20, response_size - 20, &next))
    goto error;

  if (!ov_stun_attribute_set_length(response, response_size,
                                    (next - response) - 20))
    goto error;
