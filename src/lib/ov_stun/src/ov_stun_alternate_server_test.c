/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           ov_stun_alternate_server_test.c
        ...
        @brief          Unit tests of RFC 5389 attribute "alternate server"


        ------------------------------------------------------------------------
*/

#include "ov_stun_alternate_server.c"
...
int test_ov_stun_attribute_frame_is_alternate_server() {
    ...
    // prepare valid frame
    testrun(ov_stun_attribute_set_type(buffer, size, STUN_ALTERNATE_SERVER));
    testrun(ov_stun_attribute_set_length(buffer, size, 8));
    *(buf + 5) = 0x01;

    testrun(ov_stun_attribute_frame_is_alternate_server(buffer, size));

    testrun(!ov_stun_attribute_frame_is_alternate_server(NULL, size));
    testrun(!ov_stun_attribute_frame_is_alternate_server(buffer, 0));
    testrun(!ov_stun_attribute_frame_is_alternate_server(buffer, 11));

    // min buffer min valid
    testrun(ov_stun_attribute_frame_is_alternate_server(buffer, 12));
    ...
    for (int i = 0; i < 0xff; i++) {
        ...
        *(buf + 5) = 0x02;
        ...
        if (i == STUN_ALTERNATE_SERVER) {
            testrun(ov_stun_attribute_frame_is_alternate_server(buffer, size));
        } else {
            testrun(!ov_stun_attribute_frame_is_alternate_server(buffer, size));
        }
        testrun(
            ov_stun_attribute_set_type(buffer, size, STUN_ALTERNATE_SERVER));
    }
...
int test_ov_stun_alternate_server_encoding_length() {

    struct sockaddr_storage sa = {0};

    sa.ss_family = AF_UNIX;
    testrun(0 == ov_stun_alternate_server_encoding_length(&sa));

    sa.ss_family = AF_INET;
    testrun(12 == ov_stun_alternate_server_encoding_length(&sa));

    sa.ss_family = AF_INET6;
    testrun(24 == ov_stun_alternate_server_encoding_length(&sa));
...
int test_ov_stun_alternate_server_encode() {
    ...
    testrun(!ov_stun_alternate_server_encode(NULL, 0, NULL, NULL));
    testrun(!ov_stun_alternate_server_encode(NULL, size, NULL, &sa));
    testrun(!ov_stun_alternate_server_encode(buffer, 0, NULL, &sa));
    testrun(!ov_stun_alternate_server_encode(buffer, size, NULL, NULL));

    testrun(!ov_stun_alternate_server_encode(buffer, 11, NULL, &sa));
    testrun(ov_stun_alternate_server_encode(buffer, 12, NULL, &sa));
    ...
    testrun(ov_stun_alternate_server_encode(buffer, size, &next, &sa));
    testrun(STUN_ALTERNATE_SERVER == ov_stun_attribute_get_type(buffer, size));
    testrun(8 == ov_stun_attribute_get_length(buffer, size));
    testrun(buffer[4] == 0);
    testrun(buffer[5] == 1);
    ...
    testrun(!ov_stun_alternate_server_encode(buffer, 22, &next, &sa));
    testrun(!ov_stun_alternate_server_encode(buffer, 23, &next, &sa));
...
int test_ov_stun_alternate_server_decode() {
    ...
    *(uint16_t *)(buffer + 6) = htons(66);
    ...
    testrun(!ov_stun_alternate_server_decode(NULL, 0, NULL));
    testrun(!ov_stun_alternate_server_decode(buffer, size, NULL));
    testrun(!ov_stun_alternate_server_decode(buffer, 0, &address));
    testrun(!ov_stun_alternate_server_decode(NULL, size, &address));

    // check min
    testrun(!ov_stun_alternate_server_decode(buffer, 11, &address));
    testrun(ov_stun_alternate_server_decode(buffer, 12, &address));
    testrun(address);
...
int all_tests() {
    ...
    testrun_init();
    testrun_test(test_ov_stun_attribute_frame_is_alternate_server);

    testrun_test(test_ov_stun_alternate_server_encoding_length);
    testrun_test(test_ov_stun_alternate_server_encode);
    testrun_test(test_ov_stun_alternate_server_decode);
