...
int test_ov_stun_attr_password_algorithm_encoding_length() {
  ...
  for (size_t i = 1; i < 1000; i++) {
    ...
    testrun(8 + i + pad == ov_stun_attr_password_algorithm_encoding_length(i));
  }
...
int test_ov_stun_attr_password_algorithm_encode() {
  ...
  testrun(
      !ov_stun_attr_password_algorithm_encode(buffer, size, NULL, 1, 8, NULL));

  memset(buf, 0, size);
  testrun(
      ov_stun_attr_password_algorithm_encode(buffer, size, &next, 1, 8, nonce));
...
int test_ov_ov_stun_attr_password_algorithm_decode() {
  ...
  uint8_t *nonce = (uint8_t *)"test1234";
  testrun(
      ov_stun_attr_password_algorithm_encode(buffer, size, NULL, 1, 8, nonce));

  testrun(
      !ov_stun_attr_password_algorithm_decode(NULL, size, NULL, NULL, NULL));
  testrun(
      !ov_stun_attr_password_algorithm_decode(NULL, size, &algo, &out, &para));
  testrun(
      !ov_stun_attr_password_algorithm_decode(buffer, 0, &algo, &out, &para));
  testrun(
      !ov_stun_attr_password_algorithm_decode(buffer, size, NULL, &out, &para));
  testrun(!ov_stun_attr_password_algorithm_decode(buffer, size, &algo, NULL,
                                                  &para));
  testrun(
      !ov_stun_attr_password_algorithm_decode(buffer, size, &algo, &out, NULL));

  testrun(
      ov_stun_attr_password_algorithm_decode(buffer, size, &algo, &out, &para));
  testrun(algo == 1);
  testrun(out == 8);
