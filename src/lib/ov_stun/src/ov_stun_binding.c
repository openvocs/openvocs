...
bool ov_stun_method_is_binding(const uint8_t *frame, size_t length) {
    ...
    if (STUN_BINDING == ov_stun_frame_get_method(frame, length)) return true;
...
bool ov_stun_generate_binding_request_plain(uint8_t *start,
    ...
    return ov_stun_generate_binding_request(start,
                                            length,
                                            next,
                                            transaction_id,
                                            software,
...
bool ov_stun_generate_binding_request(uint8_t *start,
    ...
    if (!ov_stun_frame_set_method(start, length, STUN_BINDING)) goto error;
    ...
    if (integrity)
        if (!ov_stun_add_message_integrity(
                start, length, ptr, &ptr, key, key_len))
            goto error;
