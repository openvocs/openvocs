...
*//**
        ...
        ------------------------------------------------------------------------
*/
#include "../include/ov_turn_allocate.h"
...
bool ov_turn_method_is_allocate(const uint8_t *frame, size_t length) {

    if (TURN_ALLOCATE == ov_stun_frame_get_method(frame, length)) return true;
...
bool ov_turn_allocate_generate_request(uint8_t *start,
    ...
    if (!ov_stun_frame_set_method(start, length, TURN_ALLOCATE)) goto error;
