...
bool ov_turn_attr_is_even_port(const uint8_t *buffer, size_t length) {
    ...
    if (type != TURN_EVEN_PORT) goto error;
...
bool ov_turn_attr_even_port_encode(uint8_t *buffer,
    ...
    return ov_stun_attribute_encode(
        buffer, length, next, TURN_EVEN_PORT, buf, 4);
error:
    return false;
}
...
bool ov_turn_attr_even_port_decode(const uint8_t *buffer,
    ...
    if (!buffer || length < 8 || !set) goto error;

    if (!ov_turn_attr_is_even_port(buffer, length)) goto error;

    *set = false;

    if (buffer[4] & 0x80) *set = true;
