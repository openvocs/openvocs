    ...
    *(buf + 5) = 0x01;
    ...
    for (int i = 0; i < 0xff; i++) {
        ...
        *(buf + 5) = 0x02;
        ...
        testrun(
            ov_stun_attribute_set_type(buffer, size, TURN_XOR_RELAYED_ADDRESS));
    }
...
int test_ov_turn_attr_xor_relayed_address_encode() {
    ...
    testrun(
        !ov_turn_attr_xor_relayed_address_encode(NULL, 0, NULL, NULL, NULL));
    testrun(
    ...
    testrun(
        ov_turn_attr_xor_relayed_address_encode(buffer, 12, head, NULL, &sa));
...
int test_ov_turn_attr_xor_relayed_address_decode() {
    ...
    *(uint32_t *)(buffer + 8) = htonl(ip ^ cookie);
    ...
    testrun(
        !ov_turn_attr_xor_relayed_address_decode(buffer, 0, head, &address));
    testrun(
        !ov_turn_attr_xor_relayed_address_decode(NULL, size, head, &address));
    testrun(
        !ov_turn_attr_xor_relayed_address_decode(buffer, size, NULL, &address));

    // check min
    testrun(
        !ov_turn_attr_xor_relayed_address_decode(buffer, 11, head, &address));
    testrun(
        ov_turn_attr_xor_relayed_address_decode(buffer, 12, head, &address));
    testrun(address);
