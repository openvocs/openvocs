/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           ov_turn_attr_xor_relayed_address_test.c
        ...
        ------------------------------------------------------------------------
*/
#include "ov_turn_attr_xor_relayed_address.c"
#include <ov_test/testrun.h>

int test_ov_turn_attr_frame_is_xor_relayed_address() {
  ...
  // prepare valid frame
  testrun(ov_stun_attribute_set_type(buffer, size, TURN_XOR_RELAYED_ADDRESS));
  testrun(ov_stun_attribute_set_length(buffer, size, 8));
  *(buf + 5) = 0x01;

  testrun(ov_turn_attr_frame_is_xor_relayed_address(buffer, size));

  testrun(!ov_turn_attr_frame_is_xor_relayed_address(NULL, size));
  testrun(!ov_turn_attr_frame_is_xor_relayed_address(buffer, 0));
  testrun(!ov_turn_attr_frame_is_xor_relayed_address(buffer, 11));

  // min buffer min valid
  testrun(ov_turn_attr_frame_is_xor_relayed_address(buffer, 12));
...
int test_ov_turn_attr_xor_relayed_address_encoding_length() {

  struct sockaddr_storage sa = {0};

  sa.ss_family = AF_UNIX;
  testrun(0 == ov_turn_attr_xor_relayed_address_encoding_length(&sa));

  sa.ss_family = AF_INET;
  testrun(12 == ov_turn_attr_xor_relayed_address_encoding_length(&sa));

  sa.ss_family = AF_INET6;
  testrun(24 == ov_turn_attr_xor_relayed_address_encoding_length(&sa));
...
int test_ov_turn_attr_xor_relayed_address_encode() {
  ...
  testrun(!ov_turn_attr_xor_relayed_address_encode(NULL, 0, NULL, NULL, NULL));
  testrun(
      !ov_turn_attr_xor_relayed_address_encode(NULL, size, head, NULL, &sa));
  testrun(!ov_turn_attr_xor_relayed_address_encode(buffer, 0, head, NULL, &sa));
  testrun(
      !ov_turn_attr_xor_relayed_address_encode(buffer, size, head, NULL, NULL));
  testrun(
      !ov_turn_attr_xor_relayed_address_encode(buffer, size, NULL, NULL, &sa));

  testrun(
      !ov_turn_attr_xor_relayed_address_encode(buffer, 11, head, NULL, &sa));
  testrun(ov_turn_attr_xor_relayed_address_encode(buffer, 12, head, NULL, &sa));
  testrun(TURN_XOR_RELAYED_ADDRESS == ov_stun_attribute_get_type(buffer, size));
  ...
  testrun(
      !ov_turn_attr_xor_relayed_address_encode(buffer, 12, head, &next, &sa));
  testrun(
      !ov_turn_attr_xor_relayed_address_encode(buffer, 23, head, &next, &sa));
  testrun(
      ov_turn_attr_xor_relayed_address_encode(buffer, 24, head, &next, &sa));
  testrun(TURN_XOR_RELAYED_ADDRESS == ov_stun_attribute_get_type(buffer, size));
  ...
  testrun(
      ov_turn_attr_xor_relayed_address_encode(buffer, size, head, &next, &sa));
  testrun(TURN_XOR_RELAYED_ADDRESS == ov_stun_attribute_get_type(buffer, size));
  testrun(20 == ov_stun_attribute_get_length(buffer, size));
  testrun(next = buffer + 24);
  testrun(buffer[4] == 0);
...
int test_ov_turn_attr_xor_relayed_address_decode() {
  ...
  *(uint32_t *)(buffer + 8) = htonl(ip ^ cookie);
  ...
  testrun(!ov_turn_attr_xor_relayed_address_decode(NULL, 0, head, NULL));
  testrun(!ov_turn_attr_xor_relayed_address_decode(buffer, size, head, NULL));
  testrun(!ov_turn_attr_xor_relayed_address_decode(buffer, 0, head, &address));
  testrun(!ov_turn_attr_xor_relayed_address_decode(NULL, size, head, &address));
  testrun(
      !ov_turn_attr_xor_relayed_address_decode(buffer, size, NULL, &address));

  // check min
  testrun(!ov_turn_attr_xor_relayed_address_decode(buffer, 11, head, &address));
  testrun(ov_turn_attr_xor_relayed_address_decode(buffer, 12, head, &address));
  testrun(address);
  ...
  testrun(
      !ov_turn_attr_xor_relayed_address_decode(buffer, size, head, &address));
  testrun(ov_stun_attribute_set_type(buffer, size, TURN_XOR_RELAYED_ADDRESS));
  testrun(
      ov_turn_attr_xor_relayed_address_decode(buffer, size, head, &address));
...
int all_tests() {
  ...
  testrun_init();
  testrun_test(test_ov_turn_attr_frame_is_xor_relayed_address);

  testrun_test(test_ov_turn_attr_xor_relayed_address_encoding_length);
  testrun_test(test_ov_turn_attr_xor_relayed_address_encode);
  testrun_test(test_ov_turn_attr_xor_relayed_address_decode);
