...
int test_ov_turn_attr_reservaton_token_encode() {
    ...
    testrun(
        !ov_turn_attr_reservaton_token_encode(buffer, size, NULL, nonce, 0));
    ...
    memset(buf, 0, size);
    testrun(
        !ov_turn_attr_reservaton_token_encode(buffer, size, NULL, nonce, 2));
...
int test_ov_turn_attr_reservaton_token_decode() {
    ...
    testrun(
        !ov_turn_attr_reservaton_token_decode(buffer, size, &name, &length));
