    ...
    *(buf + 5) = 0x01;
    ...
    for (int i = 0; i < 0xff; i++) {
        ...
        *(buf + 5) = i;
        if (i == 0x01) {
            testrun(
                ov_stun_attribute_frame_is_xor_mapped_address(buffer, size));
        } else {
            testrun(
                !ov_stun_attribute_frame_is_xor_mapped_address(buffer, size));
        }
    ...
    for (int i = 0; i < 0xff; i++) {
        ...
        *(buf + 5) = 0x02;
        ...
        testrun(
            ov_stun_attribute_set_type(buffer, size, STUN_XOR_MAPPED_ADDRESS));
    }
...
int test_ov_stun_xor_mapped_address_encode() {
    ...
    testrun(STUN_XOR_MAPPED_ADDRESS ==
