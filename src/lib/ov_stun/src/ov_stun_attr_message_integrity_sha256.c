...
bool ov_stun_attribute_frame_is_message_integrity_sha256(const uint8_t *buffer,
    ...
    if (type != STUN_ATTR_MESSAGE_INTEGRITY_SHA256) goto error;
...
bool ov_stun_add_message_integrity_sha256(uint8_t *head,
    ...
    if (!head || !start || !key || length < 56 || keylen < 1) goto error;
    ...
    if (!ov_stun_attribute_set_type(
            start, 24, STUN_ATTR_MESSAGE_INTEGRITY_SHA256))
        goto error;

    if (!ov_stun_attribute_set_length(start, 24, 32)) goto error;
    ...
    if (!ov_hmac(
            OV_HASH_SHA256, head, len - 36, key, keylen, content, &hmac_len))
        goto error;

    // check HMAC length
    if (hmac_len != 32) goto error;

    if (next) *next = start + 36;
...
bool ov_stun_check_message_integrity_sha256(uint8_t *head,
    ...
    if (!head || length < 56 || !attr || attr_size < 1) return false;
    ...
    // check HMAC
    if (0 != memcmp(hmac_buffer, integrity + 4, 32)) return false;
