/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           ov_turn_attr_xor_relayed_address.c
        ...
        ------------------------------------------------------------------------
*/
#include "../include/ov_turn_attr_xor_relayed_address.h"
...
bool ov_turn_attr_frame_is_xor_relayed_address(const uint8_t *buffer,
                                               size_t length) {
  ...
  if (type != TURN_XOR_RELAYED_ADDRESS)
    goto error;
...
size_t ov_turn_attr_xor_relayed_address_encoding_length(
...
bool ov_turn_attr_xor_relayed_address_encode(
  ...
  if (!buffer || !sa || !head || length < 12)
    goto error;

  struct sockaddr_in *sock4 = NULL;
  struct sockaddr_in6 *sock6 = NULL;

  if (!ov_stun_attribute_set_type(buffer, length, TURN_XOR_RELAYED_ADDRESS))
    goto error;
  ...
  case AF_INET:
    ...
    if (!memcpy(content + 4, &ip, sizeof(struct in_addr)))
      goto error;
...
bool ov_turn_attr_xor_relayed_address_decode(
    const uint8_t *buffer, size_t length, const uint8_t *head,
    struct sockaddr_storage **address) {

  bool created = false;
  if (!buffer || !head || length < 12 || !address)
    goto error;

  if (!ov_turn_attr_frame_is_xor_relayed_address(buffer, length))
    goto error;
  ...
  case 0x02:
    ...
    for (int i = 0; i < 16; i++) {
      ...
      sock6->sin6_addr.s6_addr[i] = sock6->sin6_addr.s6_addr[i] ^ head[4 + i];
    }
