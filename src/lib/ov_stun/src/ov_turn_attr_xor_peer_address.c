...
bool ov_turn_attr_xor_peer_address_encode(uint8_t *buffer, size_t length,
...
bool ov_turn_attr_xor_peer_address_decode(const uint8_t *buffer, size_t length,
                                          const uint8_t *head,
  ...
  if (!ov_turn_attr_frame_is_xor_peer_address(buffer, length))
    goto error;
