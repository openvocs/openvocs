...
int test_ov_stun_generate_binding_request_plain() {
    ...
    testrun(
        ov_stun_attribute_frame_is_fingerprint(arr[1], len - (arr[1] - buf)));
...
int test_ov_stun_generate_binding_request() {
    ...
    testrun(ov_stun_frame_slice(buf, len, arr, az));
    testrun(
        ov_stun_attribute_frame_is_fingerprint(arr[0], len - (arr[0] - buf)));
    testrun(ov_stun_check_fingerprint(buf, len, arr, az, true));
    testrun(arr[1] == NULL);
    memset(buf, 0, size);
    ...
    testrun(
        ov_stun_nonce_decode(arr[0], len - (arr[0] - buf), &content, &clen));
    testrun(0 == memcmp(nonce, content, clen));
    testrun(clen == strlen(nonce));
    testrun(arr[1] == NULL);
    ...
    testrun(
        ov_stun_realm_decode(arr[0], len - (arr[0] - buf), &content, &clen));
    testrun(0 == memcmp(realm, content, clen));
    testrun(clen == strlen(realm));
    testrun(arr[1] == NULL);
    // reset
