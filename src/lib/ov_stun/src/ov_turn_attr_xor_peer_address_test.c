...
int test_ov_turn_attr_xor_peer_address_encode() {
  ...
  testrun(!ov_turn_attr_xor_peer_address_encode(buffer, size, NULL, NULL, &sa));
  ...
  testrun(1 == inet_pton(AF_INET6, "face:aaaa:bbbb:cccc:dddd:eeee:f1f1:1234",
                         &sock6->sin6_addr));
  testrun(!ov_turn_attr_xor_peer_address_encode(buffer, 12, head, &next, &sa));
  testrun(!ov_turn_attr_xor_peer_address_encode(buffer, 23, head, &next, &sa));
  ...
  testrun(ov_turn_attr_xor_peer_address_encode(buffer, size, head, &next, &sa));
  testrun(TURN_XOR_PEER_ADDRESS == ov_stun_attribute_get_type(buffer, size));
  testrun(20 == ov_stun_attribute_get_length(buffer, size));
  testrun(next = buffer + 24);
  testrun(buffer[4] == 0);
...
int test_ov_turn_attr_xor_peer_address_decode() {
  ...
  *(uint32_t *)(buffer + 8) = htonl(ip ^ cookie);
  ...
  testrun(!ov_turn_attr_xor_peer_address_decode(buffer, size, NULL, &address));
