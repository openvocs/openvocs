...
bool ov_turn_attr_is_address_error_code(const uint8_t *buffer, size_t length) {
    ...
    if (type != TURN_ADDRESS_ERROR_CODE) goto error;
...
bool ov_turn_attr_address_error_code_encode(uint8_t *buffer,
    ...
    if (!buffer || !phrase || (phrase_len > OV_TURN_PHRASE_MAX)) goto error;
    ...
    if ((code < 99) || (code > 999)) goto error;
...
bool ov_turn_attr_address_error_code_decode(const uint8_t *buffer,
    ...
    if (!ov_turn_attr_is_address_error_code(buffer, length)) goto error;
