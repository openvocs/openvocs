/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           ov_turn_attr_requested_addr_family.c
        ...
        ------------------------------------------------------------------------
*/
#include "../include/ov_turn_attr_requested_addr_family.h"
...
bool ov_turn_attr_is_requested_addr_family(const uint8_t *buffer,
    ...
    if (type != TURN_REQUESTED_ADDRESS_FAMILY) goto error;
...
size_t ov_turn_attr_requested_addr_family_encoding_length() { return 8; }
...
bool ov_turn_attr_requested_addr_family_encode(uint8_t *buffer,
    ...
    size_t len = ov_turn_attr_requested_addr_family_encoding_length();
    ...
    return ov_stun_attribute_encode(
        buffer, length, next, TURN_REQUESTED_ADDRESS_FAMILY, buf, 4);
error:
    return false;
}
...
bool ov_turn_attr_requested_addr_family_decode(const uint8_t *buffer,
    ...
    if (!ov_turn_attr_is_requested_addr_family(buffer, length)) goto error;
