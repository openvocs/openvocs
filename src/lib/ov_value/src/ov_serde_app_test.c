...
static bool serde_state_equals(ov_serde *serde, size_t state) {
  ...
  if (0 == ts) {
    ...
    testrun_log("Serde state %zu fits expected state %zu", ts->state, state);
    return true;
  } else {
    testrun_log("Serde state %zu does not fit expected state %zu", ts->state,
                state);
    return false;
  }
}
...
static int test_ov_serde_app_register_handler() {
  ...
  testrun(ov_serde_app_register_handler(res.app, SERDE_MAGIC_BYTES, handler1));
