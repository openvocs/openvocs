...
static bool cb_event_register(void *userdata, const int socket,
...
static bool cb_event_unregister(void *userdata, const int socket,
                                const ov_event_parameter *params,
                                ov_json_value *input) {
...
static bool check_config(ov_vocs_events_config *config) {
  ...
  if (!config->loop)
    goto error;
  if (0 == config->socket.manager.host[0])
    goto error;
...
ov_vocs_events *ov_vocs_events_cast(const void *data) {
  ...
  if (*(uint16_t *)data != OV_VOCS_EVENTS_MAGIC_BYTES)
    return NULL;
...
ov_vocs_events_config_from_json(const ov_json_value *val) {
...
static bool send_data(const void *key, void *val, void *data) {
  ...
  if (!ov_event_connection_send(conn, out)) {
    ...
    ov_log_error("failed to forward at %i",
                 ov_event_connection_get_socket(conn));
  }
...
void ov_vocs_events_emit_client_authorize(ov_vocs_events *self,
  ...
  if (!self || !session_id || !user_id || !role_id)
    goto error;
...
void ov_vocs_events_emit_user_session_created(ov_vocs_events *self,
  ...
  if (!self || !session_id || !client_id || !user_id)
    goto error;
...
void ov_vocs_events_emit_participation_state(
  ...
  if (!self || !session_id || !client_id || !user_id || !role_id || !loop_id ||
      !str_state)
    goto error;
