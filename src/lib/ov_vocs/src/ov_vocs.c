...
static void cb_backend_mixer_acquired(void *userdata, const char *uuid,
                                      const char *session_id,
                                      uint64_t error_code,
                                      const char *error_desc);

static void cb_backend_mixer_released(void *userdata, const char *uuid,
                                      const char *session_id,
                                      uint64_t error_code,
                                      const char *error_desc);

static void cb_backend_mixer_join(void *userdata, const char *uuid,
                                  const char *session_id, const char *loopname,
                                  uint64_t error_code, const char *error_desc);

static void cb_backend_mixer_leave(void *userdata, const char *uuid,
                                   const char *session_id, const char *loopname,
                                   uint64_t error_code, const char *error_desc);

static void cb_backend_mixer_volume(void *userdata, const char *uuid,
                                    const char *session_id,
                                    const char *loopname, uint8_t volume,
                                    uint64_t error_code,
                                    const char *error_desc);

static void cb_backend_mixer_state(void *userdata, const char *uuid,
...
static bool env_send(ov_vocs *vocs, int socket, const ov_json_value *input) {
  ...
  bool result = vocs->config.env.send(vocs->config.env.userdata, socket, input);
...
static bool send_error_response(ov_vocs *vocs, const ov_json_value *input,
                                int socket, uint64_t code, const char *desc) {
...
static bool send_success_response(ov_vocs *vocs, const ov_json_value *input,
                                  int socket, ov_json_value **response) {
...
static bool send_switch_loop_broadcast(ov_vocs *vocs, int socket,
                                       const char *loop,
  ...
  const char *client = ov_json_string_get(ov_json_get(data, "/" OV_KEY_CLIENT));
  ...
  if (!ov_json_object_set(par, OV_KEY_PARTICIPANTS, participants))
    goto error;
  ...
  if (!ov_broadcast_registry_send(vocs->broadcasts, loop, &parameter, out,
                                  OV_LOOP_BROADCAST))
    goto error;
...
static bool send_switch_loop_user_broadcast(ov_vocs *vocs, int socket,
                                            const char *loop,
                                            ov_vocs_permission current) {
  ...
  if (!ov_broadcast_registry_send(vocs->broadcasts, user, &parameter, out,
                                  OV_USER_BROADCAST))
    goto error;
...
static bool send_switch_volume_user_broadcast(ov_vocs *vocs, int socket,
                                              const char *loop,
                                              uint8_t volume) {
...
static bool send_talking_loop_broadcast(ov_vocs *vocs, int socket,
  ...
  if (state) {
    ...
    val = NULL;
    if (!ov_json_value_copy((void **)&val, state))
      goto error;
...
static bool close_participation(const void *key, void *val, void *data) {
  ...
  default:
    if (!send_switch_loop_broadcast(c->vocs, c->socket, key, OV_VOCS_NONE)) {
...
static bool drop_connection(ov_vocs *vocs, int socket, bool frontend,
                            bool backend) {
  ...
  if (NULL != session) {
    ...
    if (backend &&
        !ov_mc_backend_release_mixer(vocs->backend, uuid, session, vocs,
...
static void cb_backend_mixer_acquired(void *userdata, const char *uuid,
  ...
  default:
    ...
    ov_log_error("acquire mixer failed %i|%s for %s", error_code, error_desc,
                 session_id);
  ...
  if (ov_json_is_true(ov_json_get(data, "/" OV_KEY_ICE))) {
    ...
    if (!ov_event_api_set_parameter(out))
      goto error;
...
drop_mixer_acquisition:
  ...
  ov_mc_backend_release_mixer(vocs->backend, new_uuid, session_id, vocs,
                              cb_backend_mixer_released);
...
static void cb_backend_mixer_released(void *userdata, const char *uuid,
  ...
  default:
    ...
    ov_log_error("release mixer failed %i|%s for %s", error_code, error_desc,
                 session_id);
...
static bool perform_switch_loop_request(ov_vocs *vocs, const char *uuid,
                                        const char *sess, const char *user,
                                        const char *role, const char *loop,
                                        ov_vocs_permission current,
                                        ov_vocs_permission request) {

  if (!vocs || !uuid || !sess || !loop)
    goto error;
  ...
  case OV_VOCS_NONE:
    ...
    case OV_VOCS_SEND:
      ...
      ov_vocs_recorder_join_loop(vocs->recorder, user, role, loop);

      return ov_mc_backend_join_loop(vocs->backend, uuid, sess, data, vocs,
  ...
  case OV_VOCS_RECV:
    ...
    case OV_VOCS_NONE:
      ...
      ov_vocs_recorder_leave_loop(vocs->recorder, user, role, loop);

      return ov_mc_backend_leave_loop(vocs->backend, uuid, sess, loop, vocs,
    ...
    case OV_VOCS_SEND:
      ...
      ov_vocs_recorder_talk_on_loop(vocs->recorder, user, role, loop);

      return ov_mc_frontend_talk(vocs->frontend, uuid, sess, true, data);
  ...
  case OV_VOCS_SEND:
    ...
    case OV_VOCS_RECV:
      ...
      ov_vocs_recorder_talk_off_loop(vocs->recorder, user, role, loop);

      return ov_mc_frontend_talk(vocs->frontend, uuid, sess, false, data);
...
static bool set_loop_state_in_data(ov_json_value *data, const char *loop,
  ...
  ov_json_object_set(loops, loop,
                     ov_json_string(ov_vocs_permission_to_string(permission)));
  return true;
}
...
static void cb_backend_mixer_join(void *userdata, const char *uuid,
  ...
  ov_vocs *vocs = ov_vocs_cast(userdata);
  if (!vocs || !loopname || !uuid || !session_id)
    goto error;
  ...
  if (!set_loop_state_in_data(data, loopname, current))
    goto drop;
  ...
  if (current != request) {
    ...
    if (!perform_switch_loop_request(vocs, uuid, session_id, user, role,
                                     loopname, current, request)) {

      ov_log_error("Failed to perform talk switch %s session %s|%s|%s",
                   loopname, session_id, user, role);
    ...
    if (!ov_json_object_set(out, OV_KEY_STATE, val))
      goto drop;

    val = ov_json_string(loopname);
    if (!ov_json_object_set(out, OV_KEY_LOOP, val))
      goto drop;
...
switch_off_loop:
  ov_mc_backend_leave_loop(vocs->backend, uuid, session_id, loopname, vocs,
                           cb_backend_mixer_leave);
  goto error;
...
static void cb_backend_mixer_leave(void *userdata, const char *uuid,
  ...
  if (!set_loop_state_in_data(data, loopname, OV_VOCS_NONE))
    goto drop;
  if (!ov_vocs_db_set_state(vocs->config.db, user, role, loopname,
                            OV_VOCS_NONE))
    goto drop;
...
static void cb_backend_mixer_state(void *userdata, const char *uuid,
  ...
  if (!ov_json_object_set(ov_event_api_get_response(orig), OV_KEY_BACKEND, val))
    goto error;
...
static bool cb_client_process(void *userdata, const int socket,
  ...
  bool (*function)(ov_vocs *vocs, int socket, const ov_event_parameter *params,
...
static bool client_logout(ov_vocs *vocs, int socket,
                          const ov_event_parameter *params,
                          ov_json_value *input) {
...
static void cb_ldap_authentication(void *userdata, const char *uuid,
  ...
  case OV_LDAP_AUTH_REJECTED:
    ...
    ov_log_error("LDAP AUTHENTICATE failed at %i | %s", adata.socket, user);

    send_error_response(vocs, adata.value, adata.socket, OV_ERROR_CODE_AUTH,
                        OV_ERROR_DESC_AUTH);
  ...
  case OV_LDAP_AUTH_GRANTED:
    ov_log_info("LDAP AUTHENTICATE granted at %i | %s", adata.socket, user);
    break;
  }
  ...
  if (!ov_broadcast_registry_set(vocs->broadcasts, user, adata.socket,
                                 OV_USER_BROADCAST)) {
    goto error;
  }
  ...
  if (!ov_broadcast_registry_set(vocs->broadcasts,
                                 ...
                                 adata.socket, OV_SYSTEM_BROADCAST)) {
    goto error;
  }
  ...
  out = ov_json_object();
  if (!ov_vocs_json_set_id(out, user))
    goto error;
  if (!ov_vocs_json_set_session_id(out, session_id))
    goto error;
  ...
  if (result) {
    ...
    ov_log_error("VOCS AUTHENTICATE LDAP failed at %i | %s", adata.socket,
                 user);
  }
...
static bool client_login(ov_vocs *vocs, int socket,
                         const ov_event_parameter *params,
                         ov_json_value *input) {
  ...
  if (user) {
    ...
    send_error_response(vocs, input, socket,
  ...
  if (session_user && user && pass) {
    ...
    if (ov_event_session_verify(vocs->user_sessions, client_id, user, pass)) {
  ...
  if (!user || !pass) {
    ...
    send_error_response(vocs, input, socket, OV_ERROR_CODE_PARAMETER_ERROR,
  ...
  if (vocs->config.ldap.enable) {
    ...
    return ov_ldap_authenticate_password(
        vocs->ldap, user, pass, uuid,
        (ov_ldap_auth_callback){.userdata = vocs,
                                .callback = cb_ldap_authentication});
  }

  if (!ov_vocs_db_authenticate(vocs->config.db, user, pass)) {

    send_error_response(vocs, input, socket, OV_ERROR_CODE_AUTH,
                        OV_ERROR_DESC_AUTH);
...
login_session_user:
  ...
  if (!ov_broadcast_registry_set(vocs->broadcasts, user, socket,
                                 OV_USER_BROADCAST)) {
    goto error;
  }

  if (!ov_broadcast_registry_set(vocs->broadcasts,
                                 OV_BROADCAST_KEY_SYSTEM_BROADCAST, socket,
                                 OV_SYSTEM_BROADCAST)) {
    goto error;
  }
...
static bool update_client_login(ov_vocs *vocs, int socket,
                                const ov_event_parameter *params,
                                ov_json_value *input) {
  ...
  if (!ov_event_session_update(vocs->user_sessions, client_id, user_id,
                               session_id)) {

    send_error_response(vocs, input, socket, OV_ERROR_CODE_PROCESSING_ERROR,
...
static bool client_media(ov_vocs *vocs, int socket,
                         const ov_event_parameter *params,
                         ov_json_value *input) {
  ...
  case OV_MEDIA_REQUEST:
    ...
    if (!ov_mc_frontend_create_session(vocs->frontend, uuid,
                                       OV_VOCS_DEFAULT_SDP)) {
  ...
  case OV_MEDIA_ANSWER:
    ...
    if (!session) {
      ...
      send_error_response(vocs, input, socket, OV_ERROR_CODE_SESSION_UNKNOWN,
                          OV_ERROR_DESC_SESSION_UNKNOWN);

      goto error;
    }

    if (!ov_mc_frontend_update_session(vocs->frontend, uuid, session, type,
                                       sdp)) {
...
static bool client_candidate(ov_vocs *vocs, int socket,
                             const ov_event_parameter *params,
                             ov_json_value *input) {
...
static bool client_end_of_candidates(ov_vocs *vocs, int socket,
                                     const ov_event_parameter *params,
                                     ov_json_value *input) {
...
static bool client_authorize(ov_vocs *vocs, int socket,
                             const ov_event_parameter *params,
                             ov_json_value *input) {
  ...
  if (role) {
    ...
    send_error_response(vocs, input, socket, OV_ERROR_CODE_NOT_IMPLEMENTED,
                        "current status: "
  ...
  if (!ov_broadcast_registry_set(vocs->broadcasts, role, socket,
                                 OV_ROLE_BROADCAST)) {
    goto error;
  }

  out = ov_json_object();
  if (!ov_vocs_json_set_id(out, role))
    goto error;
...
static bool client_get(ov_vocs *vocs, int socket,
  ...
  if (0 == strcmp(type, OV_KEY_USER)) {
    ...
    out = ov_json_object();
    if (!ov_json_object_set(out, OV_KEY_RESULT, val))
      goto error;
    ...
    val = ov_vocs_db_get_entity_domain(vocs->config.db, OV_VOCS_DB_USER, user);
    if (val)
      ov_json_object_set(out, OV_KEY_DOMAIN, val);
...
static bool client_user_roles(ov_vocs *vocs, int socket,
                              const ov_event_parameter *params,
                              ov_json_value *input) {
...
static bool add_loop_broadcast_and_state(const void *key, void *val,
                                         void *data) {
  ...
  return ov_broadcast_registry_set(c->vocs->broadcasts, name, c->socket,
                                   OV_LOOP_BROADCAST);
}

/*----------------------------------------------------------------------------*/

static bool client_role_loops(ov_vocs *vocs, int socket,
                              const ov_event_parameter *params,
                              ov_json_value *input) {
...
static bool client_event_get_recording(ov_vocs *vocs, int socket,
                                       const ov_event_parameter *params,
                                       ov_json_value *input) {
...
static bool client_event_sip_status(ov_vocs *vocs, int socket,
                                    const ov_event_parameter *params,
                                    ov_json_value *input) {
...
static bool client_switch_loop_state(ov_vocs *vocs, int socket,
                                     const ov_event_parameter *params,
                                     ov_json_value *input) {
  ...
  data = ov_socket_json_get(vocs->connections, socket);
  const char *user = ov_json_string_get(ov_json_get(data, "/" OV_KEY_USER));
  const char *sess = ov_json_string_get(ov_json_get(data, "/" OV_KEY_SESSION));
  const char *role = ov_json_string_get(ov_json_get(data, "/" OV_KEY_ROLE));
  ...
  if (!ov_vocs_permission_granted(permission, requested)) {
    ...
    send_error_response(vocs, input, socket, OV_ERROR_CODE_AUTH_PERMISSION,
  ...
  if (!perform_switch_loop_request(vocs, uuid, sess, user, role, loop, current,
                                   requested)) {
...
static bool client_switch_loop_volume(ov_vocs *vocs, int socket,
                                      const ov_event_parameter *params,
                                      ov_json_value *input) {
  ...
  if (!ov_mc_backend_set_loop_volume(vocs->backend, uuid, sess, loop, percent,
                                     vocs, cb_backend_mixer_volume)) {
...
static bool client_talking(ov_vocs *vocs, int socket,
                           const ov_event_parameter *params,
                           ov_json_value *input) {
  ...
  if (!send_talking_loop_broadcast(vocs, socket, params, loop, state,
                                   client_id)) {

    ov_log_error("Failed to send talking broadcast at %s from %s", loop, user);
  }
...
static bool client_calling(ov_vocs *vocs, int socket,
                           const ov_event_parameter *params,
                           ov_json_value *input) {
...
static bool client_hangup(ov_vocs *vocs, int socket,
                          const ov_event_parameter *params,
                          ov_json_value *input) {
...
static bool client_permit_call(ov_vocs *vocs, int socket,
                               const ov_event_parameter *params,
                               ov_json_value *input) {
...
static bool client_revoke_call(ov_vocs *vocs, int socket,
                               const ov_event_parameter *params,
                               ov_json_value *input) {
...
static bool client_call_function(ov_vocs *vocs, int socket,
                                 const ov_event_parameter *params,
                                 ov_json_value *input,
  ...
  if (!function(vocs->sip, uuid)) {
    ...
    send_error_response(vocs, adata.value, adata.socket,
...
static bool client_list_calls(ov_vocs *vocs, int socket,
                              const ov_event_parameter *params,
                              ov_json_value *input) {

  return client_call_function(vocs, socket, params, input,
                              ov_mc_backend_sip_list_calls);
}

/*----------------------------------------------------------------------------*/

static bool client_list_call_permissions(ov_vocs *vocs, int socket,
                                         const ov_event_parameter *params,
                                         ov_json_value *input) {

  return client_call_function(vocs, socket, params, input,
                              ov_mc_backend_sip_list_permissions);
}

/*----------------------------------------------------------------------------*/

static bool client_list_sip_status(ov_vocs *vocs, int socket,
                                   const ov_event_parameter *params,
                                   ov_json_value *input) {

  return client_call_function(vocs, socket, params, input,
                              ov_mc_backend_sip_get_status);
}

/*----------------------------------------------------------------------------*/

static bool client_event_set_keyset_layout(ov_vocs *vocs, int socket,
                                           const ov_event_parameter *params,
                                           ov_json_value *input) {
 ...
 */
  ...
  if (!domain || !name || !layout) {
    ...
    out = ov_event_api_create_error_response(
        input, OV_ERROR_CODE_PARAMETER_ERROR, OV_ERROR_DESC_PARAMETER_ERROR);
...
static bool client_event_get_keyset_layout(ov_vocs *vocs, int socket,
                                           const ov_event_parameter *params,
                                           ov_json_value *input) {
...
static bool client_event_set_user_data(ov_vocs *vocs, int socket,
                                       const ov_event_parameter *params,
                                       ov_json_value *input) {
...
static bool client_event_get_user_data(ov_vocs *vocs, int socket,
                                       const ov_event_parameter *params,
                                       ov_json_value *input) {
...
static bool client_event_register(ov_vocs *vocs, int socket,
                                  const ov_event_parameter *params,
                                  ov_json_value *input) {
...
static bool enable_websocket_function(ov_vocs *vocs) {
  ...
  if (!set_function(vocs->io, OV_EVENT_API_LOGOUT, client_logout))
    goto error;

  if (!set_function(vocs->io, OV_EVENT_API_LOGIN, client_login))
    goto error;
  ...
  if (!set_function(vocs->io, OV_EVENT_API_MEDIA, client_media))
    goto error;

  if (!set_function(vocs->io, OV_ICE_STRING_CANDIDATE, client_candidate))
    goto error;

  if (!set_function(vocs->io, OV_ICE_STRING_END_OF_CANDIDATES,
  ...
  if (!set_function(vocs->io, OV_EVENT_API_GET, client_get))
    goto error;
  ...
  if (!set_function(vocs->io, OV_EVENT_API_SWITCH_LOOP_STATE,
                    client_switch_loop_state))
    goto error;

  if (!set_function(vocs->io, OV_EVENT_API_SWITCH_LOOP_VOLUME,
  ...
  if (!set_function(vocs->io, OV_KEY_CALL, client_calling))
    goto error;

  if (!set_function(vocs->io, OV_KEY_HANGUP, client_hangup))
    goto error;
  ...
  if (!set_function(vocs->io, OV_KEY_LIST_CALL_PERMISSIONS,
  ...
  if (!set_function(vocs->io, OV_VOCS_DB_SET_KEYSET_LAYOUT,
                    client_event_set_keyset_layout))
    goto error;

  if (!set_function(vocs->io, OV_VOCS_DB_GET_KEYSET_LAYOUT,
                    client_event_get_keyset_layout))
    goto error;

  if (!set_function(vocs->io, OV_VOCS_DB_SET_USER_DATA,
                    client_event_set_user_data))
    goto error;

  if (!set_function(vocs->io, OV_VOCS_DB_GET_USER_DATA,
                    client_event_get_user_data))
    goto error;

  if (!set_function(vocs->io, OV_KEY_GET_RECORDING, client_event_get_recording))
    goto error;
...
static bool admin_login(ov_vocs *vocs, int socket, ov_json_value *input) {

  ov_json_value *domains = NULL;
  char *user = NULL;

  if (!vocs || !input)
    goto error;

  user = ov_string_dup(ov_json_string_get(
      ov_json_get(input, "/" OV_KEY_PARAMETER "/" OV_KEY_USER)));
  if (!user)
    goto error;

  if (!(client_login(vocs, socket, NULL, input)))
    goto error;

  domains = ov_vocs_db_get_admin_domains(vocs->config.db, user);
  if (!domains || ov_json_array_is_empty(domains))
    goto error;

  ov_data_pointer_free(user);
  domains = ov_json_value_free(domains);
  return true;
error:
  ov_data_pointer_free(user);
  ov_json_value_free(domains);
  ov_json_value_free(input);
  return false;
}
...
static bool admin_logout(ov_vocs *vocs, int socket, ov_json_value *input) {

  if (!vocs || !input)
    goto error;
  return client_logout(vocs, socket, NULL, input);
error:
  ov_json_value_free(input);
  return false;
}
...
/*----------------------------------------------------------------------------*/
/*
static bool admin_authorize(ov_vocs *vocs, int socket, ov_json_value *input) {
    ...
    const char *role = ov_json_string_get(
        ov_json_get(input, "/" OV_KEY_PARAMETER "/" OV_KEY_ROLE));

    const char *domain = ov_json_string_get(
        ov_json_get(input, "/" OV_KEY_PARAMETER "/" OV_KEY_DOMAIN));

    if (!user || !role || !domain || (0 != strcmp(role, OV_KEY_ADMIN))) {
    ...
    if (!ov_json_object_set(data, OV_KEY_ROLE, ov_json_string(role)))
        goto error;
    ...
    bool result = send_success_response(vocs, input, socket, &out);
...
static bool admin_state_mixer(ov_vocs *vocs, int socket, ov_json_value *input) {
  ...
  if (!ov_json_object_set(res, OV_KEY_MIXER, val))
    goto error;

  val = ov_json_number(count.used);
  if (!ov_json_object_set(res, OV_KEY_USED, val))
    goto error;
...
static bool admin_state_connections(ov_vocs *vocs, int socket,
                                    ov_json_value *input) {
  ...
  if (!ov_socket_json_for_each_set_data(vocs->connections, obj))
    goto error;
...
static bool admin_state_session(ov_vocs *vocs, int socket,
                                ov_json_value *input) {
...
static bool admin_start_recording(ov_vocs *vocs, int socket,
                                  ov_json_value *input) {
...
static bool admin_stop_recording(ov_vocs *vocs, int socket,
                                 ov_json_value *input) {
...
static bool admin_broadcast(ov_vocs *vocs, int socket, ov_json_value *input) {
  ...
  if (!ov_broadcast_registry_send(vocs->broadcasts,
                                  OV_BROADCAST_KEY_SYSTEM_BROADCAST, &parameter,
                                  input, OV_SYSTEM_BROADCAST)) {
...
static bool admin_get(ov_vocs *vocs, int socket, ov_json_value *input) {
  ...
  out = ov_event_api_create_success_response(input);
  ov_json_value *res = ov_event_api_get_response(out);
  if (!ov_json_object_set(res, OV_KEY_RESULT, val))
    goto error;
...
static bool admin_get_recorded_loops(ov_vocs *vocs, int socket,
                                     ov_json_value *input) {
...
static bool admin_get_all_loops(ov_vocs *vocs, int socket,
                                ov_json_value *input) {
...
static bool call_mixer_state(const void *key, void *val, void *data) {
  ...
  const char *user =
      ov_json_string_get(ov_json_get(ov_json_value_cast(val), "/" OV_KEY_USER));
  ...
  const char *session = ov_json_string_get(
      ov_json_get(ov_json_value_cast(val), "/" OV_KEY_SESSION));
  ...
  if (!ov_mc_backend_get_session_state(container->vocs->backend, uuid, session,
                                       container->vocs, cb_mixer_state)) {
  ...
  const char *role =
      ov_json_string_get(ov_json_get(ov_json_value_cast(val), "/" OV_KEY_ROLE));
...
static bool admin_get_mixer_state(ov_vocs *vocs, int socket,
                                  ov_json_value *input) {
  ...
  struct mixer_data container = (struct mixer_data){
      .vocs = vocs, .user = user, .socket = socket, .input = input};

  if (!ov_socket_json_for_each(vocs->connections, &container,
                               call_mixer_state)) {
...
static bool cb_admin_process(void *userdata, const int socket,
                             const ov_event_parameter *params,
                             ov_json_value *input) {
  ...
  if (ov_event_api_event_is(input, OV_KEY_LOGIN))
    return admin_login(vocs, socket, input);

  if (ov_event_api_event_is(input, OV_KEY_LOGOUT))
    return admin_logout(vocs, socket, input);
  ...
  if (ov_event_api_event_is(input, "get"))
    return admin_get(vocs, socket, input);

  if (ov_event_api_event_is(input, OV_EVENT_START_RECORD))
    return admin_start_recording(vocs, socket, input);

  if (ov_event_api_event_is(input, OV_EVENT_STOP_RECORD))
    return admin_stop_recording(vocs, socket, input);
  ...
  if (ov_event_api_event_is(input, OV_EVENT_API_STATE_SESSION))
    return admin_state_session(vocs, socket, input);

  if (ov_event_api_event_is(input, "get_mixer_state"))
    return admin_get_mixer_state(vocs, socket, input);
...
static void cb_frontend_session_created(
    void *userdata, const ov_response_state event, const char *session_id,
    const char *type, const char *sdp, size_t array_size,
  ...
  if (!orig) {
    ...
    if (vocs->debug)
      ov_log_debug("ignoring ice_session_create %s "
                   "with error %" PRIu64 " msg %s",
                   session_id, event.result.error_code, event.result.message);

    if (OV_ERROR_NOERROR == event.result.error_code)
  ...
  default:
    ...
    send_error_response(vocs, orig, socket, event.result.error_code,
  ...
  if (!ov_mc_backend_acquire_mixer(vocs->backend, event.id, session_id, forward,
                                   vocs, cb_backend_mixer_acquired)) {
  ...
  if (!ov_json_object_set(out, OV_KEY_SDP, val))
    goto drop_session;
...
static void cb_frontend_session_update(void *userdata,
  ...
  default:
    ...
    if (orig) {
      ...
      out = ov_event_api_create_error_response(orig, event.result.error_code,
                                               event.result.message);
...
static void cb_frontend_session_state(void *userdata,
  ...
  if (!ov_event_api_set_uuid(out, uuid))
    goto error;

  if (!ov_json_object_set(ov_event_api_get_response(out), OV_KEY_FRONTEND, val))
    goto error;
  ...
  if (!ov_mc_backend_get_session_state(vocs->backend, uuid, session_id, vocs,
                                       cb_backend_mixer_state))
    goto error;
...
static void cb_frontend_candidate(void *userdata, const ov_response_state event,
                                  const char *session_id,
  ...
  case OV_ERROR_NOERROR:
    ...
    if (orig) {
      ...
      out = ov_event_api_create_success_response(orig);
      if (!ov_json_object_set(out, OV_KEY_RESPONSE, val))
        goto error;
...
static void cb_frontend_end_of_candidates(void *userdata,
  ...
  case OV_ERROR_NOERROR:
    ...
    if (orig) {
      ...
      out =
          ov_event_api_message_create(OV_ICE_STRING_END_OF_CANDIDATES, NULL, 0);
...
static void cb_frontend_talk(void *userdata, const ov_response_state event,
                             const char *session_id,
                             const ov_mc_loop_data ldata, bool on) {
  ...
  default:
    ...
    send_error_response(vocs, orig, adata.socket, event.result.error_code,
                        event.result.message);
  ...
  if (!send_switch_loop_user_broadcast(vocs, adata.socket, loop, current)) {
    ...
    ov_log_error("failed to send switch loop user "
                 "broadcast %s",
                 loop);
  }
...
switch_off_loop:
  ...
  if (!ov_mc_backend_leave_loop(vocs->backend, event.id, session_id, loop, vocs,
                                cb_backend_mixer_leave))
    goto drop;
...
static void cb_call_init(void *userdata, const char *uuid, const char *loopname,
  ...
  default:
    ...
    out =
        ov_event_api_create_error_response(adata.value, error_code, error_desc);
  ...
  if (call_id) {
    ...
    if (!ov_json_object_set(par, OV_KEY_CALL_ID, val))
      goto error;
  }
...
response_loop:
  ...
  if (!ov_broadcast_registry_send(self->broadcasts, loopname, &parameter, out,
                                  OV_LOOP_BROADCAST)) {
...
static void cb_sip_new(void *userdata, const char *loopname,
  ...
  if (!ov_json_object_set(par, OV_KEY_PEER, val))
    goto error;
...
static void cb_sip_terminated(void *userdata, const char *call_id,
                              const char *loopname) {
...
static void cb_sip_permit(void *userdata, const ov_sip_permission permission,
                          uint64_t error_code, const char *error_desc) {
  ...
  if (0 != error_code) {
    ...
    if (!ov_json_object_set(out, OV_KEY_ERROR, val))
      goto error;
  ...
  if (!ov_broadcast_registry_send(self->broadcasts, permission.loop, &parameter,
                                  out, OV_LOOP_BROADCAST)) {
...
static void cb_sip_revoke(void *userdata, const ov_sip_permission permission,
                          uint64_t error_code, const char *error_desc) {
...
static void cb_sip_list_calls(void *userdata, const char *uuid,
  ...
  case OV_ERROR_NOERROR:
    ...
    if (!ov_json_object_set(par, OV_KEY_CALLS, val))
      goto error;
...
static void cb_sip_list_permissions(void *userdata, const char *uuid,
  ...
  case OV_ERROR_NOERROR:
    ...
    if (!ov_json_object_set(par, OV_KEY_PERMISSIONS, val))
      goto error;
...
static void cb_sip_get_status(void *userdata, const char *uuid,
  ...
  case OV_ERROR_NOERROR:
    ...
    if (!ov_json_object_set(par, OV_KEY_STATUS, val))
      goto error;
...
static void io_vad(void *userdata, const char *loop, bool on) {
  ...
  if (!ov_json_object_set(par, OV_KEY_LOOP, ov_json_string(loop)))
    goto error;
  ...
  if (!ov_json_object_set(par, OV_KEY_ON, val))
    goto error;
  ...
  if (!ov_broadcast_registry_send(self->broadcasts, loop, &parameter, out,
                                  OV_LOOP_BROADCAST))
    goto error;
...
static bool revoke_sip_call(void *item, void *data) {
  ...
  struct container_updata_sip *container = (struct container_updata_sip *)data;
...
static bool permit_sip_call(void *item, void *data) {
  ...
  return ov_mc_backend_sip_create_permission(container->vocs->sip, permission);
}
...
ov_vocs *ov_vocs_create(ov_vocs_config config) {
  ...
  if (config.trigger)
    ov_event_trigger_register_listener(
        config.trigger, "VOCS",
        (ov_event_trigger_data){.userdata = vocs, .process = process_trigger});
...
ov_vocs *ov_vocs_cast(const void *self) {
  ...
  if (*(uint16_t *)self == OV_VOCS_MAGIC_BYTES)
    return (ov_vocs *)self;
error:
  return NULL;
}
...
ov_event_io_config ov_vocs_admin_io_uri_config(ov_vocs *self) {
  ...
  return (ov_event_io_config){.name = OV_KEY_ADMIN,
                              .userdata = self,
                              .callback.close = cb_socket_close,
                              .callback.process = cb_admin_process};
}
