...
static bool create_test_db(ov_vocs_db *db) {
  ...
  if (!ov_vocs_db_set_password(db, "user11", "user11"))
    goto error;
  ...
  if (!ov_vocs_db_set_password(db, "user12", "user12"))
    goto error;
  ...
  if (!ov_vocs_db_set_password(db, "user13", "user13"))
    goto error;
  ...
  if (!ov_vocs_db_set_password(db, "user21", "user21"))
    goto error;
  ...
  if (!ov_vocs_db_set_password(db, "user22", "user22"))
    goto error;
  ...
  if (!ov_vocs_db_set_password(db, "user23", "user23"))
    goto error;
  ...
  if (!ov_vocs_db_set_password(db, "user31", "user31"))
    goto error;
  ...
  if (!ov_vocs_db_set_password(db, "user32", "user32"))
    goto error;
  ...
  if (!ov_vocs_db_set_password(db, "user33", "user33"))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop12", OV_KEY_ROLES,
                                    out))
    goto error;

  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop13", OV_KEY_ROLES,
                                    out))
    goto error;

  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop21", OV_KEY_ROLES,
                                    out))
    goto error;

  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop22", OV_KEY_ROLES,
                                    out))
    goto error;

  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop23", OV_KEY_ROLES,
                                    out))
    goto error;

  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop31", OV_KEY_ROLES,
                                    out))
    goto error;

  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop32", OV_KEY_ROLES,
                                    out))
    goto error;

  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop33", OV_KEY_ROLES,
                                    out))
    goto error;

  ov_json_value *mc = NULL;

  ov_socket_configuration_to_json((ov_socket_configuration){.type = UDP,
                                                            .host = "229.0.0.1",
                                                            .port = 10000},
                                  &mc);
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop12",
                                    OV_KEY_MULTICAST, mc))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop13",
                                    OV_KEY_MULTICAST, mc))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop21",
                                    OV_KEY_MULTICAST, mc))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop22",
                                    OV_KEY_MULTICAST, mc))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop23",
                                    OV_KEY_MULTICAST, mc))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop31",
                                    OV_KEY_MULTICAST, mc))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop32",
                                    OV_KEY_MULTICAST, mc))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_LOOP, "loop33",
                                    OV_KEY_MULTICAST, mc))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_ROLE, "role11", OV_KEY_USERS,
                                    out))
    goto error;
  ...
  if (!ov_vocs_db_update_entity_key(db, OV_VOCS_DB_ROLE, "role21", OV_KEY_USERS,
                                    out))
    goto error;
