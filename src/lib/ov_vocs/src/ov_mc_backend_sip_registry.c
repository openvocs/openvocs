...
ov_mc_backend_sip_registry *ov_mc_backend_sip_registry_free(
    ...
    if (!ov_mc_backend_sip_registry_cast(self)) return self;
...
bool ov_mc_backend_sip_registry_register_proxy(ov_mc_backend_sip_registry *self,
    ...
    if (uuid) strncpy(proxy->uuid, uuid, sizeof(ov_id));
    proxy->socket = socket;
...
static bool get_proxy_socket(const void *key, void *val, void *data) {
    ...
    if (c->socket < 1) c->socket = proxy->socket;
...
int ov_mc_backend_sip_registry_get_proxy_socket(
    ...
    if (!ov_dict_for_each(self->proxys, &c, get_proxy_socket)) goto error;
