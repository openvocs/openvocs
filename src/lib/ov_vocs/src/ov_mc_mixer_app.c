...
static void cb_connected(void *userdata, int socket) {
    ...
    ov_log_info("Mixer connected at %i from %s:%i to %s:%i",
                socket,
                app->local.host,
                app->local.port,
                app->remote.host,
...
static void cb_event_register(void *userdata,
    ...
    ov_mc_mixer_app *app = ov_mc_mixer_app_cast(userdata);
    if (!app || !name || socket < 0 || !input) goto error;
...
static void cb_event_configure(void *userdata,
                               const char *name,
                               int socket,
                               ov_json_value *input) {
...
static void cb_event_acquire(void *userdata,
    ...
    if (!ov_mc_mixer_core_release(app->mixer)) goto error_response;
    if (!ov_mc_mixer_core_set_name(app->mixer, user)) goto error_response;
    if (!ov_mc_mixer_core_set_forward(app->mixer, forward)) goto error_response;
...
static void cb_event_shutdown(void *userdata,
                              const char *name,
                              int socket,
                              ov_json_value *input) {
...
static bool register_app_callbacks(ov_mc_mixer_app *service) {
    ...
    if (!ov_event_app_register(
            service->app, OV_KEY_REGISTER, service, cb_event_register))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_CONFIGURE, service, cb_event_configure))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_ACQUIRE, service, cb_event_acquire))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_FORWARD, service, cb_event_forward))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_RELEASE, service, cb_event_release))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_JOIN, service, cb_event_join))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_LEAVE, service, cb_event_leave))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_VOLUME, service, cb_event_volume))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_SHUTDOWN, service, cb_event_shutdown))
        goto error;

    if (!ov_event_app_register(
            service->app, OV_KEY_STATE, service, cb_event_state))
        goto error;
...
ov_mc_mixer_app *ov_mc_mixer_app_create(ov_mc_mixer_app_config config) {
    ...
    if (0 == config.io) goto error;
    if (0 == config.manager.host[0]) goto error;
    ...
    if (!register_app_callbacks(app)) goto error;
...
ov_mc_mixer_app *ov_mc_mixer_app_cast(const void *data) {
    ...
    if (*(uint16_t *)data != OV_MC_MIXER_APP_MAGIC_BYTES) return NULL;
...
ov_mc_mixer_app *ov_mc_mixer_app_free(ov_mc_mixer_app *self) {
    ...
    if (!ov_mc_mixer_app_cast(self)) return self;
...
ov_mc_mixer_app_config ov_mc_mixer_app_config_from_json(
    const ov_json_value *c) {
