...
ov_mc_backend_registry *ov_mc_backend_registry_free(
    ...
    if (!ov_mc_backend_registry_cast(self)) return self;
...
ov_mc_backend_registry *ov_mc_backend_registry_cast(const void *data) {
    ...
    if (*(uint16_t *)data != OV_MC_BACKEND_REGISTRY_MAGIC_BYTES) return NULL;
...
bool ov_mc_backend_registry_register_mixer(ov_mc_backend_registry *self,
    ...
    if (data.socket > (int)self->sockets) goto error;
    if (data.socket < 1) goto error;
...
bool ov_mc_backend_registry_unregister_mixer(ov_mc_backend_registry *self,
    ...
    if (socket > (int)self->sockets) goto error;
...
ov_mc_backend_registry_count ov_mc_backend_registry_count_mixers(
    ...
    for (size_t i = 0; i < self->sockets; i++) {
        ...
        if (0 != self->socket[i].user[0]) counter.used++;
    }
...
ov_mc_mixer_data ov_mc_backend_registry_acquire_user(
    ...
    for (size_t i = 0; i < self->sockets; i++) {
        ...
        if (0 != self->socket[i].user[0]) continue;
