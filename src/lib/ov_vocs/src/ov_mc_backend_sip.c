...
static void cb_event_get_multicast(void *userdata, const char *name, int socket,
  ...
  if (0 == config.host[0]) {
    ...
    out = ov_event_api_create_error_response(input, OV_ERROR_CODE_UNKNOWN_LOOP,
                                             OV_ERROR_DESC_UNKNOWN_LOOP);
  ...
  if (0 == out) {
    ...
    out = ov_event_api_create_error_response(input, OV_ERROR_INTERNAL_SERVER,
...
static bool send_permission(void *item, void *data) {
  ...
  struct container_permission *container = (struct container_permission *)data;

  ov_sip_permission permission = {0};

  permission.loop = container->loop;
  permission.caller = ov_json_string_get(ov_json_get(item, "/" OV_KEY_CALLER));
  permission.callee = ov_json_string_get(ov_json_get(item, "/" OV_KEY_CALLEE));
...
static void cb_mixer_aquired(void *userdata, const char *uuid,
  ...
  if (!adata.value) {
    ...
    if (OV_ERROR_NO_ERROR == error_code) {
      ...
      ov_mc_backend_release_mixer(self->config.backend, uuid, user_uuid, self,
                                  cb_mixer_released);
...
static void cb_event_set_singlecast(void *userdata, const char *name,
  ...
  if (!ov_mc_backend_join_loop(self->config.backend, uuid_new, user, data, self,
                               cb_loop_joined)) {
...
static void cb_event_notify(void *userdata, const char *name, int socket,
  ...
  */
  ...
  if (0 == ov_string_compare(type, "new_call")) {
    ...
    if (!ov_mc_backend_sip_registry_register_call(self->registry, socket, id,
                                                  loop, peer))
      goto error;

    if (self->config.callback.call.new)
      self->config.callback.call.new(self->config.callback.userdata, loop, id,
                                     peer);
    ...
    if (self->config.callback.call.terminated)
      self->config.callback.call.terminated(self->config.callback.userdata, id,
                                            loop);
...
static void cb_event_call_response(ov_mc_backend_sip *self, int socket,
  ...
  */
  ...
  const char *call_id =
      ov_json_string_get(ov_json_get(input, "/" OV_KEY_RESPONSE "/" OV_KEY_ID));
  const char *uuid = ov_event_api_get_uuid(input);
  ...
  if (0 == code) {
    ...
    ov_mc_backend_sip_registry_register_call(self->registry, socket, call_id,
                                             loop, callee);
  }

  if (self->config.callback.call.init)
    self->config.callback.call.init(self->config.callback.userdata, uuid, loop,
                                    call_id, caller, callee, code, desc);
...
static void cb_event_list_permissions_response(ov_mc_backend_sip *self,
  ...
  if (self->config.callback.list_permissions)
    self->config.callback.list_permissions(self->config.callback.userdata, uuid,
                                           perm, error_code, error_desc);
...
static bool register_app_callbacks(ov_mc_backend_sip *self) {
  ...
  if (!ov_event_app_register(self->app, OV_KEY_ACQUIRE, self, cb_event_acquire))
    goto error;

  if (!ov_event_app_register(self->app, OV_KEY_RELEASE, self, cb_event_release))
    goto error;
...
ov_mc_backend_sip *ov_mc_backend_sip_create(ov_mc_backend_sip_config config) {
  ...
  self->registry =
      ov_mc_backend_sip_registry_create((ov_mc_backend_sip_registry_config){0});
