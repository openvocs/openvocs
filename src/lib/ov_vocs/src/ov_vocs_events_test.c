...
static bool read_conn_with_session(int conn, ov_event_loop *loop,
                                   const char *event) {
  ...
  if (!ov_event_api_event_is(out, event))
    goto error;
...
static bool read_conn_login(int conn, ov_event_loop *loop,
  ...
  if (!ov_event_api_event_is(out, OV_KEY_CLIENT_LOGIN))
    goto error;

  if (0 != strcmp(client_id, ov_json_string_get(ov_json_get(
                                 out, "/" OV_KEY_PARAMETER "/" OV_KEY_CLIENT))))
    goto error;

  if (0 != strcmp(user_id, ov_json_string_get(ov_json_get(
...
static bool read_conn_auth(int conn, ov_event_loop *loop, const char *sess_id,
                           const char *user_id, const char *role_id) {
  ...
  if (!ov_event_api_event_is(out, OV_KEY_AUTHORIZE))
    goto error;

  if (0 != strcmp(sess_id, ov_json_string_get(ov_json_get(
  ...
  if (0 != strcmp(role_id, ov_json_string_get(ov_json_get(
...
static bool read_conn_user_session(int conn, ov_event_loop *loop,
                                   const char *event, const char *sess_id,
                                   const char *client_id, const char *user_id) {
...
int test_ov_vocs_events_emit_user_session_created() {
  ...
  ov_vocs_events_emit_user_session_created(events, sess_id, client_id, user_id);

  testrun(read_conn_user_session(conn2, loop, OV_KEY_SESSION_CREATED, sess_id,
                                 client_id, user_id));
  testrun(read_conn_user_session(conn1, loop, OV_KEY_SESSION_CREATED, sess_id,
                                 client_id, user_id));
  testrun(read_conn_user_session(conn3, loop, OV_KEY_SESSION_CREATED, sess_id,
                                 client_id, user_id));
...
int test_ov_vocs_events_emit_user_session_closed() {
  ...
  ov_vocs_events_emit_user_session_closed(events, sess_id, client_id, user_id);

  testrun(read_conn_user_session(conn2, loop, OV_KEY_SESSION_CLOSED, sess_id,
                                 client_id, user_id));
  testrun(read_conn_user_session(conn1, loop, OV_KEY_SESSION_CLOSED, sess_id,
                                 client_id, user_id));
  testrun(read_conn_user_session(conn3, loop, OV_KEY_SESSION_CLOSED, sess_id,
                                 client_id, user_id));
...
static bool read_participation(int conn, ov_event_loop *loop,
                               const char *sess_id, const char *client_id,
                               const char *user_id, const char *role_id,
                               const char *loop_id, const char *state) {
  ...
  if (!ov_event_api_event_is(out, OV_KEY_STATE))
    goto error;
  ...
  if (0 != strcmp(loop_id, ov_json_string_get(ov_json_get(
                               out, "/" OV_KEY_PARAMETER "/" OV_KEY_LOOP))))
    goto error;

  if (0 != strcmp(state, ov_json_string_get(ov_json_get(
...
int test_ov_vocs_events_emit_participation_state() {
  ...
  ov_vocs_events_emit_participation_state(events, sess_id, client_id, user_id,
                                          role_id, loop_id, mcast,
                                          OV_PARTICIPATION_STATE_RECV);

  char const *state =
      ov_participation_state_to_string(OV_PARTICIPATION_STATE_RECV);

  testrun(read_participation(conn2, loop, sess_id, client_id, user_id, role_id,
                             loop_id, state));
  testrun(read_participation(conn1, loop, sess_id, client_id, user_id, role_id,
                             loop_id, state));
  testrun(read_participation(conn3, loop, sess_id, client_id, user_id, role_id,
                             loop_id, state));
...
static bool read_volume(int conn, ov_event_loop *loop, const char *sess_id,
                        const char *client_id, const char *user_id,
  ...
  if (!ov_event_api_event_is(out, OV_KEY_VOLUME))
    goto error;
...
int test_ov_vocs_events_emit_loop_volume() {
  ...
  ov_vocs_events_emit_loop_volume(events, sess_id, client_id, user_id, role_id,
                                  loop_id, mcast, volume);

  testrun(read_volume(conn2, loop, sess_id, client_id, user_id, role_id,
                      loop_id, volume));
  testrun(read_volume(conn1, loop, sess_id, client_id, user_id, role_id,
                      loop_id, volume));
  testrun(read_volume(conn3, loop, sess_id, client_id, user_id, role_id,
                      loop_id, volume));
...
static bool read_ptt(int conn, ov_event_loop *loop, const char *sess_id,
                     const char *client_id, const char *user_id,
                     const char *role_id, const char *loop_id) {
  ...
  if (!ov_event_api_event_is(out, OV_KEY_PTT))
    goto error;
  ...
  if (!ov_json_get(out, "/" OV_KEY_PARAMETER "/" OV_KEY_PTT))
    goto error;
...
int test_ov_vocs_events_emit_ptt() {
  ...
  ov_vocs_events_emit_ptt(events, sess_id, client_id, user_id, role_id, loop_id,
                          mcast, true);

  testrun(read_ptt(conn2, loop, sess_id, client_id, user_id, role_id, loop_id));
  testrun(read_ptt(conn1, loop, sess_id, client_id, user_id, role_id, loop_id));
  testrun(read_ptt(conn3, loop, sess_id, client_id, user_id, role_id, loop_id));
