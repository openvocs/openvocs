...
static ov_codec *impl_codec_create(uint32_t ssid,
  ...
  if (INT32_MAX < opus->sample_rate_hertz) {
    ...
    ov_log_error("Could not create Opus codec -  sample rate out of bounds: "
                 "%" PRIu32,
                 opus->sample_rate_hertz);
    goto error;
  }
...
static int32_t impl_encode(ov_codec *self, const uint8_t *input, size_t length,
  ...
  if (0 > encoded_bytes) {
    ...
    ov_log_error("Could not encode frame: %s", opus_strerror(encoded_bytes));
...
static int32_t impl_decode(ov_codec *self, uint64_t seq_number,
  ...
  */
  ...
  if (0 > samples_decoded) {
    ...
    ov_log_error("Could not decode frame: %s", opus_strerror(samples_decoded));
