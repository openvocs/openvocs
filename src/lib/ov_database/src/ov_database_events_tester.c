...
static ov_database_info (*database_info_getter)() = get_database_info_sqlite;

void set_database_info_getter(ov_database_info (*info_getter)(void)) {

    database_info_getter = info_getter;

}

size_t secs_to_wait_for_reconnect = 0;

void set_database_wait_for_reconnect_secs(size_t secs) {
    secs_to_wait_for_reconnect = secs;
}

/*----------------------------------------------------------------------------*/

static ov_database *connect_to_db() {
    ov_database *db = ov_database_connect(database_info_getter());
    fprintf(stderr, "Waiting %zu seconds to give database time to connect...\n",
            secs_to_wait_for_reconnect);
    sleep(secs_to_wait_for_reconnect);
...
int test_ov_db_recordings_remove() { return testrun_log_success(); }
...
int test_ov_db_recordings_get() {
    ...
    testrun(
        recs_found(db, expected_loops(B), ids, search_params(.id = ids[B])));
    ...
    testrun(
        recs_found(db, expected_loops(B), ids,
    ...
    testrun(ov_db_events_add_participation_state(
        db, "ernst", "role2", "B", OV_PARTICIPATION_STATE_RECV, 0));
    testrun(ov_db_events_add_participation_state(
        db, "ernst", "role2", "B", OV_PARTICIPATION_STATE_RECV, 102));

    testrun(ov_db_events_add_participation_state(
        db, "bauer", "role2", "C", OV_PARTICIPATION_STATE_RECV, 1));

    testrun(ov_db_events_add_participation_state(
        db, "bauer", "role2", "D", OV_PARTICIPATION_STATE_RECV, 4));
    testrun(ov_db_events_add_participation_state(
        db, "conrad", "role2", "D", OV_PARTICIPATION_STATE_RECV, 104));

    testrun(ov_db_events_add_participation_state(
        db, "bauer", "role2", "I", OV_PARTICIPATION_STATE_RECV, 9));
