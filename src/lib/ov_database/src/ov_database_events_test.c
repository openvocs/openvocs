...
static bool pstates_user_is_there(ov_json_value const *jval, char const *user) {
    ...
    fprintf(stderr,
            "Looking for pstate event for user: %s\n",
...
static bool recs_found(ov_database *db,
                       ssize_t const *recs_expected,
                       ov_id const all_recs[],
...
static int test_ov_db_recordings_get() {
    ...
    testrun(recs_found(db,
                       expected_loops(E, F, G, H, I, K, L),
                       ids,
...
OV_TEST_RUN("ov_database_events",
            test_ov_db_prepare,
            test_ov_db_events_add_participation_state,
            test_ov_db_events_get_partitipation_state,
            test_ov_db_recordings_add,
            test_ov_db_recordings_remove,
