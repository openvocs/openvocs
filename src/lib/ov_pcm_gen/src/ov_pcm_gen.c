...
void *create_internal_buffer(const ov_pcm_gen_from_file *config) {
  ...
  if (0 == codec_config) {
    ...
    snprintf(codec_config_str, sizeof(codec_config_str) - 1, "{\"%s\":\"%s\"}",
             OV_KEY_CODEC, ov_codec_pcm16_signed_id());

    codec_config_str[255] = 0;

    codec_config =
        ov_json_value_from_string(codec_config_str, strlen(codec_config_str));
...
bool ov_pcm_gen_config_print(FILE *out, ov_pcm_gen_config const *config,
  ...
  fprintf(out, "%*.sSample rate (Hz): %f    Frame length(usec): %" PRIu64 "\n",
