...
static void cb_session_dropped(void *userdata, const char *session_id) {
    ...
    if (session_id) strncpy(data->session_id, session_id, sizeof(ov_id));
    return;
}

/*----------------------------------------------------------------------------*/

static void cb_session_created(
    void *userdata,
    ...
    struct userdata *data = (struct userdata *)userdata;
    if (session_id) strncpy(data->session_id, session_id, sizeof(ov_id));
    if (event.id) strncpy(data->uuid, event.id, sizeof(ov_id));
    ...
    if (array) data->forward = array[0];
    return;
}

/*----------------------------------------------------------------------------*/

static void cb_session_completed(void *userdata,
                                 const char *session_id,
                                 bool success) {
...
static void cb_session_update(void *userdata,
                              const ov_response_state event,
                              const char *session_id) {
...
static void cb_session_state(void *userdata,
                             const ov_response_state event,
                             const char *session_id,
...
static void cb_candidate(void *userdata,
                         const ov_response_state event,
                         const char *session_id,
...
static void cb_end_of_candidates(void *userdata,
                                 const ov_response_state event,
                                 const char *session_id,
                                 const ov_ice_candidate_info *info) {
...
static void cb_talk(void *userdata,
                    const ov_response_state event,
                    const char *session_id,
...
static ov_json_value *msg_ice_session_create_response(
    const char *uuid,
    const char *sdp,
    const char *session_id,
    const char *type,
    ...
    arr = ov_json_array();
    if (!ov_json_object_set(res, OV_KEY_PROXY, arr)) goto error;

    msg = ov_json_object();
    if (!ov_json_array_push(arr, msg)) goto error;

    val = ov_json_number(forward.ssrc);
    if (!ov_json_object_set(msg, OV_KEY_SSRC, val)) goto error;

    val = NULL;
    if (!ov_socket_configuration_to_json(forward.socket, &val)) goto error;
    if (!ov_json_object_set(msg, OV_KEY_SOCKET, val)) goto error;
...
int check_cb_event_ice_session_create_response() {
    ...
    testrun(conn == ov_mc_frontend_registry_get_session_socket(
                        frontend->registry, "2-2-2-2"));
...
int check_cb_event_ice_session_drop() {
    ...
    testrun(ov_mc_frontend_registry_register_session(
        frontend->registry, conn, "1-1-1-1"));

    testrun(ov_mc_frontend_registry_register_session(
        frontend->registry, conn, "2-1-1-1"));

    testrun(ov_mc_frontend_registry_register_session(
        frontend->registry, conn, "3-1-1-1"));

    testrun(conn == ov_mc_frontend_registry_get_session_socket(
                        frontend->registry, "1-1-1-1"));

    testrun(conn == ov_mc_frontend_registry_get_session_socket(
                        frontend->registry, "2-1-1-1"));

    testrun(conn == ov_mc_frontend_registry_get_session_socket(
                        frontend->registry, "3-1-1-1"));
    ...
    testrun(0 == ov_mc_frontend_registry_get_session_socket(
                     frontend->registry, "2-1-1-1"));
...
int check_cb_event_ice_session_update_response() {
    ...
    msg = ov_ice_proxy_vocs_msg_update_session(
        "1-1-1-1", "2-2-2-2", OV_MEDIA_OFFER, "sdp");
...
int check_cb_event_ice_candidate() {
    ...
    ov_ice_candidate_info info =
        (ov_ice_candidate_info){.candidate = "candidate",
                                .SDPMlineIndex = 0,
                                .SDPMid = 0,
                                .ufrag = "ufrag"};
...
int check_cb_event_talk_response() {
    ...
    msg = ov_ice_proxy_vocs_msg_talk("1-1-1-1",
                                     "2-2-2-2",
                                     true,
...
int test_ov_mc_frontend_update_session() {
    ...
    testrun(!ov_mc_frontend_update_session(
        frontend, "1-1-1-1", "2-2-2-2", OV_MEDIA_OFFER, "sdp"));

    // create session
    testrun(ov_mc_frontend_registry_register_session(
        frontend->registry, conn, "2-2-2-2"));
...
int test_ov_mc_frontend_talk() {
    ...
    testrun(!ov_mc_frontend_talk(frontend,
                                 "1-1-1-1",
                                 "2-2-2-2",
                                 true,
