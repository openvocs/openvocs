...
static void dummy_mixer_acquired(void *userdata,
                                 const char *uuid,
...
static void new_call(void *userdata,
                     const char *loop,
                     const char *id,
                     const char *peer) {
...
static void terminated_call(void *userdata,
                            const char *id,
                            const char *loopname) {
...
static void permit_call(void *userdata,
...
static void revoke_call(void *userdata,
                        const ov_sip_permission permission,
                        uint64_t error_code,
                        const char *error_desc) {
...
int check_cb_mixer_acquired() {
    ...
    testrun(0 == strcmp("user",
                        ov_json_string_get(ov_json_get(
                            msg, "/" OV_KEY_PARAMETER "/" OV_KEY_NAME))));
    msg = ov_json_value_free(msg);
...
int check_cb_mixer_forwarded() {
    ...
    msg = ov_mc_sip_msg_set_sc("1-2-3-4",
                               "user",
                               "loop",
...
static ov_json_value *msg_notify(const char *type,
                                 const char *call_id,
                                 const char *peer,
                                 const char *loop) {
...
int test_ov_mc_backend_sip_terminate_call() {
    ...
    // register a call
    testrun(ov_mc_backend_sip_registry_register_call(
        sip->registry, connection, "id", "loop", "peer"));
...
int test_ov_mc_backend_sip_create_permission() {
    ...
    testrun(ov_mc_backend_sip_create_permission(
        sip,
        (ov_sip_permission){.caller = "caller",
                            .loop = "loop",
                            .from_epoch = 1,
                            .until_epoch = 2}));
