...
static void cb_mixer_lost(void *userdata, const char *uuid) {
    ...
    if (uuid) strncpy(data->uuid, uuid, sizeof(ov_id));
...
static void cb_mixer_acquired(void *userdata,
                              const char *uuid,
    ...
    if (uuid) strncpy(data->uuid, uuid, sizeof(ov_id));
    if (user_uuid) strncpy(data->session_id, user_uuid, sizeof(ov_id));
...
static void cb_mixer_join(void *userdata,
                          const char *uuid,
                          const char *user_uuid,
                          const char *loopname,
                          uint64_t error_code,
...
static void cb_mixer_state(void *userdata,
                           const char *uuid,
...
static void cb_mixer_leave(void *userdata,
                           const char *uuid,
                           const char *user_uuid,
                           const char *loopname,
                           uint64_t error_code,
...
static void cb_mixer_volume(void *userdata,
                            const char *uuid,
                            const char *user_uuid,
                            const char *loopname,
                            uint8_t volume,
...
static int register_mixer(ov_event_loop *loop,
                          ov_socket_configuration manager,
                          ov_mc_backend *backend,
                          const char *uuid) {

    if (!loop || !backend || !uuid) goto error;

    // register a mixer

    int mixer = ov_socket_create(manager, true, NULL);
    if (mixer < 0) goto error;
...
int test_ov_mc_backend_release_mixer() {
    ...
    testrun(ov_mc_backend_release_mixer(
        backend, "1-1-1-1", "2-2-2-2", &userdata, cb_mixer_released));
...
int test_ov_mc_backend_join_loop() {
    ...
    testrun(!ov_mc_backend_join_loop(backend,
                                     "1-1-1-1",
                                     "2-2-2-2",
...
int test_ov_mc_backend_get_session_state() {
    ...
    testrun(!ov_mc_backend_get_session_state(
        backend, "1-1-1-1", "2-2-2-2", &userdata, cb_mixer_state));
...
int test_ov_mc_backend_leave_loop() {
    ...
    testrun(!ov_mc_backend_leave_loop(
        backend, "1-1-1-1", "2-2-2-2", "loop1", &userdata, cb_mixer_leave));
...
int test_ov_mc_backend_set_loop_volume() {
    ...
    testrun(!ov_mc_backend_set_loop_volume(backend,
                                           "1-1-1-1",
                                           "2-2-2-2",
                                           "loop1",
                                           50,
