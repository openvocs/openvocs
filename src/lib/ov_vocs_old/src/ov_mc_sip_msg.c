...
ov_response_state ov_mc_sip_msg_get_response(
    ...
    if (!ov_response_state_from_message(message, &state) ||
        !ov_ptr_valid(
            scfg.host, "Could not parse socket information from event")) {
...
ov_json_value *ov_mc_sip_msg_set_sc(const char *uuid,
    ...
    if (!ov_socket_configuration_to_json(socket.socket, &val)) goto error;
...
ov_json_value *ov_mc_sip_msg_create_call(const char *loop,
    ...
    if (!loop || !destination) goto error;
