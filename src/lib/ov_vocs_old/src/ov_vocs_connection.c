...
bool ov_vocs_connection_is_authenticated(ov_vocs_connection *self) {
    ...
    if (ov_vocs_connection_get_user(self)) return true;
    return false;
}
...
bool ov_vocs_connection_is_authorized(ov_vocs_connection *self) {
    ...
    if (ov_vocs_connection_get_role(self)) return true;
    return false;
}
...
static bool set_key_value(ov_vocs_connection *self,
                          const char *key,
                          const char *val) {

    ov_json_value *out = NULL;

    if (!self || !key || !val) goto error;

    if (!self->data) self->data = ov_json_object();

    out = ov_json_string(val);
    if (!ov_json_object_set(self->data, key, out)) goto error;
...
static ov_json_value *get_set_loop(ov_vocs_connection *self, const char *id) {
    ...
    if (!loops) {
        ...
        if (!ov_json_object_set(self->data, OV_KEY_LOOPS, loops)) goto error;
    }
    ...
    if (!loop) {
        ...
        loop = ov_json_object();
        if (!ov_json_object_set(loops, id, loop)) goto error;
    }
...
ov_json_value *ov_vocs_connection_get_state(const ov_vocs_connection *self) {
    ...
    if (self->data) {
        ...
        if (!ov_json_value_copy((void **)&out, self->data)) goto error;
    ...
    */
    ...
    if (!ov_json_object_set(out, OV_KEY_REMOTE, val)) goto error;
