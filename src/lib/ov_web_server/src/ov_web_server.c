...
static bool init_config(ov_web_server_config *config) {
  ...
  if (0 == config->socket.host[0]) {
    ...
    config->socket =
        (ov_socket_configuration){.type = TLS, .host = "0.0.0.0", .port = 443};
...
static bool io_accept_secure(int socket, uint8_t events, void *data) {
  ...
  if (nfd < 0) {
    ...
    ov_log_error("%s failed to accept at socket %i", self->config.name, socket);
    goto error;
  }
  ...
  if (self->config.debug) {
    ...
    ov_log_debug("%s accepted secure connection socket %i at %s:%i from "
                 "%s:%i",
                 self->config.name, nfd, local.host, local.port, remote.host,
                 remote.port);
  }
...
static bool open_socket(ov_web_server *self) {
  ...
  if (!ov_event_loop_set(self->config.loop, self->socket,
                         ...
                         self, io_accept_secure))
    goto error;

  ov_log_info("opened HTTPs socket %s:%i", self->config.socket.host,
...
ov_web_server *ov_web_server_create(ov_web_server_config config) {
  ...
  /* Load domains */
  if (!ov_domain_load(self->config.domain_config_path, &self->domain.size,
                      &self->domain.array)) {
    ov_log_error("%s failed to load domains from %s", self->config.name,
...
ov_domain *ov_web_server_find_domain(ov_web_server *srv, const uint8_t *name,
                                     size_t len) {
...
bool ov_web_server_send(ov_web_server *self, int socket,
                        const ov_buffer *data) {

  if (!self || !data)
    goto error;

  ov_web_server_connection *conn =
...
bool ov_web_server_uri_file_path(ov_web_server *srv, int socket,
  ...
  ov_web_server_connection *conn =
      ov_dict_get(srv->connections, (void *)(intptr_t)socket);
...
bool ov_web_server_configure_websocket_callback(
  ...
  if (!self || !hostname.start || (hostname.length == 0))
    goto error;

  ov_domain *domain =
      ov_web_server_find_domain(self, hostname.start, hostname.length);
  ...
  *val = config;
  ...
  ov_log_debug("configured websocket callback for domain "
               "%.*s URI %s",
               (int)hostname.length, hostname.start, config.uri);
...
bool ov_web_server_configure_uri_event_io(
  ...
  if (!ov_web_server_configure_websocket_callback(self, hostname, msg_config)) {
    goto error;
  }
  ...
  *val = config;
  ...
  ov_log_debug("configured event_handler callback for domain "
               "%.*s URI %s",
               (int)hostname.length, hostname.start, config.name);
  return true;
error:
  ov_data_pointer_free(key);
  ov_data_pointer_free(val);
...
ov_web_server_config_from_json(const ov_json_value *value) {
  ...
  if (!strncpy(out.mime.path, str, PATH_MAX))
    goto error;
  ...
  if (str) {
    ...
    if (!strncpy(out.mime.ext, str, PATH_MAX))
      goto error;
  }
