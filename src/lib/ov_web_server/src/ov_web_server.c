...
static bool init_config(ov_web_server_config *config) {
    ...
    if (0 == config->socket.host[0]) {
        ...
        config->socket = (ov_socket_configuration){
            .type = TLS, .host = "0.0.0.0", .port = 443};
...
static bool io_accept_secure(int socket, uint8_t events, void *data) {
    ...
    if (nfd < 0) {
        ...
        ov_log_error("%s failed to accept at socket %i", self->config.name,
                     socket);
        goto error;
    }
    ...
    if (self->config.debug) {
        ...
        ov_log_debug("%s accepted secure connection socket %i at %s:%i from "
                     "%s:%i",
                     self->config.name, nfd, local.host, local.port,
...
bool ov_web_server_configure_uri_event_io(
    ...
    if (!ov_web_server_configure_websocket_callback(self, hostname,
                                                    msg_config)) {
        goto error;
    }
