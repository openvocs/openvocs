...
static bool tls_send_buffer(ov_web_server_connection *self, const ov_buffer *buffer);


/*----------------------------------------------------------------------------*/

static bool defragmented_callback(ov_web_server_connection *self,
                                  bool text) {
    ...
    if (!self->websocket.config.callback(
            self->websocket.config.userdata,
            self->socket,
...
static bool defragmented_wss_delivery(ov_web_server_connection *self,
    ...
    if (callback_queue) return defragmented_callback(self, text);
...
static bool process_wss_control_frame(ov_web_server_connection *self,
    ...
    switch (frame->opcode) {
        ...
        case OV_WEBSOCKET_OPCODE_PING:
            ...
            if (!tls_send_buffer(self, response->buffer)) goto error;
    ...
    while (frame) {
        ...
        switch (state) {
            ...
            case OV_WEBSOCKET_PARSER_SUCCESS:
                ...
                if (!process_wss_frame(self, frame)) goto error;
...
static bool check_websocket_enabled(ov_web_server_connection *self,
    ...
    return true;
error:
    if (self) self->websocket.config = (ov_websocket_message_config){0};
...
static bool process_https_message(ov_web_server_connection *self,
    ...
    if (ov_websocket_process_handshake_request(
        ...
        if (!check_websocket_enabled(
                self, msg, hostname, hostname_length)) {
        ...
        if (config.callback.https) {
            return config.callback.https(
                config.callback.userdata, self->socket, msg);
    ...
    while (msg) {
        ...
        switch (state) {
            ...
            case OV_HTTP_PARSER_SUCCESS:
                ...
                if (!process_https_message(self, msg)) goto error;
...
static bool tls_perform_handshake(ov_web_server_connection *self) {
    ...
    if (1 == r) {
        ...
        if (!hostname) {
            ...
            self->domain =
                ov_web_server_find_domain(self->server, 
                                          (uint8_t *)hostname, 
                                          strlen(hostname));
        }
...
static bool tls_send_buffer(ov_web_server_connection *self, const ov_buffer *buffer) {
    ...
    if (!self->tls.handshaked) goto error;
    ...
    if (self->io.out.buffer) {
        ...
        if (!ov_list_queue_push(self->io.out.queue, copy)) goto error;
    ...
    return true;
error:
    if (self) ov_web_server_close_connection(self->server, self->socket);
...
static bool io_tls_send(ov_web_server_connection *self) {
    ...
    if (-1 == self->socket || !self->tls.handshaked) goto error;
    ...
    if (self->io.out.buffer) {
        ...
        bytes = SSL_write(self->tls.ssl,
                          self->io.out.buffer->start,
...
static bool io_tls(int socket_fd, uint8_t events, void *data) {
    ...
    if (!self->tls.handshaked) return tls_perform_handshake(self);
    ...
    if (bytes > 0) {
        ...
        if (!process_tls_io(self)) goto error;
...
process_out:
    ...
    if (events & OV_EVENT_IO_OUT) {
        ...
        if (!io_tls_send(self)) goto error;
...
close_without_websocket_shutdown:
    ...
    if (self->tls.ssl) {
        ...
        if (self->flags & FLAG_SEND_SSL_SHUTDOWN) SSL_shutdown(self->tls.ssl);
    ...
    if (self->domain && self->domain->event_handler.uri) {
        ...
        ov_dict_for_each(self->domain->event_handler.uri,
                         &socket,
    ...
    */
    ...
    if (config.callback.close)
        config.callback.close(
            config.callback.userdata, socket);
...
bool ov_web_server_connection_uri_file_path(ov_web_server_connection *self,
    ...
    ssize_t bytes = snprintf(
        full_path, PATH_MAX, "%s/%s", self->domain->config.path, cleaned_path);

    if (bytes < 1) goto error;

    if (!ov_uri_path_remove_dot_segments(full_path, path)) goto error;
