...
static bool process_https_status(ov_web_server_minimal *self,
                                 int socket,
                                 ov_http_message *msg) {
    ...
    ov_log_debug(
            "unexpected STATUS message received "
            "at %i %i|%s - closing",
            socket,
            msg->status.code,
            msg->status.phrase);
...
static bool answer_head_range(ov_web_server_minimal *self,
    ...
    if (!ov_web_server_send(self->core, socket, msg->buffer)) goto error;
...
bool answer_head(ov_web_server_minimal *self,
                                   int socket,
                                   ov_file_format_desc fmt,
                                   const ov_http_message *request) {
    ...
    if (!self || (socket < 0) || !request) goto error;
    ...
    if (!ov_web_server_uri_file_path(self->core, socket, request, PATH_MAX, path))
        goto error;

    const ov_http_header *range = ov_http_header_get(
        request->header, request->config.header.capacity, "Range");

    if (range) return answer_head_range(self, fmt, request, path, range, socket);
...
static bool process_https_head(ov_web_server_minimal *self,
                               int socket,
                               ov_http_message *msg) {
    ...
    if (!ov_web_server_uri_file_path(
            self->core, socket, msg, PATH_MAX, path)) {
    ...
    if (out) {
        ...
        if (!ov_web_server_send(self->core, socket, out->buffer)) {
            ...
            ov_log_error("failed to send HTTPs response at %i",
                         socket);
        }
...
bool answer_get(ov_web_server_minimal *self,
                                  int socket,
                                  ov_file_format_desc fmt,
                                  const ov_http_message *request) {
    ...
    if (range) return answer_get_range(self, fmt, request, path, range, socket);
...
static bool process_https_get(ov_web_server_minimal *self,
                              int socket,
                              ov_http_message *msg) {
...
static bool process_https_request(ov_web_server_minimal *self,
                                  int socket,
                                  ov_http_message *msg) {
...
static bool cb_io_https(void *userdata,
                      int socket,
                      ov_http_message *msg){
...
ov_web_server_minimal *ov_web_server_minimal_free(ov_web_server_minimal *self){
    ...
    if (!ov_web_server_minimal_cast(self)) goto error;
...
static bool cb_wss_io_to_json(void *userdata,
    ...
    ov_domain *host = ov_web_server_find_domain(srv->core, domain.start, domain.length);
    if (!host) goto error;
...
bool ov_web_server_minimal_configure_uri_event_io(
    ...
    return ov_web_server_configure_uri_event_io(
        self->core, hostname, input, &wss_io_handler);
}