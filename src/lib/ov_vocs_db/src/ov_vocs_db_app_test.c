...
int check_cb_event_login() {
    ...
    ov_event_parameter params = (ov_event_parameter){.send.instance = &userdata,
                                                     .send.send = dummy_send};
...
int check_cb_event_verify() {
    ...
    testrun(!ov_vocs_db_verify_entity_item(db, OV_VOCS_DB_LOOP, "loop3", msg,
                                           &err));
    testrun(NULL != err);
