...
static bool persist_auth(ov_vocs_db_persistance *self) {
  ...
  ov_json_value *out = ov_vocs_db_eject(self->config.db, OV_VOCS_DB_TYPE_AUTH);
  if (!out)
    goto done;
...
static bool persist_state(ov_vocs_db_persistance *self) {
  ...
  ov_json_value *out = ov_vocs_db_eject(self->config.db, OV_VOCS_DB_TYPE_STATE);
  if (!out)
    goto done;
...
static void app_cb_register(void *userdata, const char *name, int socket,
  ...
  if (!ov_broadcast_registry_set(self->broadcasts, OV_KEY_UPDATE, socket,
                                 OV_SYSTEM_BROADCAST))
    goto error;
...
static void cb_event_create(void *userdata, const char *name, int socket,
  ...
  if (!ov_vocs_db_create_entity(self->config.db, entity, id, db_scope,
                                scope_id)) {
...
static void cb_event_update(void *userdata, const char *name, int socket,
  ...
  if (!ov_vocs_db_update_entity_item(self->config.db, entity, id, data,
                                     &errors)) {
...
static void cb_event_set_keyset_layout(void *userdata, const char *name,
                                       int socket, ov_json_value *input) {
...
static bool register_app_callbacks(ov_vocs_db_persistance *self) {
  ...
  if (!ov_event_app_register(self->app, OV_KEY_REGISTER, self, app_cb_register))
    goto error;

  if (!ov_event_app_register(self->app, "update_password", self,
                             cb_event_update_password))
    goto error;

  if (!ov_event_app_register(self->app, "delete", self, cb_event_delete))
    goto error;

  if (!ov_event_app_register(self->app, "create", self, cb_event_create))
    goto error;

  if (!ov_event_app_register(self->app, "update_key", self,
                             cb_event_update_key))
    goto error;

  if (!ov_event_app_register(self->app, "delete_key", self,
                             cb_event_delete_key))
    goto error;

  if (!ov_event_app_register(self->app, "update", self, cb_event_update))
    goto error;

  if (!ov_event_app_register(self->app, "save", self, cb_event_save))
    goto error;

  if (!ov_event_app_register(self->app, "set_layout", self,
                             cb_event_set_layout))
    goto error;

  if (!ov_event_app_register(self->app, "set_keyset_layout", self,
                             cb_event_set_keyset_layout))
    goto error;

  if (!ov_event_app_register(self->app, "add_domain_admin", self,
                             cb_event_add_domain_admin))
    goto error;

  if (!ov_event_app_register(self->app, "add_project_admin", self,
                             cb_event_add_project_admin))
    goto error;

  if (!ov_event_app_register(self->app, "ldap_import", self,
...
ov_vocs_db_persistance_create(ov_vocs_db_persistance_config config) {
  ...
  if (0 != config.timeout.state_snapshot_seconds) {
    ...
    self->timer.state_snapshot = config.loop->timer.set(
        config.loop, config.timeout.state_snapshot_seconds * 1000 * 1000, self,
        timer_state_snapshot);
  ...
  if (0 != config.timeout.auth_snapshot_seconds) {
    ...
    self->timer.auth_snapshot = config.loop->timer.set(
        config.loop, config.timeout.auth_snapshot_seconds * 1000 * 1000, self,
        timer_auth_snapshot);
...
ov_vocs_db_persistance_config_from_json(const ov_json_value *value) {
  ...
  config.timeout.ldap_request_usec =
      ov_json_number_get(ov_json_get(conf, "/" OV_KEY_TIMEOUT "/" OV_KEY_LDAP));

  config.timeout.state_snapshot_seconds = ov_json_number_get(
      ov_json_get(conf, "/" OV_KEY_TIMEOUT "/" OV_KEY_STATE_SNAPSHOT_TIMEOUT));
...
bool ov_vocs_db_persistance_ldap_import(ov_vocs_db_persistance *self,
  ...
  if (!ov_thread_loop_send_message(self->thread_loop, msg, OV_RECEIVER_THREAD))
    goto error;
...
bool ov_vocs_db_persistance_broadcast(ov_vocs_db_persistance *self,
  ...
  if (!ov_vocs_db_persistance_save(self))
    goto error;
