...
int test_ov_vocs_db_persistance_free() {
    ...
    testrun(NULL == ov_vocs_db_persistance_free(self));
    testrun((void *)loop == (void *)ov_vocs_db_persistance_free(
                                (ov_vocs_db_persistance *)loop));
    testrun((void *)db ==
...
int test_ov_vocs_db_persistance_save() {
    ...
    testrun(OV_VOCS_SEND ==
            ov_vocs_db_get_state(db, "user1", "role2", "loop1"));
    testrun(OV_VOCS_SEND ==
            ov_vocs_db_get_state(db, "user1", "role1", "loop2"));
