...
static bool authorize_connection_for_id(ov_vocs_db_app *app, int socket,
    ...
    case OV_VOCS_DB_SCOPE_DOMAIN:
        ...
        if (!ov_vocs_db_authorize_domain_admin(app->config.db, user,
                                               parent.id)) {
            goto error;
        }
    ...
    case OV_VOCS_DB_SCOPE_PROJECT:
        ...
        if (!ov_vocs_db_authorize_project_admin(app->config.db, user,
                                                parent.id)) {
            goto error;
        }
...
static void cb_ldap_authentication(void *userdata, const char *uuid,
    ...
    if (!ov_broadcast_registry_set(self->broadcasts,
                                   ...
                                   data.socket, OV_SYSTEM_BROADCAST)) {
        goto error;
    }
...
bool register_event_callbacks(ov_vocs_db_app *self) {
    ...
    if (!ov_event_engine_register(self->engine, OV_EVENT_API_LOGIN, self,
                                  cb_event_login))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_EVENT_API_UPDATE_LOGIN, self,
                                  cb_update_client_login))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_EVENT_API_AUTHENTICATE, self,
                                  cb_event_login))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_EVENT_API_LOGOUT, self,
                                  cb_event_logout))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_UPDATE_PASSWORD,
                                  self, cb_event_update_password))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_ADMIN_DOMAINS, self,
                                  cb_event_get_admin_domains))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_ADMIN_PROJECTS, self,
                                  cb_event_get_admin_projects))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_ID_EXISTS, self,
                                  cb_event_check_id_exists))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_GET, self,
                                  cb_event_get))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_DELETE, self,
                                  cb_event_delete))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_CREATE, self,
                                  cb_event_create))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_GET_KEY, self,
                                  cb_event_get_key))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_UPDATE_KEY, self,
                                  cb_event_update_key))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_DELETE_KEY, self,
                                  cb_event_delete_key))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_VERIFY, self,
                                  cb_event_verify))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_UPDATE, self,
                                  cb_event_update))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_LOAD, self,
                                  cb_event_load))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_SAVE, self,
                                  cb_event_save))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_SET_LAYOUT, self,
                                  cb_event_set_layout))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_GET_LAYOUT, self,
                                  cb_event_get_layout))
        goto error;
    ...
    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_LDAP_IMPORT, self,
    ...
    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_SET_USER_DATA, self,
                                  cb_event_set_user_data))
        goto error;

    if (!ov_event_engine_register(self->engine, OV_VOCS_DB_GET_USER_DATA, self,
                                  cb_event_get_user_data))
        goto error;

    if (!ov_event_engine_register(self->engine, "get_highest_port", self,
...
bool ov_vocs_db_app_send_broadcast(ov_vocs_db_app *self,
    ...
    if (!ov_broadcast_registry_send(self->broadcasts,
                                    OV_BROADCAST_KEY_SYSTEM_BROADCAST,
                                    &parameter, broadcast, OV_SYSTEM_BROADCAST))
        goto error;
