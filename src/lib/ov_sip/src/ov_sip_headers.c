...
static ov_buffer *hash_buffer(ov_buffer const *input) {
  ...
  if (ov_ptr_valid(input, "Invalid input to hash")) {
    ...
    bool hash_successful = ov_hash_string(
        OV_HASH_MD5, input->start, input->length, &hash->start, &hash->length);
