...
static SipSerde *as_sip_serde(ov_serde *ss, ov_result *res) {
  ...
  if (0 == ss) {
    ov_result_set(res, OV_ERROR_INTERNAL_SERVER, "No SIP Serde (0 pointer)");
    return 0;
...
static ElementParseState parse_headers(SipSerde *self, ov_buffer *buf,
  ...
  if (P_OK != state) {
    ...
    if (0 == line_length) {
      ...
      ElementParseState state = parse_header(self, line, line_length, res);
...
static ssize_t serialize_request_start_line(ov_sip_message *msg,
  ...
  if ((0 == method) || (0 == uri)) {
    ...
    return print_formatted(write_ptr, capacity, res, "%s %s %s%s", method, uri,
                           SIP_VERSION, CRLF);
  }
}
...
static bool impl_serialize(ov_serde *self, int fh, ov_serde_data data,
  ...
  if ((0 == as_sip_serde(self, res)) || (!is_data_valid(data, res))) {
    ...
    ssize_t written = serialize_start_line(self, msg, write_ptr, capacity, res);
    ...
    if (-1 < written) {
      ...
      if (-1 < written) {
        ...
        if ((-1 < written) && (capacity > 1)) {
          ...
          written = serialize_body(self, msg, write_ptr, capacity, res);
          capacity -= written;
          written_total += written;
          write_ptr += written;
          ...
          ov_log_error("Cannot serialize SIP message: write buffer too "
                       "small");
        }
      }
    }
