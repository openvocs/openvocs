...
static bool is_code_valid(int16_t code) {
  ...
  if (100 > code) {
    ov_log_error("Invalid SIP response code: %" PRIi16 " - too small", code);
    return false;
  } else if (699 < code) {
    ov_log_error("Invalid SIP response code: %" PRIi16 " - too small", code);
    return false;
...
static ov_sip_message *basic_message_copy(ov_sip_message const *self) {
  ...
  case OV_SIP_RESPONSE:
    ...
    return ov_sip_message_response_create(ov_sip_message_response_code(self),
...
static uint32_t parse_cseq_header(char const *header, char const **method) {
  ...
  if (!ov_ptr_valid(endptr, "Could not parse CSEQ header - no valid number") ||
...
bool ov_sip_message_body_set(ov_sip_message *self, ov_buffer *body,
  ...
  if ((!is_msg_valid(self)) || (!ptr_valid(body, "body")) ||
    ...
    ov_sip_message_header_set(self, OV_SIP_HEADER_CONTENT_TYPE, content_type);
...
static void dump_headers(FILE *out, ov_sip_message const *self) {
  ...
  if (ov_ptr_valid(out,
                   "Tried to dump SIP message headers, but no output stream")) {
