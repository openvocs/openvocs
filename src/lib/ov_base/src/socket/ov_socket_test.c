...
int test_ov_socket_create() {
  ...
  cfg = (ov_socket_configuration){.type = LOCAL, .host = "/tmp/testsocket_path"
...
int test_ov_socket_get_sockaddr_storage() {
  ...
  testrun(ov_socket_parse_sockaddr_storage(&loc, ip, OV_HOST_NAME_MAX, &port));
  testrun(port > 0);
  testrun(check_hostname_localhost(ip));
  close(socket);
  ...
  default:
    ...
    port = 0;
    testrun(
        ov_socket_parse_sockaddr_storage(&rem, ip, OV_HOST_NAME_MAX, &port));
    testrun(port > 0);
    testrun(check_hostname_localhost(ip));
...
int test_ov_socket_parse_sockaddr_storage() {
  ...
  testrun(!ov_socket_parse_sockaddr_storage(NULL, ip, INET6_ADDRSTRLEN, &port));
  testrun(!ov_socket_parse_sockaddr_storage(&s, NULL, INET6_ADDRSTRLEN, &port));
...
int test_ov_socket_get_data() {
  ...
  testrun(ov_socket_get_sockaddr_storage(client, &sa_local, &sa_remote, NULL));
...
int test_ov_socket_configuration_to_json() {
  ...
  testrun(0 ==
          strncmp(ov_json_string_get(ov_json_get(value, "/type")), "DTLS", 4));
...
int test_ov_socket_log() {
  ...
  config =
      (ov_socket_configuration){.type = LOCAL, .host = "/tmp/some_unix_socket"};
...
int test_ov_socket_configuration_list() {
  ...
  ov_socket_configuration *config = calloc(1, sizeof(ov_socket_configuration));
