...
static bool install_handler(ov_event_loop *loop,
  ...
  if (ov_ptr_valid(loop, "Cannot setup RT(C)P handler - invalid event loop") &&
...
static int create_socket_for(ov_socket_configuration scfg, bool mc,
  ...
  if (sd > -1) {
    ...
    ov_log_error("Failed to open %s socket at %s:%" PRIu16, type_str, scfg.host,
                 scfg.port);
  }
  ...
  for (size_t num_tries = 0; 10 > num_tries; ++num_tries) {
    ...
    if (0 > fds[0]) {
      ...
      if (-1 < fds[1]) {
        ...
        ov_log_error("Could not open RTCP socket on %s:%" PRIu16 " :   %s",
...
or_default_rtp_socket(ov_socket_configuration cfg, bool set_default_port) {
  ...
  if ((0 == res_cfg.port) && set_default_port) {
    ov_log_warning("Port to listen on not set - using %i", OV_DEFAULT_RTP_PORT);
...
bool ov_rtp_app_get_rtp_socket(ov_rtp_app const *self,
  ...
  if (ov_ptr_valid(scfg, "Cannot get RTP socket - invalid target socket config "
                         "structure (0 pointer)") &&
...
bool ov_rtp_app_send_socket(int sd_to_send_from, ov_rtp_frame const *frame,
  ...
  if (ov_cond_valid(ov_socket_fill_sockaddr_storage(&dest, address_family,
    ...
    return ov_cond_valid(ov_rtp_app_send_to_sockaddr(sd_to_send_from, frame,
