...
int check_json_pointer_replace_special_encoding() {
    ...
    memset(buffer2, '\0', size);
    testrun(
        json_pointer_replace_special_encoding(result, 4, buffer1, "~1", "/"));
    ...
    memset(buffer2, '\0', size);
    testrun(
        json_pointer_replace_special_encoding(result, 7, buffer1, "~1", "/"));
    testrun(strlen(buffer2) == 6);
    ...
    memset(buffer2, '\0', size);
    testrun(
        json_pointer_replace_special_encoding(result, 7, buffer1, "~0", "~"));
    testrun(strlen(buffer2) == 6);
    testrun(strncmp("12~abc", buffer2, strlen(buffer2)) == 0);
