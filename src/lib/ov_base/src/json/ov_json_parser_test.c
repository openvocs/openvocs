...
int test_ov_json_parser_encode() {
    ...
    expect = "{}";
    memset(buffer, 0, size);
    testrun(strlen(expect) == (size_t)ov_json_parser_encode(
                                  value, &conf_minimal, NULL, buffer, size));
    testrun(0 == strncmp(buffer, expect, strlen(expect)));

    memset(buffer, 0, size);
    testrun(strlen(expect) == (size_t)ov_json_parser_encode(
                                  value, &conf_default, NULL, buffer, size));
    testrun(0 == strncmp(buffer, expect, strlen(expect)));
    ...
    // check input error
    testrun(-1 ==
            ov_json_parser_encode(NULL, &conf_default, NULL, buffer, size));
    testrun(-1 ==
            ov_json_parser_encode(value, &conf_default, NULL, NULL, size));
