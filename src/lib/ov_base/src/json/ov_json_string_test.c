...
int test_ov_json_string_set_length() {
  ...
  testrun(4 == string->buffer.size);
  testrun(0 ==
          strncmp("abcd", string->buffer.start, strlen(string->buffer.start)));
...
int test_ov_json_string_set() {
  ...
  testrun(8 == string->buffer.size);
  testrun(0 ==
          strncmp("1234", string->buffer.start, strlen(string->buffer.start)));

  // set with shorter string as initial alloc
  testrun(ov_json_string_set(value, "abcde"));
  testrun(8 == string->buffer.size);
  testrun(0 ==
          strncmp("abcde", string->buffer.start, strlen(string->buffer.start)));
