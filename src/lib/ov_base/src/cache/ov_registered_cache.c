...
static bool cache_to_json(void const *key, void const *value, void *void_arg) {
  ...
  ov_json_object_set(jcache, OV_KEY_CAPACITY, ov_json_number(cache->capacity));
...
bool ov_registered_cache_sizes_configure(ov_registered_cache_sizes *cfg) {
  ...
  ok =
      ok & enable_caching_for(cfg, ov_linked_list_enable_caching, OV_KEY_LISTS);

  ok = ok &
       enable_caching_for(cfg, ov_rtp_frame_enable_caching, OV_KEY_RTP_FRAMES);
...
static bool add_cache_size(void const *vkey, void *value, void *varg) {
  ...
  if (!ov_json_is_number(value)) {
    ...
    ov_log_error("Malformed config: Expected number for key %s, got %s", key,
                 cval == 0 ? "0" : cval);
    free(cval);
