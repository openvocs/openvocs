...
static int check_concurrent_access() {
  ...
  ov_registered_cache *cache = ov_registered_cache_extend("multithreaded", cfg);
...
static ov_registered_cache_sizes *sizes_from_string(char const *str) {
  ...
  OV_ASSERT(0 != jval);
  ov_registered_cache_sizes *sizes = ov_registered_cache_sizes_from_json(jval);
...
static bool check_caches_enabled(ov_hashtable *sizes, char const **cache_names,
  ...
  for (size_t i = 0; i < num_names; ++i) {
    ...
    ov_registered_cache *cache = ov_hashtable_get(g_registry, cache_names[i]);
