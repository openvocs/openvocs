...
static bool unload_module(void const *id, void const *restrict entry,
  ...
  if (0 != entry) {
    ...
    void (*unload)() = ov_plugin_system_get_symbol(handle, OV_PLUGIN_UNLOAD);

    if (0 != unload) {
      unload();
      ov_log_info("Unloaded plugin %s", ov_string_sanitize(id));
    } else {
      ov_log_warning("Could not unload plugin %s", ov_string_sanitize(id));
    }
...
static int so_dir_loader(const char *fpath, const struct stat *sb, int typeflag,
  ...
  if (FTW_F == typeflag) {
    ...
    if (0 != ov_plugin_system_load(fpath, (ov_plugin_load_flags){0})) {
      ...
      ov_log_info("Could not load plugin from %s", ov_string_sanitize(fpath));
    }
  }
...
void *ov_plugin_system_get_symbol(void *restrict handle,
  ...
  if (ov_ptr_valid(handle, "Cannot get symbol - invalid plugin handle") &&
      ov_ptr_valid(symname, "Cannot get plugin symbol - invalid symbol name")) {
