...
static int64_t json_decode(ov_item **value, const char *buffer, size_t length);

/*----------------------------------------------------------------------------*/

static int64_t json_number_decode(ov_item **value, uint8_t *buffer,
                                  size_t size) {
    ...
    */
    ...
    if (!ov_item_set_number(*value, number))
        goto error;
...
static int64_t json_literal_decode(ov_item **value, uint8_t *buffer,
                                   size_t size) {
...
static int64_t json_string_decode(ov_item **value, uint8_t *buffer,
                                  size_t size) {
...
static int64_t json_array_decode(ov_item **value, uint8_t *buffer,
                                 size_t size) {
...
static bool json_object_set_with_length(ov_item *obj, const char *key,
                                        size_t length, ov_item *value) {
...
static int64_t json_object_decode(ov_item **value, uint8_t *buffer,
                                  size_t size) {
...
static bool add_encoding_size(const char *key, const ov_item *value,
                              void *data) {
...
static int64_t parser_calculate_object(const ov_item *value,
    ...
    size_t length = 0;
    if (ov_item_is_empty((ov_item *)value))
        return 2;
...
static int64_t json_value_encode(ov_item *value, EncodingParameter *parameter);

/*----------------------------------------------------------------------------*/

static bool object_write_entry(const char *key_in, const ov_item *value,
                               void *parameter) {
...
static bool add_key_collocated(const char *key, const ov_item *value,
                               void *data) {
...
static ov_list *collocate_object_keys(const ov_item *value,
    ...
    if (!ov_item_object_for_each((ov_item *)value, add_key_collocated, &data))
        goto error;
...
static int64_t parser_encode_object(ov_item *value,
    ...
    if (parameter->collocate_keys) {
        ...
        // unordered
        if (!ov_item_object_for_each(value, object_write_entry, parameter))
            goto error;
