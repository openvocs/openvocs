...
bool ov_parser_verify_interface(const void *data) {
  ...
  if (!parser || !parser->free || !parser->decode || !parser->buffer.is_enabled)
    goto error;
...
bool ov_parser_trash_buffered_data(ov_parser *self) {
  ...
  do {
    ...
    if (!parser->buffer.empty_out(parser, &raw, &free_raw)) {
      ov_log_error("failed to empty out buffer of %p|%s", parser, parser->name);
      goto error;
    }
...
#define AS_DUMMY_PARSER(x)                                                     \
  (((ov_parser_cast(x) != 0) && (IMPL_DUMMY_PARSER == ((ov_parser *)x)->type)) \
       ? (ov_parser *)(x)                                                      \
       : 0)
