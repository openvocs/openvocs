...
static bool impl_parser_json_buffer_has_data(const ov_parser *self) {
    ...
    JsonParser *parser = AS_JSON_PARSER(self);
    if (!parser || !parser->buffer || !parser->buffer->start) goto error;

    if (!impl_parser_json_buffer_is_enabled(self)) goto error;

    if (0 < parser->buffer->length) return true;
...
static bool impl_parser_json_buffer_empty_out(ov_parser *self,
                                              void **raw,
                                              void *(**free_raw)(void *)) {
...
static ov_parser_state decode_buffered(JsonParser *parser,
    ...
    if (!ov_json_match(
            parser->buffer->start, parser->buffer->length, true, &last)) {
        goto mismatch;
    }
    ...
    if (!ov_json_match_object(&start, &end, (end - start) + 1)) goto progress;
    ...
    if (-1 == ov_json_parser_decode(
                  &value, (char *)parser->buffer->start, parsed_len))
        goto error;
    ...
    if (parser->buffer->length > parsed_len) {
        ...
        if (!ov_buffer_set(
                parser->buffer, temp, parser->buffer->length - parsed_len))
            goto error;
...
static ov_parser_state impl_parser_json_decode(ov_parser *self,
    ...
    if (true == parser->config.buffering) return decode_buffered(parser, data);
...
static ov_parser_state impl_parser_json_encode(ov_parser *self,
    ...
    bytes = ov_json_parser_encode(data->in.data,
                                  &parser->stringify,
...
static bool json_parser_init(JsonParser *jp,
                             ov_parser_config config,
                             ov_json_stringify_config stringify) {

    if (!jp) goto error;

    if (!ov_parser_set_head((ov_parser *)jp, MAGIC_BYTE)) goto error;
...
error:
    if (jp) memset(jp, 0, sizeof(JsonParser));
    return false;
}
/*
 *      ------------------------------------------------------------------------
...
ov_parser *ov_parser_json_create(ov_parser_config config,
    ...
    if (json_parser_init(parser, config, stringify)) return (ov_parser *)parser;
