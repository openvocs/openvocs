...
static bool path_is_project_top_dir(char const *const path) {
  ...
  ssize_t bytes =
      snprintf(include, PATH_MAX, "%s/%s", path, OV_SOURCE_FILE_FOLDER_INCLUDE);
...
static bool create_path(const char *path) {
  ...
  while (size > (size_t)(pointer - path)) {
    ...
    } else if (mkdir(new_path, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP |
                                   S_IROTH | S_IXOTH) != 0) {
...
static bool create_c_test_file(const char *project_root_path,
  ...
  if (!create_file(config->project.name, "_test.c", content, project_root_path,
                   OV_SOURCE_FILE_FOLDER_SOURCE))
    goto error;
...
static bool create_module_files(ov_source_file_config *config, bool project) {
  ...
  if (!create_c_test_file(config->project.path, config, copyright_string, date))
    goto error;

  if (project) {

    if (!create_makefile(config->project.path, config, copyright_string, date))
      goto error;
    ...
    if (config->copyright.copyright.generate_full_text_licence) {
      ...
      copyright_string = config->copyright.copyright.generate_full_text_licence(
...
bool ov_source_file_create_source_files(ov_source_file_config *config) {
  ...
  char *project_path = ov_source_file_search_project_path(config->project.path);
  if (!project_path) {
    fprintf(stderr,
...
static void print_usage(const char *name) {
  ...
  fprintf(stdout, "\n");
  fprintf(stdout, "               -p,     --project       create a new project "
                  "instead of a module\n");
  fprintf(stdout,
  ...
  fprintf(stdout, "               -u,     --author        define an author "
                  "explizit as argument\n");
  fprintf(stdout, "               -x,     --note          define an copyright "
...
bool ov_source_file_config_dump(FILE *stream,
  ...
  if (!fprintf(stream,
               ...
               "     note:%s\n"
               "\n",
               config->project.name, config->project.path, config->project.url,
               config->project.create, config->copyright.author,
               config->copyright.owner, config->copyright.note))
    goto error;
