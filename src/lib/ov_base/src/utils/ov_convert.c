...
bool ov_convert_string_to_uint(const char *string,
    ...
    if (!memcpy(buffer, string, size)) return false;
    ...
    *number = nbr;
    ...
    if ((uint64_t)nbr > max) goto error;
...
bool ov_convert_string_to_int64(const char *string,
    ...
    *number = strtoll(buffer, &ptr, 10);
    ...
    if (*number != (long long)*number) goto error;
...
bool ov_convert_string_to_double(const char *string,
    ...
    *number = strtod(buffer, &ptr);
    ...
    if (*number != (double)*number) goto error;
...
bool ov_convert_double_to_string(double number, char **string, size_t *size) {
    ...
    if (*string) {
        ...
        if (!memcpy(*string, local, used)) return false;
...
bool ov_convert_binary_to_hex(const uint8_t *binary,
    ...
    if (!binary || !out || !out_length || binary_length < 1) goto error;

    if (*out) {

        if (*out_length < (binary_length * 2) + 1) goto error;
...
bool ov_convert_hex_to_binary(const uint8_t *hex,
    ...
    if (!hex || !out || !out_length || hex_length < 1) goto error;
    ...
    for (size_t i = 0; i < hex_length; i++) {
        ...
        if ((hex[i] == '0') && (hex[i + 1] == 'x')) {
            ...
            if (hex_length < i + 4) goto error;

            // check followed by 2 valid hex chars
            if (!hextobin((char)hex[i + 2], &c)) goto error;
            if (!hextobin((char)hex[i + 3], &c)) goto error;

            i++;
            continue;
        }

        if (!hextobin((char)hex[i], &c)) goto error;

        clean[count] = tolower(hex[i]);
        count++;
    }

    if ((count % 2) != 0) goto error;

    if (*out) {

        if (*out_length < (count / 2)) goto error;
...
static uint8_t vol_percent_to_multiplier[11] = {
    ...
    0, 1, 11, 14, 15, 26, 46, 55, 70, 99, 100
