...
int test_ov_event_add_default_connection_accept() {
  ...
  testrun(!ov_event_add_default_connection_accept(NULL, socketTCP,
                                                  OV_EVENT_IO_IN, &buffer, cb));
  testrun(!ov_event_add_default_connection_accept(loop, 0, OV_EVENT_IO_IN,
                                                  &buffer, cb));
  testrun(
      !ov_event_add_default_connection_accept(loop, socketTCP, 0, &buffer, cb));
  testrun(!ov_event_add_default_connection_accept(loop, socketTCP,
                                                  OV_EVENT_IO_IN, NULL, NULL));
  ...
  // LOCAL OK
  testrun(ov_event_add_default_connection_accept(loop, socketLOCAL,
                                                 OV_EVENT_IO_IN, &buffer, cb));

  // UDP NOK
  testrun(!ov_event_add_default_connection_accept(loop, socketUDP,
                                                  OV_EVENT_IO_IN, &buffer, cb));
