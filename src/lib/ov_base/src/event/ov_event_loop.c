...
bool ov_event_loop_set(ov_event_loop *self, int sfd, uint8_t events,
  ...
  if (ov_cond_valid(-1 < sfd, "Cannot set callback on socket fd: Invalid fd") &&
...
static bool accept_callback(int socket_fd, uint8_t events, void *data) {
  ...
  case AF_INET6:
    ...
    if (!ov_socket_parse_sockaddr_storage(&local_sa, local_ip, OV_HOST_NAME_MAX,
                                          &local_port)) {
...
ov_event_loop_config_from_json(const ov_json_value *value) {
  ...
  double timers =
      ov_json_number_get(ov_json_object_get(obj, OV_EVENT_LOOP_KEY_MAX_TIMERS));
