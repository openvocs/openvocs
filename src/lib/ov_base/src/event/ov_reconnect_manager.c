...
static ov_thread_message *connect_message_free(ov_thread_message *self) {
    ...
    if ((0 != self) && (CONNECT_MSG_MAGIC_BYTES == self->type)) {
        ...
        ov_log_error(
            "Cannot free connect message - wrong message type or invalid "
            "message");
        return self;
    }
}
...
static ov_thread_message *new_con_msg_free(ov_thread_message *self) {
    ...
    if ((0 != self) && (NEW_CON_MSG_MAGIC_BYTES == self->type)) {
        ...
        ov_log_error(
            "Cannot free new connection message - wrong message type or "
            "invalid message");
        return self;
    }
}
...
static bool handle_in_thread(ov_thread_loop *self, ov_thread_message *msg) {
    ...
    if (0 == self) {
        ...
        if (0 >= fd) {
            ...
            if ((0 != nc_msg) && ov_thread_loop_send_message(
                                     self, nc_msg, OV_RECEIVER_EVENT_LOOP)) {
    ...
    if (0 == self) {
        ...
        if (manager->capacity <= cmsg->i) {
            ...
            if ((!rd->in_use) || (-1 < rd->fd)) {
                ov_log_error(
                    "reconnect data index points to either unused or already est connection");
                close(cmsg->fd);
                cmsg->fd = -1;
            } else {
                ...
                uint8_t events =
                    rd->events | OV_EVENT_IO_CLOSE | OV_EVENT_IO_ERR;
...
static bool cb_reconnect_timer(uint32_t id, void *data) {
    ...
    if (0 == rm) {
        ov_log_error(
            "reconnect timer called with wrong userdata");
        goto error;
    }
    ...
    for (size_t i = 0; rm->capacity > i; ++i) {
        ...
        if (0 > rm->fd_reconnect_data[i].fd) {
            ...
            if (ov_thread_loop_send_message(rm->tloop, msg,
                                            OV_RECEIVER_THREAD)) {
                msg = 0;
