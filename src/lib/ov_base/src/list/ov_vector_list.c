...
#define AS_VECTOR_LIST(x)                                                      \
  (((ov_list_cast(x) != 0) && (IMPL_VECTOR_LIST_TYPE == ((ov_list *)x)->type)) \
       ? (VectorList *)(x)                                                     \
       : 0)
...
static bool impl_vector_list_set(ov_list *self, size_t pos, void *item,
                                 void **old);
...
static bool impl_vector_list_for_each(ov_list *self, void *data,
