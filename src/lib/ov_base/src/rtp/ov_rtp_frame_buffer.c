...
static inline ov_rtp_frame *insert_into_stage(ov_list *stage,
  ...
  for (size_t i = 1; 0 != iter; ++i) {
    ...
    uint32_t const CURRENT_SEQUENCE_NUMBER = current->expanded.sequence_number;
...
static inline ov_rtp_frame *drop_oldest_frame(ov_list *stages,
  ...
  while (0 != stage_iter) {
    ...
    ov_rtp_frame *frame_to_shift = drop_frame_from_stage(current_stage, SSID);
...
ov_rtp_frame *ov_rtp_frame_buffer_add(ov_rtp_frame_buffer *restrict self,
  ...
  ov_rtp_frame *dropped_frame = drop_oldest_frame(internal->stages, remainder);
