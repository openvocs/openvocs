...
bool frames_equal(const ov_rtp_frame_expansion ref, const ov_rtp_frame *frame) {
  ...
  if (0 != ref.payload.length) {
    ...
    if (0 != memcmp(ref.payload.data, exp->payload.data, ref.payload.length)) {
  ...
  if (0 != ref.padding.length) {
    ...
    if (0 != memcmp(ref.padding.data, exp->padding.data, ref.padding.length)) {
...
#define ASSERT_BYTES_EQUAL(exp, frame, len)                                    \
  ...
  uint8_t csrcs_encoded[] = {0, 0,    0, 0x03, 0, 0,    0,    0x04, 0,    0,
                             0, 0x07, 0, 0,    0, 0x0b, 0x19, 0x28, 0x37, 0xff};
