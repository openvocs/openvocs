...
size_t ov_rtcp_message_len_octets(ov_rtcp_message const *self) {
  ...
  if (ov_ptr_valid(as_rtcp_message((void *)self), "Not a valid RTCP message")) {
...
static struct rtcp_header decode_header(struct data *data) {
  ...
  if ((0 == data) || (4 > data->length)) {
    ...
    fprintf(stderr, "RTCP type: %s, length %zu, Count: %i\n",
            ov_rtcp_type_to_string(header.type), header.length, header.count);
  ...
  while (true) {
    ...
    if (!ov_cond_valid(*in_octets >= 1,
      ...
      if (!ov_cond_valid(len_item_octets + consumed_octets <= *in_octets,
        ...
        copy_text(target_buffer, target_buffer_capacity_octets, data + 2,
                  len_item_octets);
        found = true;
      }
...
static bool parse_chunk(uint8_t const **in, size_t *in_num_octets,
  ...
  if ((!ov_ptr_valid(in,
      ...
      (!ov_ptr_valid(in_num_octets,
                     "Cannot parse chunk of RTCP SDES message: No input "
                     "length (0 pointer)")) ||
      (!ov_ptr_valid(msg, "Cannot parse chunk of RTCP SDES message: No target "
                          "message (0 pointer)")) ||
...
static bool encode_header(ov_rtcp_message const *self, uint8_t **out,
  ...
  if ((!ov_ptr_valid(self, "Cannot encode RTCP header: No message")) ||
    ...
    write_ptr[0] = 0x81; // beware: We only support SDES with 1 chunk for now
    write_ptr[1] = OV_RTCP_SOURCE_DESC;

    // Remember: Size is size without header, in 32 bit words
    uint16_t payload_len = htons((ov_rtcp_message_len_octets(self) - 4) / 4);
...
static size_t calc_required_octets(ov_rtcp_message const *msgs[]) {
  ...
  for (size_t i = 0; 0 != msgs[i]; ++i) {
    ...
    if (4 > msg_octets) {
      ov_log_error("Cannot encode RTCP messages: Message too short, expect at "
                   "least 4 "
                   "octets, got %zu",
                   msg_octets);
      octets = 0;
...
ov_rtcp_message *ov_rtcp_message_sdes(char const *cname, uint32_t ssrc) {
  ...
  if (!ov_cond_valid(cname_len < 256,
    ...
    ov_string_copy(msg->chunks[0].cname, cname, sizeof(msg->chunks[0].cname));
