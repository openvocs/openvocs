...
ov_json_value *ov_mc_interconnect_msg_register(const char *name,
    ...
    if (!name || !pass) goto error;

    out = ov_event_api_message_create(
        OV_MC_INTERCONNECT_REGISTER, NULL, OV_MC_INTERCONNECT_VERSION);
...
ov_json_value *ov_mc_interconnect_msg_connect_media(const char *name,
    ...
    if (!name || !codec || !host || !port) goto error;

    out = ov_event_api_message_create(
        OV_MC_INTERCONNECT_CONNECT_MEDIA, NULL, OV_MC_INTERCONNECT_VERSION);
    par = ov_event_api_set_parameter(out);
    ...
    if (!ov_json_object_set(par, OV_KEY_CODEC, val)) goto error;

    val = ov_json_string(host);
    if (!ov_json_object_set(par, OV_KEY_HOST, val)) goto error;

    val = ov_json_number(port);
    if (!ov_json_object_set(par, OV_KEY_PORT, val)) goto error;
...
ov_json_value *ov_mc_interconnect_msg_connect_loops() {
    ...
    out = ov_event_api_message_create(
        OV_MC_INTERCONNECT_CONNECT_LOOPS, NULL, OV_MC_INTERCONNECT_VERSION);
