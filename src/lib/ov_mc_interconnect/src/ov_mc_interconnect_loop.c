...
static bool io_multicast(int socket, uint8_t events, void *userdata) {
    ...
    if (frame->expanded.ssrc == self->ssrc) goto error;
    /*
    ...
    */
    ...
    return ov_mc_interconnect_multicast_io(
        self->config.base, self, buffer, bytes);
error:
    frame = ov_rtp_frame_free(frame);
    return false;
}
...
ov_mc_interconnect_loop *ov_mc_interconnect_loop_create(
    ...
    ov_log_debug("opened LOOP %s | %s:%i",
                 self->config.name,
                 self->local.host,
                 self->local.port);
...
ov_mc_interconnect_loop *ov_mc_interconnect_loop_free(
    ...
    if (!ov_mc_interconnect_loop_cast(self)) goto error;
...
ov_mc_interconnect_loop *ov_mc_interconnect_loop_cast(const void *data) {
    ...
    if (*(uint16_t *)data != OV_MC_INTERCONNECT_LOOP_MAGIC_BYTES) return NULL;
