...
static void cb_connected(void *userdata, int socket, bool result) {
    ...
    ov_json_value *msg = ov_mc_interconnect_msg_register(self->config.name,
...
static bool event_connect_media_response(ov_mc_interconnect *self,
    ...
    strncpy(config.remote.interface, name,
            OV_MC_INTERCONNECT_INTERFACE_NAME_MAX);
...
static bool register_events(ov_mc_interconnect *self) {
    ...
    if (!ov_event_engine_register(self->event.engine,
                                  OV_MC_INTERCONNECT_REGISTER, self,
                                  event_register))
        goto error;

    if (!ov_event_engine_register(self->event.engine,
                                  OV_MC_INTERCONNECT_CONNECT_MEDIA, self,
                                  event_connect_media))
        goto error;

    if (!ov_event_engine_register(self->event.engine,
                                  OV_MC_INTERCONNECT_CONNECT_LOOPS, self,
                                  event_connect_loops))
        goto error;
...
bool ov_mc_interconnect_register_session(ov_mc_interconnect *self, ov_id id,
    ...
    snprintf(buf, OV_HOST_NAME_MAX + 20, "%s:%i", signaling.host,
             signaling.port);
...
bool ov_mc_interconnect_srtp_ready(ov_mc_interconnect *self,
    ...
    ov_event_socket_send(
        self->event.socket,
        ov_mc_interconnect_session_get_signaling_socket(session), out);
