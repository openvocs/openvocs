...
static void cb_connected(void *userdata, int socket, bool result) {
  ...
  ov_json_value *msg =
      ov_mc_interconnect_msg_register(self->config.name, self->config.password);
...
static ov_mc_interconnect_session *
get_session_by_signaling_socket(ov_mc_interconnect *self, int socket) {
...
static ov_mc_interconnect_session *
get_session_by_media_remote(ov_mc_interconnect *self, ov_socket_data *remote) {
...
static bool event_register_response(ov_mc_interconnect *self, const int socket,
  ...
  out = ov_mc_interconnect_msg_connect_media(
...
static bool event_connect_media_response(ov_mc_interconnect *self,
  ...
  strncpy(config.remote.interface, name, OV_MC_INTERCONNECT_INTERFACE_NAME_MAX);
...
static bool event_connect_media(void *userdata, const int socket,
                                const ov_event_parameter *parameter,
                                ov_json_value *input) {
  ...
  if (0 != strcmp(codec, OV_MC_INTERCONNECT_DEFAULT_CODEC)) {
    ...
    out = ov_event_api_create_error_response(input, OV_ERROR_CODE_CODEC_ERROR,
                                             OV_ERROR_DESC_CODEC_ERROR);
...
static bool event_connect_loops_response(ov_mc_interconnect *self,
  ...
  if (!self || !socket || !parameter || !input)
    goto error;
...
static bool event_connect_loops(void *userdata, const int socket,
                                const ov_event_parameter *parameter,
                                ov_json_value *input) {
  ...
  if (!session) {
    ...
    out = ov_event_api_create_error_response(
        input, OV_ERROR_CODE_SESSION_UNKNOWN, OV_ERROR_DESC_SESSION_UNKNOWN);
  ...
  if (!ov_json_array_for_each((ov_json_value *)loops, &container, add_loops)) {
...
static bool io_external_rtp(ov_mc_interconnect *self, uint8_t *buffer,
...
static bool io_external_ssl(ov_mc_interconnect *self, uint8_t *buffer,
                            size_t bytes, ov_socket_data *remote) {
...
static bool io_stun(ov_mc_interconnect *self, int socket, uint8_t *buffer,
                    size_t bytes, ov_socket_data *remote) {
  ...
  if (!ov_stun_xor_mapped_address_encode(buffer + 20, bytes - 20, buffer, NULL,
                                         &remote->sa))
    goto error;
...
ov_mc_interconnect *
ov_mc_interconnect_create(ov_mc_interconnect_config config) {
  ...
  if (!ov_event_socket_load_ssl_config(self->event.socket,
                                       self->config.tls.domains))
    goto error;
  ...
  if (self->config.socket.client) {
    ...
    if (-1 == self->socket.signaling) {
      ...
      ov_log_debug("opened listener %s:%i", self->config.socket.signaling.host,
                   self->config.socket.signaling.port);
    }
  }
  ...
  if (-1 == self->socket.media) {
    ...
    ov_log_debug("opened media listener %s:%i", self->config.socket.media.host,
                 self->config.socket.media.port);
  }
...
ov_mc_interconnect *ov_mc_interconnect_free(ov_mc_interconnect *self) {
  ...
  if (!ov_mc_interconnect_cast(self))
    goto error;
...
ov_mc_interconnect *ov_mc_interconnect_cast(const void *data) {
  ...
  if (*(uint16_t *)data != OV_MC_INTERCONNECT_MAGIC_BYTES)
    return NULL;
...
bool ov_mc_interconnect_register_session(ov_mc_interconnect *self, ov_id id,
  ...
  snprintf(buf, OV_HOST_NAME_MAX + 20, "%s:%i", signaling.host, signaling.port);
...
ov_json_value *
ov_mc_interconnect_get_loop_definitions(ov_mc_interconnect *self) {
...
bool ov_mc_interconnect_srtp_ready(ov_mc_interconnect *self,
  ...
  ov_event_socket_send(self->event.socket,
...
static bool load_loop(const void *key, void *val, void *data) {
  ...
  ov_mc_interconnect_loop_config config =
      (ov_mc_interconnect_loop_config){.loop = self->config.loop, .base = self};
...
ov_mc_interconnect_config_from_json(const ov_json_value *val) {
  ...
  if (ov_json_is_true(ov_json_get(conf, "/" OV_KEY_SOCKET "/" OV_KEY_CLIENT))) {
  ...
  const char *str =
      ov_json_string_get(ov_json_get(conf, "/" OV_KEY_TLS "/" OV_KEY_DOMAINS));
  ...
  if (str)
    strncpy(config.name, str, OV_MC_INTERCONNECT_INTERFACE_NAME_MAX);

  str = ov_json_string_get(ov_json_get(conf, "/" OV_KEY_PASSWORD));

  if (str)
    strncpy(config.password, str, OV_MC_INTERCONNECT_PASSWORD_MAX);

  config.limits.threads = ov_json_number_get(
      ov_json_get(conf, "/" OV_KEY_LIMITS "/" OV_KEY_THREADS));

  config.limits.client_connect_trigger_usec = ov_json_number_get(
      ov_json_get(conf, "/" OV_KEY_LIMITS "/" OV_KEY_RECONNECT_INTERVAL_SECS));
