...
static bool send_stun_binding_request(ov_mc_interconnect_session *session) {
    ...
    if (!ov_stun_generate_binding_request_plain(ptr,
                                                ...
                                                &nxt,
                                                transaction_id,
                                                (uint8_t *)"ov_mc_interconnect",
                                                strlen("ov_mc_interconnect"),
                                                false)) {
...
ov_mc_interconnect_session *ov_mc_interconnect_session_create(
    ...
    if (!ov_mc_interconnect_register_session(self->config.base,
                                             self->id,
...
ov_mc_interconnect_session *ov_mc_interconnect_session_free(
    ...
    if (!ov_mc_interconnect_session_cast(self)) goto error;
    ...
    if (OV_TIMER_INVALID != self->timer.handshake) {
        ov_event_loop_timer_unset(
            self->config.loop, self->timer.handshake, NULL);
        self->timer.handshake = OV_TIMER_INVALID;
    }

    if (OV_TIMER_INVALID != self->timer.keepalive) {
        ov_event_loop_timer_unset(
            self->config.loop, self->timer.keepalive, NULL);
...
static int check_cert(ov_mc_interconnect_session *self) {
    ...
    if (!self || !self->dtls.ssl) goto error;
...
call_again_later:
    ...
    self->timer.handshake =
        loop->timer.set(loop,
                        self->config.reconnect_interval_usecs,
                        self,
                        perform_ssl_client_handshake_triggered);

    if (OV_TIMER_INVALID == self->timer.handshake) goto error;
...
close:
    ...
    if (shutdown) SSL_shutdown(self->dtls.ssl);
...
bool ov_mc_interconnect_session_handshake_active(
    ...
    if (file || path) {
        ...
        if (0 == ov_string_len(file)) file = NULL;
        if (0 == ov_string_len(path)) path = NULL;
...
static const char *ov_mc_interconnect_session_get_srtp_keys(
    ...
    */
    ...
    if (1 != SSL_export_keying_material(self->dtls.ssl,
                                        buffer,
                                        size,
...
static bool prepare_streams(ov_mc_interconnect_session *session) {
    ...
    } else if (0 == ov_string_compare(
                        "SRTP_AES128_CM_SHA1_32", session->srtp.profile)) {
    ...
    switch (session->dtls.type) {
        ...
        case OV_DTLS_ACTIVE:
            ...
            memcpy(session->srtp.local.key,
                   session->srtp.server.key,
                   session->srtp.key_len);

            memcpy(session->srtp.remote.key,
                   session->srtp.client.key,
                   session->srtp.key_len);

            srtp_append_salt_to_key(session->srtp.local.key,
                                    session->srtp.key_len,
                                    session->srtp.server.salt,
                                    session->srtp.salt_len);

            srtp_append_salt_to_key(session->srtp.remote.key,
                                    session->srtp.key_len,
    ...
    if (!ov_dict_for_each(session->ssrcs, session, add_srtp_stream)) goto error;
...
bool ov_mc_interconnect_session_dtls_io(ov_mc_interconnect_session *self,
    ...
    if (!SSL_is_init_finished(self->dtls.ssl)) {
        ...
        const char *profile =
            ov_mc_interconnect_session_get_srtp_keys(self,
                                                     &self->srtp.key_len,
                                                     &self->srtp.salt_len,
                                                     self->srtp.server.key,
                                                     self->srtp.server.salt,
                                                     self->srtp.client.key,
                                                     self->srtp.client.salt);
...
int ov_mc_interconnect_session_send(ov_mc_interconnect_session *self,
    ...
    int type = AF_INET;
    if (!ov_socket_destination_address_type(
            self->config.remote.media.host, &type))
        goto error;
...
bool ov_mc_interconnect_session_add(ov_mc_interconnect_session *self,
    ...
    if (!self || !loop_name || !remote_ssrc) goto error;
...
bool ov_mc_interconnect_session_forward_rtp_external_to_internal(
    ...
    if (frame->expanded.payload_type != 100) goto ignore;

    char *loop_name =
        ov_dict_get(self->ssrcs, (void *)(intptr_t)frame->expanded.ssrc);

    if (!loop_name) goto ignore;
