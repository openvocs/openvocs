...
void *ov_ice_candidate_free(void *self) {
  ...
  if (candidate->stream) {
    ...
    ov_node_remove_if_included((void **)&candidate->stream->candidates.local,
                               candidate);
    ov_node_remove_if_included((void **)&candidate->stream->candidates.remote,
                               candidate);
  }
...
char *ov_ice_candidate_to_string(const ov_ice_candidate *candidate) {
  ...
  while (true) {
    ...
    ptr = string;
    len = snprintf(ptr, size, "%s %i %s %i %s %i %s %s", candidate->foundation,
                   candidate->component_id, transport, candidate->priority,
                   candidate->addr, candidate->port, ICE_STRING_TYPE,
...
bool ov_ice_candidate_parse(ov_ice_candidate *candidate, const char *string,
  ...
  if (!ov_string_to_uint64(ptr, space - ptr, (uint64_t *)&candidate->priority))
    goto error;
  ...
  if (space &&
    ...
    if (!ov_string_to_uint64((char *)rport, len, (uint64_t *)&candidate->rport))
      goto error;
  }
