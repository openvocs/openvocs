...
static bool load_certificates(SSL_CTX *ctx, const ov_ice_config *config,
                              const char *type) {
  ...
  if (SSL_CTX_use_certificate_chain_file(ctx, config->dtls.cert) != 1) {
    ...
    ov_log_error("ICE core %s config failure load certificate "
                 "from %s | error %d | %s",
                 type, config->dtls.cert, errno, strerror(errno));
    goto error;
  }
  ...
  if (SSL_CTX_use_PrivateKey_file(ctx, config->dtls.key, SSL_FILETYPE_PEM) !=
    ...
    ov_log_error("ICE core %s config failure load key "
                 "from %s | error %d | %s",
                 type, config->dtls.key, errno, strerror(errno));
    goto error;
  }

  if (SSL_CTX_check_private_key(ctx) != 1) {

    ov_log_error("ICE core %s config failure private key for\n"
                 "CERT | %s\n"
                 " KEY | %s",
                 type, config->dtls.cert, config->dtls.key);
    goto error;
  }

  ov_log_info("ICE core loaded %s certificate \n file %s\n key %s\n", type,
              config->dtls.cert, config->dtls.key);
...
static bool renew_dtls_keys(uint32_t timer_id, void *data) {
  ...
  if (!ov_ice_dtls_cookie_store_initialize(self->dtls.cookies,
                                           self->config.dtls.dtls.keys.quantity,
                                           self->config.dtls.dtls.keys.length))
    goto error;

  self->timer.dtls_key_renew = self->config.loop->timer.set(
      self->config.loop, self->config.dtls.dtls.keys.lifetime_usec, self,
      renew_dtls_keys);
...
static bool create_fingerprint_cert(const char *path, ov_hash_function hash,
  ...
  if (size >= OV_ICE_DTLS_FINGERPRINT_MAX)
    goto error;

  memset(out, 0, OV_ICE_DTLS_FINGERPRINT_MAX);

  if (!snprintf(out, OV_ICE_DTLS_FINGERPRINT_MAX, "%s %s", hash_string,
                x509_fingerprint))
    goto error;
...
static bool init_dtls(ov_ice *self) {
  ...
  if (!load_certificates(self->dtls.ctx, &self->config, "DTLS"))
    goto error;
  ...
  if (0 == SSL_CTX_set_tlsext_use_srtp(self->dtls.ctx,
  ...
  if (!create_fingerprint_cert(self->config.dtls.cert, OV_HASH_SHA256,
...
ov_ice *ov_ice_cast(const void *data) {
  ...
  if (*(uint16_t *)data == OV_ICE_MAGIC_BYTES)
    return (ov_ice *)data;
error:
  return NULL;
}
...
ov_ice *ov_ice_free(ov_ice *self) {
  ...
  if (OV_TIMER_INVALID != self->timer.dtls_key_renew)
    ov_event_loop_timer_unset(self->config.loop, self->timer.dtls_key_renew,
                              NULL);
...
bool ov_ice_add_server(ov_ice *self, ov_ice_server server) {
  ...
  if (0 == server.socket.host[0])
    goto error;
  if (0 == server.socket.port)
    goto error;
  ...
  case OV_ICE_TURN_SERVER:
    ...
    if (0 == server.auth.user[0])
      goto error;

    if (0 == server.auth.pass[0])
      goto error;
...
bool ov_ice_add_interface(ov_ice *self, const char *host) {
  ...
  if (!ov_dict_set(self->interfaces, key, NULL, NULL))
    goto error;
...
bool ov_ice_add_candidate(ov_ice *self, const char *session_uuid,
  ...
  if (!self || !session_uuid || !candidate)
    goto error;
    ...
    case AF_INET:
      ...
      if (interface_permit_ipv4((struct sockaddr_in *)ifa->ifa_addr)) {
        ...
        if (0 == getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in),
                             socket_config.host, OV_HOST_NAME_MAX, NULL, 0,
                             NI_NUMERICHOST)) {
...
static bool gather_interface(const void *key, void *value, void *data) {
  ...
  if (-1 == socket) {
    ...
    ov_log_error("failed to gather socket at interface %s in range %i:%i", key,
                 container->core->ports.min, container->core->ports.max);
  ...
  if (!base) {
    ...
    ov_log_error("failed to gather base at interface %s in range %i:%i", key,
                 container->core->ports.min, container->core->ports.max);
...
static bool base_gather_remote(ov_ice *ice, ov_ice_base *base,
  ...
  if (base->local.data.sa.ss_family != socket_data.sa.ss_family)
    goto done;
...
static int interfaces_count(const ov_ice *core) {
  ...
  for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++) {
    ...
    case AF_INET:
      ...
      if (0 != getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in), hostname,
                           OV_HOST_NAME_MAX, NULL, 0, NI_NUMERICHOST)) {
        continue;
      }
    ...
    case AF_INET6:
      ...
      if (0 != getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in6), hostname,
                           OV_HOST_NAME_MAX, NULL, 0, NI_NUMERICHOST)) {
        continue;
      }
...
bool ov_ice_trickle_candidate(ov_ice *self, const char *session_uuid,
  ...
  if (!self->config.callbacks.candidates.new)
    goto done;

  return self->config.callbacks.candidates.new(self->config.callbacks.userdata,
                                               session_uuid, ufrag, stream_id,
                                               *candidate);
...
bool ov_ice_trickle_end_of_candidates(ov_ice *self, const char *session_uuid,
  ...
  if (!self->config.callbacks.candidates.end_of_candidates)
    goto done;
...
bool ov_ice_session_drop_callback(ov_ice *self, const char *session_uuid) {
  ...
  if (self->config.callbacks.session.drop)
    self->config.callbacks.session.drop(self->config.callbacks.userdata,
                                        session_uuid);
