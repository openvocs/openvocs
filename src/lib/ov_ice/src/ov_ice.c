...
static bool renew_dtls_keys(uint32_t timer_id, void *data) {
    ...
    if (!ov_ice_dtls_cookie_store_initialize(
            self->dtls.cookies, self->config.dtls.dtls.keys.quantity,
...
static int interfaces_count(const ov_ice *core) {
    ...
    for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++) {
        ...
        case AF_INET:
            ...
            if (0 != getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in),
                                 hostname, OV_HOST_NAME_MAX, NULL, 0,
                                 NI_NUMERICHOST)) {
                continue;
            }
...
bool ov_ice_trickle_candidate(ov_ice *self, const char *session_uuid,
    ...
    return self->config.callbacks.candidates.new(
        self->config.callbacks.userdata, session_uuid, ufrag, stream_id,
        *candidate);
