...
ov_ice_session *ov_ice_session_create_answer(ov_ice *ice, ov_sdp_session *sdp) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    user = ov_sdp_attribute_get(description->attributes, OV_ICE_STRING_USER);
    pass = ov_sdp_attribute_get(description->attributes, OV_ICE_STRING_PASS);
    ...
    if (0 == ov_string_compare(setup, OV_ICE_STRING_ACTIVE_PASSIVE)) {
      ...
      ov_sdp_attribute_del(&description->attributes, OV_KEY_SETUP);
      ov_sdp_attribute_add(&description->attributes, OV_ICE_STRING_PASSIVE,
                           NULL);
    ...
    while (ov_sdp_attributes_iterate(&attribute, OV_KEY_CANDIDATE, &out)) {
      ...
      ov_ice_candidate *c = ov_ice_candidate_from_string(out, strlen(out));
      if (!c)
        continue;
...
ssize_t ov_ice_session_stream_send(ov_ice_session *session, int stream_id,
  ...
  return ov_ice_stream_send_stream(ov_node_get(session->streams, stream_id + 1),
                                   buffer, size);
}
...
static bool have_same_foundation(const ov_ice_candidate *a,
  ...
  if ((0 != a->server.socket.host[0]) && (0 != b->server.socket.host[0])) {
    ...
    if (0 ==
        strncmp(a->server.socket.host, b->server.socket.host, OV_HOST_NAME_MAX))
      return true;
  }
...
bool ov_ice_session_update(ov_ice_session *session) {
  ...
  if (failed) {
    ...
    if (config.callbacks.session.state)
      config.callbacks.session.state(config.callbacks.userdata, session->uuid,
                                     session->state);
