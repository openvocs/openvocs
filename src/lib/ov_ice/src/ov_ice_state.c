...
ov_ice_state ov_ice_state_from_string(const char *string) {
    ...
    if (0 == strcmp(string, OV_KEY_COMPLETED)) return OV_ICE_COMPLETED;

    if (0 == strcmp(string, OV_KEY_FAILED)) return OV_ICE_FAILED;

    if (0 == strcmp(string, OV_KEY_RUNNING)) return OV_ICE_RUNNING;

    if (0 == strcmp(string, OV_KEY_INIT)) return OV_ICE_INIT;
