...
static bool io_external_ssl(ov_ice_base *base, const uint8_t *buffer,
  ...
  if (!pair->dtls.handshaked) {
    ...
    if (ov_ice_debug_dtls(base->stream->session->ice))
      ov_log_debug(
          "DTLS handshake at pair %p %s|%i %s %" PRIu64 " %s:%i <-> %s:%i %s",
          pair, base->stream->session->uuid, base->stream->index,
          ov_ice_candidate_type_to_string(pair->local->type), pair->priority,
          pair->local->addr, pair->local->port, pair->remote->addr,
          pair->remote->port, ov_ice_pair_state_to_string(pair->state));
...
static bool io_external_rtp(ov_ice_base *base, uint8_t *buffer, size_t size,
  ...
  if (config.callbacks.stream.io)
    config.callbacks.stream.io(config.callbacks.userdata, stream->session->uuid,
                               stream->index, buffer, size);
ignore:
  return true;
error:
  return false;
...
static bool io_process_stun_request(ov_ice_base *base,
  ...
  if (!ov_stun_username_decode(attr_username, length - (buffer - attr_username),
                               &ptr, &name_length))
    goto error;
  ...
  if (!ov_stun_check_message_integrity((uint8_t *)buffer, length, attr,
                                       attr_size, (uint8_t *)pass, strlen(pass),
                                       true))
    goto unauthorized;
  ...
  if (ov_list_is_empty(pair_list)) {
    ...
    if (ov_ice_debug_stun(stream->session->ice))
      ov_log_debug("New PEER-REFLEXIVE candidate from %s:%i", remote->host,
                   remote->port);
    ...
    if (!ov_node_push((void **)&stream->candidates.remote, remote_candidate)) {
      remote_candidate = ov_ice_candidate_free(remote_candidate);
  ...
  while (pair) {
    ...
    if (stream->session->controlling && attr_use_candidate) {
      ...
      ov_log_info("ICE session %s|%i selected pair at %s:%i from %s:%i - state "
                  "%s",
...
static bool io_process_stun_candidate_success_response(
  ...
  if (!ov_stun_xor_mapped_address_decode(xmap, length - (xmap - buffer), buffer,
                                         &xor_ptr))
    goto ignore;
...
static bool io_process_stun_pair_success_response(
  ...
  if (prio) {
    ...
    if (!ov_stun_ice_priority_decode(prio, length - (prio - buffer), &priority))
      goto ignore;
  }
  ...
  if (!base->stream->session->controlling) {
    ...
    if (pair->nominated) {
      ...
      if (ov_ice_debug_stun(base->stream->session->ice))
        ov_log_debug("ICE session %s|%i selected pair at %s:%i from %s:%i - "
                     "state "
                     "%s",
...
static bool io_process_stun_error_response(ov_ice_base *base,
  ...
  if (ov_ice_candidate_cast(data))
    return io_process_stun_candidate_error_response(base, data, remote, buffer,
                                                    length, attr, attr_size);
  ...
  if (ov_stun_frame_class_is_request(buffer, length)) {
    ...
    if (ov_ice_debug_stun(ice))
      ov_log_debug("STUN binding request at %s:%i from %s:%i",
                   base->local.data.host, base->local.data.port, remote->host,
                   remote->port);
    ...
    return io_process_stun_success_response(base, remote, buffer, length, attr,
                                            attr_size);
    ...
    return io_process_stun_error_response(base, remote, buffer, length, attr,
                                          attr_size);
...
static bool io_external_stun(ov_ice_base *base, uint8_t *buffer, size_t size,
  ...
  case STUN_BINDING:
    return io_base_stun_process(base, remote, buffer, size, attr, attr_size);
  ...
  case TURN_CREATE_PERMISSION:
    return io_base_turn_process_create_permission(base, remote, buffer, size,
                                                  attr, attr_size);

  case TURN_CHANNEL_BIND:
    return io_base_turn_process_channel_bind(base, remote, buffer, size, attr,
                                             attr_size);
...
bool ov_ice_base_send_stun_binding_request(ov_ice_base *base,
  ...
  if (!ov_stun_generate_binding_request_plain(ptr, OV_UDP_PAYLOAD_OCTETS, &nxt,
