...
ov_http_message *ov_websocket_upgrade_request(const char *host, const char *uri,
  ...
  msg = ov_http_create_request_string((ov_http_message_config){0},
                                      ...
                                      OV_HTTP_METHOD_GET, uri);

  if (!msg)
    goto error;

  if (!ov_http_message_add_header_string(msg, OV_HTTP_KEY_UPGRADE,
                                         OV_WEBSOCKET_KEY))
    goto error;

  if (!ov_http_message_add_header_string(msg, OV_HTTP_KEY_CONNECTION,
                                         OV_WEBSOCKET_KEY_UPGRADE))
    goto error;
  ...
  if (!ov_http_message_add_header_string(msg, OV_WEBSOCKET_KEY_SECURE_VERSION,
...
bool ov_websocket_is_upgrade_response(const ov_http_message *msg,
  ...
  if (header->value.length != strlen(OV_WEBSOCKET_KEY))
    goto error;

  if (0 != memcmp(OV_WEBSOCKET_KEY, header->value.start, header->value.length))
    goto error;
  ...
  if (header->value.length != strlen(OV_HTTP_KEY_UPGRADE))
    goto error;
  ...
  if (!ov_websocket_generate_secure_accept_key(sec_key.start, sec_key.length,
