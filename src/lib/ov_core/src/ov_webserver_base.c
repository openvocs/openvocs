...
static bool create_redirect_reply(ov_webserver_base *srv, Connection *conn,
    ...
    if (ov_http_header_get_next(msg->header, msg->config.header.capacity,
                                &index, OV_HTTP_KEY_HOST))
        goto error;
    ...
    if (colon) {
        ...
        bytes = snprintf(location, PATH_MAX, "https://%.*s:%i",
                         (int)host->value.length, host->value.start,
                         srv->config.http.secure.port);
    }
    ...
    if (srv->config.debug)
        ov_log_debug("%s redirect at %i to %s (%i bytes sent)",
                     srv->config.name, conn->fd, location, bytes);
...
static bool io_tls_send(ov_webserver_base *srv, Connection *conn,
    ...
    if (conn->io.out.buffer) {
        ...
        if (bytes == (ssize_t)conn->io.out.buffer->length) {
            ...
            if (srv->config.debug)
                ov_log_debug("%s send %zu at %i %i bytes", srv->config.name,
                             bytes, conn->fd, (int)conn->io.out.buffer->length);
...
static bool io_tcp_accept_redirect(int socket_fd, uint8_t events, void *data) {
    ...
    if (nfd >= (int)srv->config.limit.max_sockets) {
        ...
        ov_log_error("%s fd %i max_sockets %" PRIu32, srv->config.name,
                     socket_fd, srv->config.limit.max_sockets);
    ...
    if (NULL != srv->config.callback.accept) {
        ...
        if (!srv->config.callback.accept(srv->config.callback.userdata,
                                         socket_fd, nfd))
            goto error;
    }
...
static bool cb_wss_send_json(void *self, int socket, const ov_json_value *msg) {
    ...
    if (conn->fd == -1) {
        ...
        ov_log_error("%s connection for %i gone - abort", srv->config.name,
                     socket);
        goto error;
    }
    ...
    if (!value) {
        ...
        if (srv->config.debug) {
            ov_log_error("%s wss io at %i for %.*s uri %s not JSON - closing",
                         srv->config.name, socket, (int)domain.length,
                         domain.start, uri);
        }
    ...
    if (NULL == config->userdata) {
        ...
        if (srv->config.debug)
            ov_log_debug("%s websocket request | "
                         ...
                         srv->config.name, (int)hostname_length,
                         (char *)hostname, key);
...
static bool process_https_message(ov_webserver_base *srv, Connection *conn,
    ...
    if (srv->config.debug) {
        ...
        ov_log_debug("%s HTTPs at %i %.*s from %s:%i status %i",
                     srv->config.name, conn->fd, hostname_length, hostname,
                     conn->remote.host, conn->remote.port, msg->status.code);
    }
    ...
    */
    ...
    if (ov_websocket_process_handshake_request(msg, &response,
        ...
        if (!check_websocket_enabled(srv, conn, msg, hostname,
                                     hostname_length)) {
            response = ov_http_message_free(response);
            goto error;
        }
        ...
        */
        ...
        if (srv->config.callback.https) {
            return srv->config.callback.https(srv->config.callback.userdata,
                                              conn->fd, msg);
        ...
        if (conn->websocket.counter >= conn->websocket.config.max_frames) {
            ...
            if (!callback_queue) {
                ...
                ov_log_error("%s websocket fragmentation max frames reached "
                             "%zu at %i - closing",
                             srv->config.name, conn->websocket.counter,
                             conn->fd);
...
static int create_socket(ov_webserver_base *srv, ov_socket_configuration config,
    ...
    ov_log_info("%s created HTTP(s) listener %i %s:%i", srv->config.name,
...
static bool check_io_timeout(uint32_t timer, void *data) {
    ...
    for (size_t i = 0; i < srv->connections_max; i++) {
        ...
        if (0 == srv->conn[i].io.in.last) {
            ...
            ov_log_error("%s conn %i remote %s:%i accepted "
                         ...
                         "closing",
                         srv->config.name, srv->conn[i].fd,
                         srv->conn[i].remote.host, srv->conn[i].remote.port,
                         srv->config.timer.accept_to_io_timeout_usec);
    ...
    srv->timer.io_timeout_check =
        loop->timer.set(loop, srv->config.timer.accept_to_io_timeout_usec, srv,
                        check_io_timeout);
...
ov_webserver_base *ov_webserver_base_create(ov_webserver_base_config config) {
    ...
    if (default_sockets) {
        ...
        if (0 != config.http.redirect.host[0])
            srv->sockets.http = create_socket(srv, config.http.redirect,
                                              io_tcp_accept_redirect);
...
bool ov_webserver_base_configure_websocket_callback(
    ...
    if ((NULL == config.callback) && (NULL == config.fragmented)) {
        ...
        if (0 != config.uri[0]) {
            ...
            ov_log_debug("%s websocket callback for domain "
                         "%.*s - unset",
                         srv->config.name, (int)hostname.length,
                         hostname.start);
        }
...
static bool answer_get_range(ov_webserver_base *srv, ov_file_format_desc fmt,
    ...
    if (!ov_http_message_add_header_string(msg, OV_KEY_SERVER,
                                           srv->config.name))
        goto error;
...
bool ov_webserver_base_answer_get(ov_webserver_base *srv, int socket,
    ...
    if (1 > fmt.desc.bytes) {
        ...
        ov_log_error("%s answer get without bytes - unsupported",
                     srv->config.name);
