  ...
  */
  ...
  if (0 == SSL_client_hello_get0_ext(ssl, TLSEXT_NAMETYPE_host_name, &str,
    ...
    for (size_t i = 0; i < srv->domain.size; i++) {
      ...
      if (0 == memcmp(hostname, srv->domain.array[i].config.name.start,
                      hostname_length)) {
...
static bool tls_init_context(ov_webserver_base *srv, ov_domain *domain) {
  ...
  if (!domain->context.tls) {
    ov_log_error("%s TLS context for domain |%s| not set", srv->config.name,
                 domain->config.name);
    goto error;
  }
...
static void close_connection(ov_webserver_base *srv, ov_event_loop *loop,
                             struct Connection *conn) {
...
close_without_websocket_shutdown:
  ...
  if (conn->domain && conn->domain->event_handler.uri) {
    ...
    ov_dict_for_each(conn->domain->event_handler.uri, &socket,
...
static bool close_tcp_listener(ov_webserver_base *srv, int socket_fd) {
  ...
  if (srv->sockets.http == socket_fd)
    srv->sockets.http = -1;

  if (srv->sockets.https == socket_fd)
    srv->sockets.https = -1;

  for (size_t i = 0; i < srv->connections_max; i++) {

    if (srv->conn[i].listener != socket_fd)
      continue;
...
static bool push_outgoing(Connection *conn, void *data) {
  ...
  if (!ov_list_queue_push(conn->io.out.queue, data))
    goto error;
...
static bool create_redirect_reply(ov_webserver_base *srv, Connection *conn,
                                  ov_http_message *msg) {
  ...
  if (ov_http_header_get_next(msg->header, msg->config.header.capacity, &index,
                              OV_HTTP_KEY_HOST))
    goto error;
  ...
  response = ov_http_create_status_string(msg->config, msg->version, 301,
                                          OV_HTTP_MOVED_PERMANENTLY);
  ...
  if (colon) {
    bytes = snprintf(location, PATH_MAX, "https://%.*s:%i",
                     (int)(colon - host->value.start), host->value.start,
                     srv->config.http.secure.port);

  } else {
    bytes =
        snprintf(location, PATH_MAX, "https://%.*s:%i", (int)host->value.length,
                 host->value.start, srv->config.http.secure.port);
  }
  ...
  if (!ov_uri_string_is_valid(location, strlen(location)))
    goto error;

  if (!ov_http_message_add_header_string(response, OV_HTTP_KEY_LOCATION,
                                         location))
    goto error;
  ...
  bytes = send(conn->fd, response->buffer->start, response->buffer->length, 0);
  ...
  if ((-1 == bytes) || ((bytes < 0) && (EAGAIN == errno))) {
    ...
    if (!push_outgoing(conn, response))
      goto error;
    ...
    if (!conn->io.out.buffer) {
      ...
      if (!ov_buffer_shift(conn->io.out.buffer,
                           conn->io.out.buffer->start + bytes))
        goto error;

    } else {

      /* This is a copy push! */
      if (!ov_buffer_push(conn->io.out.buffer, response->buffer->start + bytes,
  ...
  if (srv->config.debug)
    ov_log_debug("%s redirect at %i to %s (%i bytes sent)", srv->config.name,
                 conn->fd, location, bytes);
...
static bool process_tcp_redirect(ov_webserver_base *srv, Connection *conn) {
  ...
  while (msg) {
    ...
    case OV_HTTP_PARSER_SUCCESS:
      ...
      if (!create_redirect_reply(srv, conn, msg))
        goto error;
...
static bool tls_send_buffer(ov_webserver_base *srv, Connection *conn,
  ...
  if (conn->io.out.buffer) {
    ...
    if (!ov_list_queue_push(conn->io.out.queue, copy))
      goto error;
  ...
  case SSL_ERROR_WANT_X509_LOOKUP:
    ov_log_error("%s SSL want connect/accept/lookup after handshaked? "
  ...
  case SSL_ERROR_SYSCALL:
    ...
    ov_log_error("%s SSL_ERROR_SYSCALL"
                 "%d | %s",
                 srv->config.name, errno, strerror(errno));
  ...
  case SSL_ERROR_SSL:
    ...
    ERR_error_string_n(errorcode, errorstring, OV_SSL_ERROR_STRING_BUFFER_SIZE);

    ov_log_error("%s SSL_ERROR_SSL %s at socket %i", srv->config.name,
                 errorstring, conn->fd);
...
static bool io_tls_send(ov_webserver_base *srv, Connection *conn,
                        ov_event_loop *loop) {
  ...
  if (-1 == conn->fd || !conn->tls.handshaked)
    goto error;
  ...
  if (conn->io.out.buffer) {
    ...
    bytes = SSL_write(conn->tls.ssl, conn->io.out.buffer->start,
                      conn->io.out.buffer->length);

    if (bytes == (ssize_t)conn->io.out.buffer->length) {

      if (srv->config.debug)
        ov_log_debug("%s send %zu at %i %i bytes", srv->config.name, bytes,
                     conn->fd, (int)conn->io.out.buffer->length);
    ...
    case SSL_ERROR_WANT_ACCEPT:
    case SSL_ERROR_WANT_X509_LOOKUP:
      ov_log_error("%s SSL want connect/accept/lookup after "
                   "handshaked? "
                   "Should never happen here",
                   srv->config.name);
      OV_ASSERT(1 == 0);
  ...
  if (NULL == out) {
    ...
    if (!loop->callback.set(loop, conn->fd,
      ...
      ov_log_error("%s failed to reset events at %i", srv->config.name,
                   conn->fd);
...
static bool cleaned_path_for_conn(Connection *conn, const ov_http_message *msg,
                                  size_t len, char *out) {
  ...
  ssize_t bytes = snprintf(full_path, PATH_MAX, "%s/%s",
                           conn->domain->config.path, cleaned_path);
...
static bool io_tcp_send(ov_webserver_base *srv, Connection *conn,
                        ov_event_loop *loop) {
  ...
  if (conn->io.out.buffer) {
    ...
    bytes = send(conn->fd, conn->io.out.buffer->start,
                 conn->io.out.buffer->length, 0);
  ...
  if (ov_http_message_cast(out)) {
    ...
    ov_log_error("%s unspecific data in send queue at %i", srv->config.name,
                 conn->fd);
...
static bool io_tcp_redirect(int socket_fd, uint8_t events, void *data) {
  ...
  if ((events & OV_EVENT_IO_CLOSE) || (events & OV_EVENT_IO_ERR)) {
    ...
    if (srv->config.debug)
      ov_log_debug("%s TCP close at %i remote %s:%i", srv->config.name,
                   conn->fd, conn->remote.host, conn->remote.port);
  ...
  if (srv->config.debug)
    ov_log_debug("%s IO TCP redirect at %i | %i bytes", srv->config.name,
                 socket_fd, in);
  ...
  conn->io.in.bytes += in;
  if (!process_tcp_redirect(srv, conn))
    goto error;
...
static bool io_tcp_accept_redirect(int socket_fd, uint8_t events, void *data) {
  ...
  if (nfd < 0) {
    ...
    ov_log_error("%s failed to accept at socket %i", srv->config.name,
                 socket_fd);
    goto error;
  }
  ...
  if (nfd >= (int)srv->config.limit.max_sockets) {
    ...
    ov_log_error("%s fd %i max_sockets %" PRIu32, srv->config.name, socket_fd,
  ...
  if (NULL != srv->config.callback.accept) {
    ...
    if (!srv->config.callback.accept(srv->config.callback.userdata, socket_fd,
                                     nfd))
      goto error;
  }
  ...
  if (srv->config.debug) {
    ...
    ov_log_debug("%s accepted connection socket %i at %s:%i from %s:%i",
                 srv->config.name, nfd, local.host, local.port, remote.host,
                 remote.port);
  }
...
static ov_domain *find_domain(const ov_webserver_base *srv, const uint8_t *name,
                              size_t len) {
  ...
  for (size_t i = 0; i < srv->domain.size; i++) {
    ...
    if (srv->domain.array[i].config.name.length != len)
      continue;
...
static bool tls_perform_handshake(ov_webserver_base *srv, Connection *conn) {
  ...
  if (1 == r) {
    ...
    if (!hostname) {
      ...
      conn->domain = find_domain(srv, (uint8_t *)hostname, strlen(hostname));
    }
...
static bool cb_wss_send_json(void *self, int socket, const ov_json_value *msg) {
  ...
  if ((uint32_t)socket > srv->connections_max)
    goto error;
  ...
  if (conn->fd == -1) {
    ...
    ov_log_error("%s connection for %i gone - abort", srv->config.name, socket);
    goto error;
  }

  if (conn->type != CONNECTION_TYPE_WEBSOCKET) {

    ov_log_error("%s connection for %i not websocket - abort", srv->config.name,
                 socket);
  ...
  if (length < chunk) {
    ...
    if (!ov_websocket_set_data(frame, (uint8_t *)str, length, false)) {
      ...
      ov_log_error("%s failed to set websocket data for %i", srv->config.name,
                   socket);
    ...
    if (!tls_send_buffer(srv, conn, frame->buffer)) {
      ...
      ov_log_error("%s failed to send websocket data at %i", srv->config.name,
                   socket);
  ...
  if (srv->config.debug)
    ov_log_debug("%s send WSS JSON at %i over %zu frames", srv->config.name,
                 socket, counter);
...
static bool cb_wss_io_to_json(void *userdata, int socket,
  ...
  if (!event_config || !event_config->callback.process)
    goto error;
  ...
  if (!value) {
    ...
    if (srv->config.debug) {
      ov_log_error("%s wss io at %i for %.*s uri %s not JSON - closing",
                   srv->config.name, socket, (int)domain.length, domain.start,
                   uri);
    }
  ...
  */
  ...
  return event_config->callback.process(event_config->userdata, socket,
                                        &parameter, value);
...
static bool check_websocket_enabled(ov_webserver_base *srv, Connection *conn,
  ...
  if (NULL == config->userdata) {
    ...
    if (srv->config.debug)
      ov_log_debug("%s websocket request | "
                   "- websocket not enabled for host %.*s uri %s\n",
                   srv->config.name, (int)hostname_length, (char *)hostname,
                   key);
...
static bool process_https_message(ov_webserver_base *srv, Connection *conn,
                                  ov_http_message *msg) {
  ...
  if (srv->config.debug) {
    ...
    ov_log_debug("%s HTTPs at %i %.*s from %s:%i status %i", srv->config.name,
                 conn->fd, hostname_length, hostname, conn->remote.host,
                 conn->remote.port, msg->status.code);
  }
  ...
  */
  ...
  if (ov_websocket_process_handshake_request(msg, &response,
    ...
    if (!check_websocket_enabled(srv, conn, msg, hostname, hostname_length)) {
      response = ov_http_message_free(response);
      goto error;
    }
    ...
    */
    ...
    if (!tls_send_buffer(srv, conn, response->buffer))
      goto error;
    ...
    if (srv->config.callback.https) {
      return srv->config.callback.https(srv->config.callback.userdata, conn->fd,
                                        msg);
...
static bool process_wss_control_frame(ov_webserver_base *srv, Connection *conn,
  ...
  case OV_WEBSOCKET_OPCODE_CLOSE:
    ...
    if (srv->config.debug)
      ov_log_debug("%s io wss at %i from %s:%i | close received\n",
                   srv->config.name, conn->fd, conn->remote.host,
                   conn->remote.port);
...
static bool defragmented_callback(ov_webserver_base *srv, Connection *conn,
                                  bool text) {
  ...
  if (!conn->websocket.config.callback(
          conn->websocket.config.userdata, conn->fd,
          (ov_memory_pointer){conn->domain->config.name.start,
                              conn->domain->config.name.length},
          conn->websocket.config.uri,
          (ov_memory_pointer){.start = buffer->start, .length = buffer->length},
          text))
    goto error;
...
static bool defragmented_wss_delivery(ov_webserver_base *srv, Connection *conn,
  ...
  case OV_WEBSOCKET_FRAGMENTATION_NONE:
    ...
    if (!conn->websocket.config.callback(
            ...
            (ov_memory_pointer){conn->domain->config.name.start,
                                conn->domain->config.name.length},
            conn->websocket.config.uri, frame->content, text)) {
  ...
  case OV_WEBSOCKET_FRAGMENTATION_START:
    ...
    if (!conn->websocket.queue) {
      ...
      if (!conn->websocket.queue) {
        ...
        ov_log_error("%s websocket fragmentation failed to create "
                     " frame queue at %i",
                     srv->config.name, conn->fd);
    ...
    if (out) {
      ...
      ov_log_error("%s websocket fragmentation failed - "
  ...
  default:
    ...
    ov_log_error("%s websocket fragmentation mismatch at %i", srv->config.name,
                 conn->fd);
  ...
  if (0 != conn->websocket.config.max_frames) {
    ...
    if (conn->websocket.counter >= conn->websocket.config.max_frames) {
      ...
      if (!callback_queue) {
        ...
        ov_log_error("%s websocket fragmentation max frames reached "
                     "%zu at %i - closing",
                     srv->config.name, conn->websocket.counter, conn->fd);
  ...
  if (callback_queue)
    return defragmented_callback(srv, conn, text);
...
static bool process_wss_frame(ov_webserver_base *srv, Connection *conn,
                              ov_websocket_frame *frame) {
  ...
  if (!ov_websocket_frame_unmask(frame)) {
    ...
    ov_log_error("%s io wss at %i from %s:%i  "
  ...
  if (conn->websocket.config.fragmented) {
    ...
    return conn->websocket.config.fragmented(
        ...
        (ov_memory_pointer){conn->domain->config.name.start,
                            conn->domain->config.name.length},
        conn->websocket.config.uri, frame);
  }
...
static bool process_wss(ov_webserver_base *srv, Connection *conn) {
  ...
  while (frame) {
    ...
    case OV_WEBSOCKET_PARSER_SUCCESS:
      ...
      if (!process_wss_frame(srv, conn, frame))
        goto error;
...
static bool process_https(ov_webserver_base *srv, Connection *conn) {
  ...
  while (msg) {
    ...
    case OV_HTTP_PARSER_SUCCESS:
      ...
      if (!process_https_message(srv, conn, msg))
        goto error;
...
static bool io_tls(int socket_fd, uint8_t events, void *data) {
  ...
  if (!conn->tls.handshaked)
    return tls_perform_handshake(srv, conn);
...
static bool io_tcp_accept_secure(int socket_fd, uint8_t events, void *data) {
  ...
  if (srv->config.debug) {
    ...
    ov_log_debug("%s accepted secure connection socket %i at %s:%i from "
                 "%s:%i",
                 srv->config.name, nfd, local.host, local.port, remote.host,
                 remote.port);
  }
...
static bool io_stun(int socket_fd, uint8_t events, void *data) {
  ...
  ssize_t send = sendto(socket_fd, buffer, out, 0, (const struct sockaddr *)&sa,
  ...
  if (srv->config.debug) {
    ...
    ov_log_debug("%s STUN request at %i "
                 "sent response %s:%i with %zu bytes",
                 srv->config.name, socket_fd, data.host, data.port, send);
  }
...
static bool add_stun_listener(ov_webserver_base *srv,
                              ov_socket_configuration config, int number) {
  ...
  errno = 0;
  if (-1 == socket) {
    ov_log_error("%s failed to create socket %s:%i %i|%s", srv->config.name,
                 config.host, config.port, errno, strerror(errno));
  }
  ...
  ov_log_info("%s created STUN listener %s:%i", srv->config.name, config.host,
              config.port);
...
static int create_socket(ov_webserver_base *srv, ov_socket_configuration config,
                         bool (*callback)(int socket, uint8_t events,
                                          void *data)) {
  ...
  if (!loop->callback.set(loop, socket, EVENTS, srv, callback))
    goto error;

  ov_log_info("%s created HTTP(s) listener %i %s:%i", srv->config.name, socket,
...
static bool check_io_timeout(uint32_t timer, void *data) {
  ...
  for (size_t i = 0; i < srv->connections_max; i++) {
    ...
    if (0 == srv->conn[i].io.in.last) {
      ...
      ov_log_error("%s conn %i remote %s:%i accepted "
                   "but no IO during timeout of %" PRIu64 " (usec) - "
                   "closing",
                   srv->config.name, srv->conn[i].fd, srv->conn[i].remote.host,
    ...
    if (0 != srv->config.timer.io_timeout_usec) {
      ...
      if ((now - srv->conn[i].io.in.last) < srv->config.timer.io_timeout_usec)
        continue;
  ...
  srv->timer.io_timeout_check = loop->timer.set(
      loop, srv->config.timer.accept_to_io_timeout_usec, srv, check_io_timeout);
...
ov_webserver_base *ov_webserver_base_create(ov_webserver_base_config config) {
  ...
  if (0 == config.domain_config_path[0])
    strncpy(config.domain_config_path,
            OV_WEBSERVER_BASE_DOMAIN_CONFIG_PATH_DEFAULT, PATH_MAX);
  ...
  for (uint32_t i = 0; i < srv->connections_max; i++) {
    ...
    srv->conn[i].websocket.close.wait_for_response = 0;
    memset(srv->conn[i].websocket.close.phrase, 0,
  ...
  if (0 == config.http.secure.host[0])
    default_sockets = true;
  ...
  if (default_sockets) {
    ...
    if (!config.ip4_only) {
      ...
      srv->sockets.http = create_socket(
          srv, (ov_socket_configuration){.type = TCP, .host = "::", .port = 80},
          io_tcp_accept_redirect);

      srv->sockets.https = create_socket(
          srv,
          (ov_socket_configuration){.type = TCP, .host = "::", .port = 443},
    ...
    if (0 != config.http.redirect.host[0])
      srv->sockets.http =
          create_socket(srv, config.http.redirect, io_tcp_accept_redirect);
  ...
  if (-1 == srv->sockets.https) {
    ...
    ov_log_error("%s failed to open listener socket - unrecoverable "
                 "failure",
                 srv->config.name);
    ...
    ov_socket_data local = {0};
    if (!ov_socket_get_data(srv->sockets.https, &local, NULL))
      goto error;

    ov_log_info("%s open https at %s:%i", srv->config.name, local.host,
                local.port);
  }
  ...
  if (recv_buf_size > send_buf_size)
    default_buf_size = recv_buf_size;
  ...
  for (size_t i = 0; i < OV_WEBSERVER_BASE_MAX_STUN_LISTENER; i++) {
    ...
    if (0 == config.stun.socket[i].host[0])
      continue;
    ...
    if (!add_stun_listener(srv, config.stun.socket[i], i)) {
      ...
      ov_log_error("%s failed to create socket %s:%i %i|%s", config.name,
                   config.stun.socket[i].host, config.stun.socket[i].port,
                   errno, strerror(errno));
    }
  }

  /* Load domains */
  if (!ov_domain_load(srv->config.domain_config_path, &srv->domain.size,
                      &srv->domain.array)) {
    ov_log_error("%s failed to load domains from %s", srv->config.name,
  ...
  for (size_t i = 0; i < srv->domain.size; i++) {
    ...
    if (!tls_init_context(srv, &srv->domain.array[i]))
      goto error;
...
ov_webserver_base_config_from_json(const ov_json_value *value) {
  ...
  if (http) {
    ...
    out.http.redirect =
        ov_socket_configuration_from_json(http, (ov_socket_configuration){0});
  }

  if (https) {

    out.http.secure =
        ov_socket_configuration_from_json(https, (ov_socket_configuration){0});
  }
  ...
  if (stun) {
    ...
    for (size_t i = 1; i <= ov_json_array_count(stun); i++) {
      ...
      out.stun.socket[i - 1] =
          ov_socket_configuration_from_json(item, (ov_socket_configuration){0});
    }
  }
...
ov_webserver_base_config_to_json(ov_webserver_base_config config) {
  ...
  if (!ov_json_object_set(out, OV_KEY_SOCKETS, val))
    goto error;
  ...
  if (ov_socket_configuration_to_json(config.http.secure, &val))
    if (!ov_json_object_set(sockets, OV_KEY_HTTPS, val))
      goto error;

  val = NULL;

  if (ov_socket_configuration_to_json(config.http.redirect, &val))
    if (!ov_json_object_set(sockets, OV_KEY_HTTP, val))
      goto error;

  val = NULL;
  val = ov_json_array();

  if (!ov_json_object_set(sockets, OV_KEY_STUN, val))
    goto error;
  ...
  for (size_t i = 0; i < OV_WEBSERVER_BASE_MAX_STUN_LISTENER; i++) {
    ...
    if (!ov_json_array_push(stun, val))
      goto error;
  }

  if (config.ip4_only) {
    val = ov_json_true();
    if (!ov_json_object_set(out, OV_KEY_IP4_ONLY, val))
      goto error;
  }

  val = ov_http_message_config_to_json(config.http_message);
  if (val && !ov_json_object_set(out, OV_KEY_HTTP_MESSAGE, val))
    goto error;

  val = ov_websocket_frame_config_to_json(config.websocket_frame);
  if (val && !ov_json_object_set(out, OV_KEY_WEBSOCKET, val))
    goto error;

  if (0 != config.domain_config_path[0]) {
    val = ov_json_string(config.domain_config_path);

    if (!ov_json_object_set(out, OV_KEY_DOMAINS, val))
      goto error;
  }
  ...
  if (!ov_json_object_set(timer, OV_KEY_IO, val))
    goto error;

  val = ov_json_number(config.timer.accept_to_io_timeout_usec);
  if (!ov_json_object_set(timer, OV_KEY_ACCEPT, val))
    goto error;

  val = ov_json_object();
  if (!ov_json_object_set(out, OV_KEY_LIMITS, val))
    goto error;
  ...
  if (!ov_json_object_set(limits, OV_KEY_SOCKETS, val))
    goto error;

  val = ov_json_number(config.limit.max_content_bytes_per_websocket_frame);
  if (!ov_json_object_set(limits, OV_KEY_WEBSOCKET, val))
    goto error;
...
static void close_socket(ov_webserver_base *srv, ov_event_loop *loop,
                         int *socket) {
  ...
  *socket = -1;
  ...
  if (!loop->callback.unset(loop, sock, NULL))
    ov_log_error("%s failed to unset IO callback at %i", srv->config.name,
                 sock);
...
ov_webserver_base *ov_webserver_base_free(ov_webserver_base *srv) {
  ...
  srv->domain.array = ov_domain_array_free(srv->domain.size, srv->domain.array);
...
bool ov_webserver_base_configure_websocket_callback(
    ov_webserver_base *srv, const ov_memory_pointer hostname,
    ov_websocket_message_config config) {

  char *key = NULL;
  ov_websocket_message_config *val = NULL;

  if (!srv || !hostname.start || (hostname.length == 0))
    goto error;
  ...
  if (!domain) {
    ...
    ov_log_error("%s cannot configure websocket callback for domain "
                 "%.*s - domain not enabled",
                 srv->config.name, (int)hostname.length, hostname.start);
  ...
  if (NULL == config.userdata) {
    ...
    ov_log_debug("%s all websocket callbacks for domain "
                 "%.*s - unset",
                 srv->config.name, (int)hostname.length, hostname.start);
  ...
  if ((NULL == config.callback) && (NULL == config.fragmented)) {
    ...
    if (0 != config.uri[0]) {
      ...
      ov_log_debug("%s websocket callback for domain "
                   "%.*s uri %s - unset",
                   srv->config.name, (int)hostname.length, hostname.start,
                   config.uri);
  ...
  if (0 == config.uri[0]) {
    ...
    ov_log_debug("%s configured websocket callback for domain "
                 "%.*s",
                 srv->config.name, (int)hostname.length, hostname.start);
  ...
  *val = config;
  ...
  if (!ov_dict_set(domain->websocket.uri, key, val, NULL)) {
    ...
    ov_log_error("%s configure websocket callback for domain "
                 "%.*s URI %s - failed",
                 srv->config.name, (int)hostname.length, hostname.start,
                 config.uri);
...
bool ov_webserver_base_configure_uri_event_io(
  ...
  if (NULL != config.userdata && (0 == config.name[0])) {
    ov_log_error("%s cannot configure event_handler callback for domain "
  ...
  if (domain->event_handler.uri) {
    ...
    if (ov_dict_get(domain->event_handler.uri, config.name)) {
      ...
      ov_log_error("%s cannot override or delete existing event_handler "
  ...
  if (wss_io_handler)
    msg_config = *wss_io_handler;

  if (0 != config.name[0])
    strncat(msg_config.uri, config.name, PATH_MAX - 1);

  // perform WS config reset on no userdata
  if (!config.userdata)
    msg_config = (ov_websocket_message_config){0};

  // perform WS URI config reset on no processing
  if (0 == config.callback.process)
    msg_config.callback = NULL;

  if (!ov_webserver_base_configure_websocket_callback(srv, hostname,
                                                      msg_config)) {
  ...
  if (!domain->event_handler.uri) {
    ...
    if (check) {
      ...
      if (check->userdata != config.userdata)
        goto error;
    }
  }
  ...
  *val = config;
  ...
  if (!ov_dict_set(domain->event_handler.uri, key, val, NULL)) {
    ...
    ov_log_error("%s configure event_handler callback for domain "
                 "%.*s URI %s - failed",
                 srv->config.name, (int)hostname.length, hostname.start,
                 config.name);
  ...
  ov_log_debug("%s configured event_handler callback for domain "
               "%.*s URI %s",
               srv->config.name, (int)hostname.length, hostname.start,
               config.name);
  return true;
...
bool ov_webserver_base_send_secure(ov_webserver_base *srv, int socket,
                                   ov_buffer const *const buffer) {

  if (!srv || (socket < 0) || !buffer)
    goto error;
  ...
  if (!conn) {
    ...
    if (srv->config.debug)
      ov_log_debug("%s send request for socket %i - "
                   "socket not connected | abort",
                   srv->config.name, socket);
...
static bool answer_get_range(ov_webserver_base *srv, ov_file_format_desc fmt,
                             const ov_http_message *request, const char *path,
                             const ov_http_header *range, Connection *conn) {
  ...
  if (request) {
    ...
    msg = ov_http_create_status_string(request->config, request->version, 206,
                                       OV_HTTP_PARTIAL_CONTENT);

  } else {

    msg = ov_http_create_status_string(
        srv->config.http_message, (ov_http_version){.major = 1, .minor = 1},
        200, OV_HTTP_OK);
  }
  ...
  if (!ov_http_message_add_header_string(msg, OV_KEY_SERVER, srv->config.name))
    goto error;
...
bool ov_webserver_base_answer_get(ov_webserver_base *srv, int socket,
  ...
  if (1 > fmt.desc.bytes) {
    ...
    ov_log_error("%s answer get without bytes - unsupported", srv->config.name);
  ...
  if (0 == fmt.mime[0]) {
    ...
    ov_log_error("%s answer get without mime type - unsupported",
                 srv->config.name);
  ...
  if (!conn) {
    ...
    if (srv->config.debug)
      ov_log_debug("%s answer get for socket %i - "
                   "socket not connected | abort",
                   srv->config.name, socket);
  ...
  if (!cleaned_path_for_conn(conn, request, PATH_MAX, path)) {
    ...
    ov_log_error("%s failed to create clean path at %i", srv->config.name,
                 socket);
  ...
  if (range)
    return answer_get_range(srv, fmt, request, path, range, conn);
  ...
  if (request) {
    ...
    msg = ov_http_create_status_string(request->config, request->version, 200,
                                       OV_HTTP_OK);
...
static bool answer_head_range(ov_webserver_base *srv, ov_file_format_desc fmt,
                              const ov_http_message *request, const char *path,
                              const ov_http_header *range, Connection *conn) {
...
bool ov_webserver_base_answer_head(ov_webserver_base *srv, int socket,
  ...
  if (range)
    return answer_head_range(srv, fmt, request, path, range, conn);
...
bool ov_webserver_base_close(ov_webserver_base *srv, int socket) {
  ...
  if (socket > (int)srv->connections_max)
    goto error;
...
bool ov_webserver_base_uri_file_path(ov_webserver_base *srv, int socket,
                                     const ov_http_message *msg, size_t length,
                                     char *path) {
