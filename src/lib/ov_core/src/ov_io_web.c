...
static bool close_connection(ov_io_web *self, Connection *conn) {
  ...
  if (!ov_event_loop_unset(self->config.loop, socket, NULL)) {
...
static void cb_io_close(void *userdata, int socket) {
  ...
  if (socket >= self->config.limits.sockets)
    goto error;
...
static bool cb_io_accept(void *userdata, int listener, int connection) {
  ...
  if (connection >= self->config.limits.sockets)
    goto error;

  if (listener != self->socket)
    goto error;
...
static bool process_wss_control_frame(ov_io_web *self, Connection *conn,
...
static bool defragmented_callback(ov_io_web *self, Connection *conn,
                                  bool text) {
  ...
  bool (*callback)(void *userdata, int socket, const ov_memory_pointer domain,
      ...
      conn->websocket->config.callback;
...
static bool defragmented_wss_delivery(ov_io_web *self, Connection *conn,
  ...
  case OV_WEBSOCKET_FRAGMENTATION_NONE:
    ...
    const char *domain = conn->domain;
    const char *uri = conn->websocket->config.uri;
    bool (*callback)(void *, int, const ov_memory_pointer, const char *,
                     ov_memory_pointer, bool) =
        conn->websocket->config.callback;

    if (!callback(userdata, socket,
                  (ov_memory_pointer){.start = (uint8_t *)domain,
                                      .length = strlen(domain)},
                  uri, frame->content, text))
      goto error;
  ...
  if (callback_queue)
    return defragmented_callback(self, conn, text);
...
static bool process_wss_frame(ov_io_web *self, Connection *conn,
                              ov_websocket_frame *frame) {
  ...
  if (conn->websocket->config.fragmented) {
    ...
    bool (*callback)(void *, int, const ov_memory_pointer, const char *,
                     ov_websocket_frame *) = conn->websocket->config.fragmented;

    return callback(userdata, socket,
...
static bool process_wss(ov_io_web *self, Connection *conn) {
  ...
  do {
    ...
    case OV_WEBSOCKET_PARSER_SUCCESS:
      ...
      if (!process_wss_frame(self, conn, frame))
        goto error;
...
static ov_websocket_message_config
get_websocket_message_config(ov_io_web *self, const ov_http_message *msg) {
  ...
  if (!memcpy(hostname_key, hostname, hostname_length))
    goto error;
...
static bool upgrade_to_websocket(ov_io_web *self, Connection *conn,
  ...
  ov_websocket_message_config config = get_websocket_message_config(self, msg);
...
static bool process_https_message(ov_io_web *self, Connection *conn,
  ...
  if (ov_websocket_process_handshake_request(msg, &response,
    ...
    if (self->config.callbacks.https) {
      ...
      return self->config.callbacks.https(self->config.callbacks.userdata,
                                          conn->socket, msg);
    }
  }
...
static bool process_https(ov_io_web *self, Connection *conn) {
  ...
  do {
    ...
    case OV_HTTP_PARSER_SUCCESS:
      ...
      if (!process_https_message(self, conn, msg))
        goto error;
...
static bool init_config(ov_io_web_config *config) {
  ...
  if (0 == config->socket.host[0])
    config->socket =
        (ov_socket_configuration){.host = "0.0.0.0", .port = 443, .type = TCP};
...
ov_io_web *ov_io_web_create(ov_io_web_config config) {
  ...
  size_t size = sizeof(ov_io_web) + config.limits.sockets * sizeof(Connection);
...
ov_io_web *ov_io_web_cast(const void *data) {
  ...
  if (*(uint16_t *)data == OV_IO_WEB_MAGIC_BYTES)
    return (ov_io_web *)data;
error:
  return NULL;
}
...
bool ov_io_web_send(ov_io_web *self, int socket, const ov_http_message *msg) {
  ...
  return ov_io_send(self->config.io, socket,
...
static bool cb_wss_send_json(void *userdata, int socket,
  ...
  if (length < chunk) {
    ...
    if (!ov_io_send(self->config.io, socket,
...
bool ov_io_web_send_json(ov_io_web *self, int socket,
                         const ov_json_value *msg) {
...
bool ov_io_web_configure_websocket_callback(
    ov_io_web *self, const char *hostname, ov_websocket_message_config config) {
  ...
  *val = config;
  ...
  ov_log_debug("configured websocket callback for domain "
               "%s URI %s",
               hostname, key);
done:
  return true;
...
bool ov_io_web_configure_uri_event_io(ov_io_web *self, const char *hostname,
  ...
  if (0 != config.name[0])
    strncpy(wss_io_handler.uri, config.name, PATH_MAX);
  ...
  *val = config;
  ...
  ov_log_debug("configured event_handler callback for domain "
               "%s URI %s",
...
static bool cleaned_path_for_conn(ov_io_web *self, Connection *conn,
