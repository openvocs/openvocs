...
static bool close_connection(ov_io_web *self, Connection *conn) {
    ...
    if (conn->websocket) {
        ...
        if (conn->websocket->config.close) {
            ...
            conn->websocket->config.close(conn->websocket->config.userdata,
                                          socket);
        }
...
static bool init_config(ov_io_web_config *config) {
    ...
    if (0 == config->socket.host[0])
        config->socket = (ov_socket_configuration){
            .host = "0.0.0.0", .port = 443, .type = TCP};
...
ov_io_web *ov_io_web_create(ov_io_web_config config) {
    ...
    size_t size =
        sizeof(ov_io_web) + config.limits.sockets * sizeof(Connection);
...
static bool cleaned_path_for_conn(ov_io_web *self, Connection *conn,
    ...
    ssize_t bytes = snprintf(full_path, PATH_MAX, "%s/%s", domain->config.path,
                             cleaned_path);
