...
static bool load_domains(ov_io *self) {
  ...
  if (!ov_domain_load(self->config.domain.path, &self->domain.size,
                      &self->domain.array)) {
    ov_log_error("failed to load domains from %s", self->config.domain.path);
    goto error;
  }
...
static bool run_reconnect(uint32_t timer, void *data) {
  ...
  while (config) {
    ...
    if (socket < 0)
      ov_log_error("reconnect attempt to %s:%i failed", config->socket.host,
                   config->socket.port);
...
ov_io *ov_io_free(ov_io *self) {
  ...
  if (OV_TIMER_INVALID != self->timer.reconnects) {
    ov_event_loop_timer_unset(self->config.loop, self->timer.reconnects, NULL);
    self->timer.reconnects = OV_TIMER_INVALID;
  }

  if (OV_TIMER_INVALID != self->timer.timeouts) {
    ov_event_loop_timer_unset(self->config.loop, self->timer.timeouts, NULL);
    self->timer.timeouts = OV_TIMER_INVALID;
  }
...
static bool io_stream_ssl(int socket, uint8_t events, void *data) {
  ...
  conn = (Connection *)ov_dict_get(self->connections, (void *)(intptr_t)socket);
  if (!conn)
    goto error;
...
static bool load_verify_locations(SSL_CTX *ctx, ov_io_ssl_config *config) {
  ...
  if (file || path) {
    ...
    if (r != 1) {
      ...
      ov_log_error("SSL_CTX_load_verify_locations failed | %s", errorstring);
...
int ov_io_open_listener(ov_io *self, ov_io_socket_config config) {
  ...
  if (!ov_dict_set(self->connections, (void *)(intptr_t)listener, conn, NULL)) {
    conn = ov_data_pointer_free(conn);
    goto error;
  }
...
ov_io_config ov_io_config_from_json(const ov_json_value *input) {
  ...
  const char *string =
      ov_json_string_get(ov_json_get(conf, "/" OV_KEY_DOMAIN "/" OV_KEY_PATH));
