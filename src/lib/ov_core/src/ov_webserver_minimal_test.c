...
static bool dummy_process(void *userdata,
                          const int socket,
...
static bool send_json_over_websocket(ov_event_loop *loop,
                                     SSL *ssl,
                                     ov_json_value *msg) {
    ...
    if (bytes != (ssize_t)frame->buffer->length) goto error;
...
int test_ov_webserver_minimal_configure_uri_event_io() {
    ...
    testrun(!ov_webserver_minimal_configure_uri_event_io(
        ...
        (ov_memory_pointer){.start = (uint8_t *)TEST_DOMAIN_NAME,
                            .length = strlen(TEST_DOMAIN_NAME)},
        (ov_event_io_config){
            .userdata = &userdata, .callback.process = dummy_process}));
...
int test_ov_webserver_minimal_config_from_json() {
    ...
    testrun(socket_configs_equal(
        &config.base.http.redirect, &EMPTY_SOCKET_CONFIGURATION));
    testrun(socket_configs_equal(
        &config.base.http.secure, &EMPTY_SOCKET_CONFIGURATION));
    for (size_t i = 0; i < OV_WEBSERVER_BASE_MAX_STUN_LISTENER; i++) {
        testrun(socket_configs_equal(
            &config.base.stun.socket[i], &EMPTY_SOCKET_CONFIGURATION));
    }
...
OV_TEST_RUN("ov_webserver_minimal",
            domains_init,
            test_ov_webserver_minimal_create,
            test_ov_webserver_minimal_free,
            test_ov_webserver_minimal_close,
            test_ov_webserver_minimal_configure_uri_event_io,
            test_ov_webserver_minimal_config_from_json,
            domains_deinit);
