...
int test_ov_domain_config_to_json() {
  ...
  testrun(
      ov_json_is_null(ov_json_get(out, "/" OV_KEY_CERTIFICATE "/" OV_KEY_KEY)));
  ...
  testrun(0 == strcmp("3", ov_json_string_get(ov_json_get(
  ...
  testrun(0 == strcmp("5", ov_json_string_get(ov_json_get(
                               out, "/" OV_KEY_CERTIFICATE "/" OV_KEY_AUTHORITY
                                    "/" OV_KEY_FILE))));
...
int test_ov_domain_load() {
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    if (0 != strcmp((char *)array[i].config.name.start, "domain3")) {
      ...
      testrun(0 == strcmp(array[i].config.certificate.ca.file, OV_TEST_CERT));
      testrun(0 == array[i].config.certificate.ca.path[0]);
    }
  }
