...
static ov_io_base *ov_io_base_cast(const void *self) {
    ...
    if (*(uint16_t *)self == OV_IO_BASE_MAGIC_BYTE) return (ov_io_base *)self;
error:
    return NULL;
}
...
static void close_connection(Connection *conn) {
    ...
    for (i = 0; i < 100; i++) {
        ...
        if (ov_thread_lock_try_lock(&conn->lock)) break;
    }
    ...
    if (conn->type == LISTENER) {
        ...
        for (size_t i = 0; i < base->connections_max; i++) {
            ...
            if (-1 == base->conn[i].fd) continue;

            if (conn->fd != base->conn[i].listener.fd) continue;
...
static void callback_connection_success(Connection *conn) {
    ...
    if (conn->config.connected)
        conn->config.connected(
            conn->config.connection.callback.userdata, conn->fd, true);

    if (conn->config.auto_reconnect) {

        ov_log_info("%s auto reconnect with socket %i",
                    conn->base->config.name,
                    conn->fd);
    }
...
static void callback_connection_error(Connection *conn) {
    ...
    if (!conn->config.auto_reconnect)
        if (conn->config.connected)
            conn->config.connected(
                conn->config.connection.callback.userdata, conn->fd, false);
...
ov_io_base *ov_io_base_create(ov_io_base_config config) {
    ...
    if (0 == config.name[0]) strncpy(config.name, "base", OV_IO_BASE_NAME_MAX);
    ...
    if (OV_TIMER_INVALID == base->timer.io_timeout_check) goto error;
    ...
    for (uint32_t i = 0; i < base->connections_max; i++) {
        ...
        if (!ov_thread_lock_init(
                &base->conn[i].lock, config.limit.thread_lock_timeout_usec))
            goto error;
    }
    ...
    ov_log_debug("IO base allocated %" PRIu64 " bytes supporting %" PRIu32
                 " so"
                 "cke"
                 "ts",
                 size,
...
static ov_domain *find_domain(const ov_io_base *base,
                              const uint8_t *name,
                              size_t len) {
    ...
    for (size_t i = 0; i < base->domain.size; i++) {
        ...
        if (base->domain.array[i].config.name.length != len) continue;
...
static bool tls_init_context(ov_io_base *base, ov_domain *domain) {
    ...
    if (!domain->context.tls) {
        ov_log_error("%s TLS context for |%s| not set",
                     base->config.name,
                     domain->config.name);
        goto error;
    }
...
bool ov_io_base_load_ssl_config(ov_io_base *base, const char *path) {
    ...
    if (!ov_domain_load(path, &base->domain.size, &base->domain.array)) {
        ...
        ov_log_error(
            "%s failed to load domains from %s", base->config.name, path);
    ...
    for (size_t i = 0; i < base->domain.size; i++) {
        ...
        if (!tls_init_context(base, &base->domain.array[i])) goto error;
...
static bool tls_perform_handshake(ov_io_base *base, Connection *conn) {
    ...
    if (!base || !conn || !conn->tls.ssl) goto error;
...
call_again_later:
    ...
    conn->timer_id = loop->timer.set(loop,
                                     conn->config.client_connect_trigger_usec,
                                     conn,
...
static bool init_ssl_client(Connection *conn) {
    ...
    if (0 != conn->config.ssl.domain[0]) {
        ...
        if (1 != SSL_set_tlsext_host_name(
            ...
            ov_log_error(
                "SSL set hostname failed "
                "at socket %i | %s",
                conn->fd,
                conn->config.ssl.domain);
...
cleanup:
    ...
    if (conn->tls.ssl) SSL_free(conn->tls.ssl);

    if (conn->tls.ctx) SSL_CTX_free(conn->tls.ctx);
...
ov_io_base_statistics ov_io_base_get_statistics(ov_io_base *base, int socket) {
    ...
    if (!base || (0 > socket)) return (ov_io_base_statistics){0};
...
static bool io_send(ov_io_base *base,
                    Connection *conn,
                    ov_event_loop *loop,
    ...
    if (conn->out.buffer) {
        ...
        if (!send_out(conn, &conn->out.buffer)) goto error;
        ...
        if (NULL == buffer) {
            ...
            if (!r) {
                ...
                if (buffer) conn->out.buffer = buffer;
            }
        }
    }
...
static bool ssl_recv(Connection *conn) {
    ...
    if (conn->config.connection.callback.io(
        ...
        if (conn->base->config.debug)
            ov_log_debug(
                "%s leave data in network queue due to "
...
static bool io_ssl_client(int socket, uint8_t events, void *data) {
    ...
    if ((events & OV_EVENT_IO_CLOSE) || (events & OV_EVENT_IO_ERR)) {
        ...
        if (base->config.debug)
            ov_log_debug("%s ssl close at %i remote %s:%i",
                         base->config.name,
                         conn->fd,
                         conn->stats.remote.host,
                         conn->stats.remote.port);
    ...
    if (events & OV_EVENT_IO_IN) {
        ...
        if (!io_send(base, conn, loop, ssl_send, io_ssl_client)) goto error;
...
static bool io_ssl(int socket, uint8_t events, void *data) {
    ...
    if (!conn->tls.handshaked) return tls_perform_handshake(base, conn);
    ...
    if (events & OV_EVENT_IO_IN) {
        ...
        if (!io_send(base, conn, loop, ssl_send, io_ssl)) goto error;
...
static bool io_stream(int socket, uint8_t events, void *data) {
    ...
    if (events & OV_EVENT_IO_IN) {
        ...
        if (!io_send(base, conn, loop, stream_send, io_stream)) goto error;
...
static Connection *init_connection(ov_io_base *base,
                                   Connection *listener,
                                   int socket) {
    ...
    if (nfd < 0) {
        ...
        ov_log_error(
            "%s failed to accept at socket %i", base->config.name, socket);
    ...
    if (-1 != base->conn[nfd].fd) goto error;
    ...
    switch (conn->stats.type) {
        ...
        case LOCAL:
            ...
            conn->in.recv_buffer_size =
                ov_socket_get_recv_buffer_size(conn->fd);
            if (-1 == conn->in.recv_buffer_size) goto error;
...
static Connection *accept_stream_base(int socket,
                                      uint8_t events,
                                      void *userdata) {
...
static void log_accepted(ov_io_base *base, int socket) {
    ...
    if (!ov_socket_get_data(socket, &local, &remote)) {
        ov_log_error(
            "%s failed to parse data at accepted socket "
            "%i",
            base->config.name,
            socket);
        goto error;
...
static bool accept_stream(int socket, uint8_t events, void *userdata) {
    ...
    if (base->config.debug) log_accepted(base, conn->fd);
...
static bool accept_tls(int socket, uint8_t events, void *userdata) {
    ...
    int id = 0;
    if (-1 != base->domain.standard) id = base->domain.standard;

    ssl = SSL_new(base->domain.array[id].context.tls);
    if (!ssl) goto error;

    if (1 != SSL_set_fd(ssl, conn->fd)) goto error;
...
int ov_io_base_create_listener(ov_io_base *base,
    ...
    ov_log_debug("%s created listener %s:%i",
                 base->config.name,
                 config.socket.host,
                 config.socket.port);
...
int ov_io_base_create_connection(ov_io_base *base,
    ...
    if (TLS == config.connection.socket.type) {
        ...
        if (!init_ssl_client(conn)) goto cleanup;
    }
    ...
    switch (config.connection.socket.type) {
        ...
        case LOCAL:
            ...
            conn->in.recv_buffer_size =
                ov_socket_get_recv_buffer_size(conn->fd);
            if (-1 == conn->in.recv_buffer_size) goto cleanup;
    ...
    ov_log_debug("%s created connection socket %i to %s:%i",
                 base->config.name,
                 socket,
                 config.connection.socket.host,
                 config.connection.socket.port);
...
bool ov_io_base_send(ov_io_base *base,
                     int socket,
    ...
    if (socket > (int)base->connections_max) goto error;

    Connection *conn = &base->conn[socket];

    if (conn->fd == -1) goto error;

    if ((conn->type != CONNECTION) && (conn->type != CLIENT)) goto error;
    ...
    if (!ov_thread_lock_try_lock(&conn->lock)) goto error;
    ...
    result = loop->callback.set(
        loop,
        conn->fd,
        OV_EVENT_IO_CLOSE | OV_EVENT_IO_IN | OV_EVENT_IO_ERR | OV_EVENT_IO_OUT,
        conn->base,
        send_out);
...
ov_io_base_config ov_io_base_config_from_json(const ov_json_value *input) {
    ...
    if (name) strncpy(out.name, name, OV_IO_BASE_NAME_MAX);
...
ov_json_value *ov_io_base_config_to_json(const ov_io_base_config config) {
    ...
    val = ov_json_object();
    if (!ov_json_object_set(out, OV_KEY_LIMIT, val)) goto error;

    ov_json_value *tmp = val;
    val = NULL;

    val = ov_json_number(config.limit.max_sockets);
    if (!ov_json_object_set(tmp, OV_KEY_SOCKETS, val)) goto error;

    val = ov_json_number(config.limit.max_send);
    if (!ov_json_object_set(tmp, OV_KEY_SEND, val)) goto error;
    ...
    if (!ov_json_object_set(tmp, OV_KEY_TIMEOUT_USEC, val)) goto error;

    val = ov_json_number(config.timer.accept_to_io_timeout_usec);
    if (!ov_json_object_set(tmp, OV_KEY_ACCEPT_TIMEOUT_USEC, val)) goto error;

    val = ov_json_number(config.timer.reconnect_interval_usec);
    if (!ov_json_object_set(tmp, OV_KEY_RECONNECT_USEC, val)) goto error;

    val = ov_json_object();
    if (!ov_json_object_set(out, OV_KEY_CACHE, val)) goto error;

    tmp = val;
    val = ov_json_number(config.cache.capacity);
    if (!ov_json_object_set(tmp, OV_KEY_CAPACITY, val)) goto error;
