...
static void callback_connection_success(Connection *conn) {
  ...
  if (conn->config.connected)
    conn->config.connected(conn->config.connection.callback.userdata, conn->fd,
                           true);
...
static bool check_io_timeout(uint32_t timer, void *data) {
  ...
  base->timer.io_timeout_check =
      loop->timer.set(loop, base->timer.timeout_usecs, base, check_io_timeout);
  ...
  if (now >=
      (base->last_reconnect_run + base->config.timer.reconnect_interval_usec)) {
...
ov_io_base *ov_io_base_create(ov_io_base_config config) {
  ...
  uint64_t size =
      config.limit.max_sockets * sizeof(struct Connection) + sizeof(ov_io_base);
...
bool ov_io_base_load_ssl_config(ov_io_base *base, const char *path) {
  ...
  if (!ov_domain_load(path, &base->domain.size, &base->domain.array)) {
    ...
    ov_log_error("%s failed to load domains from %s", base->config.name, path);
...
static bool tls_perform_handshake(ov_io_base *base, Connection *conn) {
  ...
  if (1 == r) {
    ...
    if (!hostname) {
      ...
      conn->domain = find_domain(base, (uint8_t *)hostname, strlen(hostname));
    }
  ...
  if (conn->out.buffer) {
    ...
    if (NULL == buffer) {
      ...
      if (!loop->callback.set(loop, conn->fd,
        ...
        ov_log_error("%s failed to reset events at %i", base->config.name,
                     conn->fd);
...
static bool ssl_send(Connection *conn, ov_buffer **input) {
  ...
  *input = NULL;
  ...
  if (conn->base->config.debug)
    ov_log_debug("%s send %zi bytes at %i to %s:%i", conn->base->config.name,
                 bytes, conn->fd, conn->stats.remote.host,
...
static bool ssl_recv(Connection *conn) {
  ...
  if (conn->config.connection.callback.io(
    ...
    if (conn->base->config.debug)
      ov_log_debug("%s processed %zi data from %i", conn->base->config.name,
                   bytes, conn->fd);
  ...
  if ((events & OV_EVENT_IO_CLOSE) || (events & OV_EVENT_IO_ERR)) {
    ...
    if (base->config.debug)
      ov_log_debug("%s ssl close at %i remote %s:%i", base->config.name,
                   conn->fd, conn->stats.remote.host, conn->stats.remote.port);
...
static bool io_stream(int socket, uint8_t events, void *data) {
  ...
  if ((events & OV_EVENT_IO_CLOSE) || (events & OV_EVENT_IO_ERR)) {
    ...
    if (base->config.debug)
      ov_log_debug("%s stream close at %i remote %s:%i", base->config.name,
                   conn->fd, conn->stats.remote.host, conn->stats.remote.port);
...
static Connection *init_connection(ov_io_base *base, Connection *listener,
  ...
  if (nfd < 0) {
    ...
    ov_log_error("%s failed to accept at socket %i", base->config.name, socket);
    ...
    if (config.auto_reconnect) {
      ...
      *conf = config;
      ...
      ov_log_error("failed to create socket %s:%i %i|%s",
                   config.connection.socket.host, config.connection.socket.port,
...
ov_io_base_config ov_io_base_config_from_json(const ov_json_value *input) {
  ...
  if (limit) {
    ...
    out.timer.reconnect_interval_usec =
        ov_json_number_get(ov_json_object_get(limit, OV_KEY_RECONNECT_USEC));
  }
