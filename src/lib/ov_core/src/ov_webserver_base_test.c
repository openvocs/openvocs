...
static bool socket_configs_equal(ov_socket_configuration const *s1,
    ...
    if ((0 == s1) && (0 == s2)) return true;

    if (0 == s1) return false;
    if (0 == s2) return false;

    if (0 != memcmp(s1->host, s2->host, OV_HOST_NAME_MAX)) return false;
    if (s1->port != s2->port) return false;
...
int test_ov_webserver_base_create() {
    ...
    testrun_log(
        "Trying to open socket at port 80 and 443,"
    ...
    if (webserver) {
        ...
        testrun_log(
            "Webserver creation in default mode failed, root may be "
            "required");
    }
    ...
    if (webserver) {
        ...
        testrun_log(
            "Webserver creation in default mode failed, root mey be "
            "required");
    }
    webserver = ov_webserver_base_free(webserver);
...
int check_stun() {
    ...
    for (size_t i = 0; i < OV_WEBSERVER_BASE_MAX_STUN_LISTENER; i++) {
        ...
        testrun(ov_socket_get_sockaddr_storage(
            srv->sockets.stun[i], &sa_out, NULL, NULL));
        ...
        while (in == -1) {
            testrun(loop->run(loop, TEST_RUNTIME_USECS));
            in = recvfrom(
                client, buf2, size, 0, (struct sockaddr *)&sa_in, &in_len);
            counter++;
            testrun(counter < 10);
        }
        ...
        if (test_debug_log) fprintf(stdout, "check STUN %zu\n", i);
    ...
    testrun(ov_socket_get_sockaddr_storage(
        srv->sockets.stun[0], &sa_out, NULL, NULL));
    ...
    testrun(ov_stun_software_encode(
        buf1 + 20, size - 20, NULL, (uint8_t *)software, strlen(software)));
    testrun(
        ov_stun_frame_set_length(buf1,
                                 size,
                                 ov_stun_software_encoding_length(
                                     (uint8_t *)software, strlen(software))));

    size_t len = 20 + ov_stun_software_encoding_length(
                          (uint8_t *)software, strlen(software));
    ...
    testrun(ov_stun_frame_set_length(
        buf1, size, ov_stun_fingerprint_encoding_length()));
    ...
    if (test_debug_log) fprintf(stdout, "\nSending RANDOM buffers\n");
    ...
    for (size_t i = 0; i < 10; i++) {
        ...
        if (test_debug_log) fprintf(stdout, "Check random %zu\n", i);
...
int check_redirect() {
    ...
    if (test_debug_log)
        fprintf(stdout,
                "Roundtrip %" PRIu64 " (usec) %zi bytes\n",
                end - start,
                bytes);
    // fprintf(stdout, "|%s|\n", buf1);
    testrun(snprintf((char *)buf2,
                     size,
                     "HTTP/1.1 301 Moved "
                     "Permanently\r\nLocation:https://x:%i\r\n\r\n",
    ...
    testrun(loop->run(loop, TEST_RUNTIME_USECS));
    if (test_debug_log) fprintf(stdout, "--> reconnect client");
    ...
    for (size_t i = 0; i < 25; i++) {
        ...
        if (errno == ETIMEDOUT) {
            ...
            if (test_debug_log)
                fprintf(
                    stdout, "---> %zu| client %i TCP TIMEDOUT\n", i, client);
    ...
    if (test_debug_log) fprintf(stdout, "-------------------- cleanup\n");
    testrun(loop->run(loop, TEST_RUNTIME_USECS));
    testrun(NULL == ov_webserver_base_free(srv));
    testrun(NULL == ov_event_loop_free(loop));
        ...
        if (bytes < 1) {
            ...
            switch (n) {
                ...
                case SSL_ERROR_SYSCALL:
                    ...
                    fprintf(stdout,
                            "SSL_ERROR_SYSCALL - stop %i|%s errno %i|%s\n",
                            errorcode,
                            errorstring,
                            errno,
                            strerror(errno));
                ...
                case SSL_ERROR_SSL:
                    ...
                    fprintf(stdout,
                            "SSL_ERROR_SSL - stop %i|%s errno %i|%s\n",
                            errorcode,
                            errorstring,
                            errno,
                            strerror(errno));
...
static bool dummy_ws_callback(void *userdata,
                              int socket,
    ...
    if (!userdata || socket < 1) goto error;
    ...
    if (test_debug_log)
        fprintf(stdout,
                "dummy_ws_callback at %i | %i text %s bytes %zu\n",
                socket,
                x,
                text ? "true" : "false",
                content.length);
...
static bool dummy_ws_callback_fragmented_copy_to_userdata(
    ...
    if (!userdata || socket < 1 || !frame) goto error;
    ...
    if (!ov_buffer_clear(buffer)) goto error;
...
static bool dummy_ws_callback_copy_to_userdata(void *userdata,
                                               int socket,
                                               const ov_memory_pointer domain,
                                               const char *uri,
                                               ov_memory_pointer content,
    ...
    if (!ov_buffer_set(buffer, content.start, content.length)) goto error;

    if (text) { /* ignore */
    };

    if (test_debug_log) fprintf(stdout, "copy_to_userdata at %i\n", socket);
    return true;
error:
    return false;
}
...
int test_ov_webserver_base_configure_websocket_callback() {
    ...
    testrun(0 == memcmp("openvocs.test",
    ...
    testrun(!ov_webserver_base_configure_websocket_callback(
        NULL, hostname, ws_config));

    hostname.start = NULL;

    testrun(!ov_webserver_base_configure_websocket_callback(
        srv, hostname, ws_config));
    ...
    ws_config = (ov_websocket_message_config){
        .userdata = userdata, .callback = dummy_ws_callback};
...
static bool dummy_event_callback(void *userdata,
                                 const int socket,
...
int test_ov_webserver_base_configure_uri_event_io() {
    ...
    testrun(!ov_webserver_base_configure_uri_event_io(
        NULL, hostname, event_config, NULL));

    hostname.start = NULL;

    testrun(!ov_webserver_base_configure_uri_event_io(
        srv, hostname, event_config, NULL));
...
int check_websocket_callback_fragmentation() {
    ...
    if (test_debug_log) fprintf(stdout, "CHECK openvocs.test with request /\n");
    ssize_t bytes = -1;
...
int check_websocket_callback() {
    ...
    testrun(0 == memcmp("openvocs.test",
                        openvocs->config.name.start,
                        openvocs->config.name.length));

    testrun(0 == memcmp("one.test",
                        one->config.name.start,
                        one->config.name.length));

    testrun(0 == memcmp("two.test",
                        two->config.name.start,
    ...
    testrun(ov_webserver_base_configure_websocket_callback(
        srv,
        (ov_memory_pointer){
            .start = one->config.name.start, .length = one->config.name.length},
        ws_config));
    ...
    testrun(ov_webserver_base_configure_websocket_callback(
        srv,
        (ov_memory_pointer){
            .start = two->config.name.start, .length = two->config.name.length},
        ws_config));
    ...
    msg = ov_http_create_request_string(
        http_config, http_version, "GET", "/unenabled");
    testrun(msg);
    testrun(
    ...
    msg = ov_http_create_request_string(
        http_config, http_version, "GET", "/chat");
    testrun(msg);
    testrun(
        ov_http_message_add_header_string(msg, OV_HTTP_KEY_HOST, "one.test"));
    testrun(ov_http_message_add_header_string(
    ...
    if (test_debug_log) fprintf(stdout, "CHECK one.test with request /chat\n");
...
int test_ov_webserver_base_config_from_json() {
    ...
    testrun(socket_configs_equal(
        &config.http.redirect, &EMPTY_SOCKET_CONFIGURATION));
    testrun(
    ...
    for (size_t i = 0; i < OV_WEBSERVER_BASE_MAX_STUN_LISTENER; i++) {
        testrun(socket_configs_equal(
            &config.stun.socket[i], &EMPTY_SOCKET_CONFIGURATION));
    }
...
int test_ov_webserver_base_config_to_json() {
    ...
    config = (ov_webserver_base_config){
        ...
        .stun.socket[0] =
            (ov_socket_configuration){
                .type = UDP, .host = "localhost", .port = 1},
    ...
    testrun(0 ==
            strcmp("name",
                   ov_json_string_get(ov_json_object_get(out, OV_KEY_NAME))));
    testrun(0 == strcmp("path",
                        ov_json_string_get(
...
int test_ov_webserver_base_answer_get() {
    ...
    while (retry) {
        ...
        bytes = SSL_read(ssl,
                         response->buffer->start + response->buffer->length,
    ...
    testrun(header);
    testrun(0 == memcmp(mime_with_charset,
                        header->value.start,
                        header->value.length));
...
int check_wss_events() {
    ...
    testrun(ov_websocket_set_data(
        frame, (uint8_t *)content, strlen(content), true));
        ...
        switch (i) {
            ...
            case 3:
                ...
                testrun(0 == strcmp("json",
                                    ov_json_string_get(ov_json_object_get(
                                        data[0].value, "some"))));
    ...
    for (size_t i = 0; i < items; i++) {
        ...
        // check content
        val = ov_json_value_from_string(
            (char *)frame->content.start, frame->content.length);
        testrun(val);
    ...
    // check not masked
    testrun(!(frame->buffer->start[1] & 0x80));
    testrun(ov_buffer_push(
        content_buffer, (void *)frame->content.start, frame->content.length));
    ...
    val = ov_json_value_from_string(
        (char *)content_buffer->start, content_buffer->length);
    testrun(val);
    testrun(1 == ov_json_object_count(val));
    testrun(0 == strcmp("json",
                        ov_json_string_get(ov_json_object_get(val, "some"))));
...
int test_ov_webserver_base_uri_file_path() {
    ...
    testrun(ov_uri_path_remove_dot_segments(
        connection->domain->config.path, root_path));
...
OV_TEST_RUN("ov_webserver_base",
            domains_init,

            test_ov_webserver_base_cast,
            test_ov_webserver_base_create,
            test_ov_webserver_base_free,

            test_ov_webserver_base_close,

            test_ov_webserver_base_send_secure,
            test_ov_webserver_base_send_json,
            test_ov_webserver_base_answer_get,
            ...
            check_stun,
            check_redirect,
            check_tls_send_buffer,
            check_sni,
            check_wss_events,
            check_websocket_callback,
            check_websocket_callback_fragmentation,
            check_process_https,
            domains_deinit);
