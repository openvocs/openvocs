...
int check_stun() {
  ...
  for (size_t i = 0; i < OV_WEBSERVER_BASE_MAX_STUN_LISTENER; i++) {
    ...
    testrun(ov_socket_get_sockaddr_storage(srv->sockets.stun[i], &sa_out, NULL,
                                           NULL));
    ...
    while (in == -1) {
      testrun(loop->run(loop, TEST_RUNTIME_USECS));
      in = recvfrom(client, buf2, size, 0, (struct sockaddr *)&sa_in, &in_len);
      counter++;
      testrun(counter < 10);
    }
  ...
  testrun(ov_stun_frame_set_length(
      buf1, size,
      ov_stun_software_encoding_length((uint8_t *)software, strlen(software))));
  ...
  testrun(ov_stun_frame_generate_transaction_id(buf1 + 8));
  testrun(ov_stun_frame_set_request(buf1, size));
  testrun(ov_stun_realm_encode(buf1 + 20, size - 20, NULL, (uint8_t *)software,
                               strlen(software)));
  testrun(ov_stun_frame_set_length(
      buf1, size,
  ...
  len =
      20 + ov_stun_realm_encoding_length((uint8_t *)software, strlen(software));
...
int check_redirect() {
  ...
  for (size_t i = 0; i < 25; i++) {
    ...
    if (test_debug_log)
      fprintf(stdout, "%zu| client %i send %zu random bytes read %zi bytes\n",
              i, client, size, bytes);
...
int check_sni() {
  ...
  testrun(0 == memcmp(TEST_DOMAIN_NAME, srv->domain.array[0].config.name.start,
  ...
  domain->config.name.start[6] = 0xF0; // 4 byte UTF8 Gothic Letter Ahsa U+10330
...
int check_tls_send_buffer() {
  ...
  size = 4;
  testrun(0 == setsockopt(conn->fd, SOL_SOCKET, SO_SNDBUF, (const void *)&size,
                          sizeof(size)));
...
static bool dummy_ws_callback(void *userdata, int socket,
  ...
  if (test_debug_log)
    fprintf(stdout, "dummy_ws_callback at %i | %i text %s bytes %zu\n", socket,
            x, text ? "true" : "false", content.length);
...
int test_ov_webserver_base_configure_websocket_callback() {
  ...
  ov_memory_pointer hostname = {.start = srv->domain.array[0].config.name.start,
                                .length =
                                    srv->domain.array[0].config.name.length};
  ...
  // min input == reset
  testrun(
      ov_webserver_base_configure_websocket_callback(srv, hostname, ws_config));
  ...
  ov_websocket_message_config *out = ov_dict_get(domain->websocket.uri, "/abc");
  testrun(out);
...
int test_ov_webserver_base_configure_uri_event_io() {
  ...
  testrun(!ov_webserver_base_configure_uri_event_io(srv, hostname, event_config,
                                                    NULL));
...
int check_websocket_callback() {
  ...
  testrun(0 ==
          memcmp("one.test", one->config.name.start, one->config.name.length));

  testrun(0 ==
          memcmp("two.test", two->config.name.start, two->config.name.length));
  ...
  msg = ov_http_create_request_string(http_config, http_version, "GET",
                                      "/unenabled");
  testrun(msg);
  testrun(ov_http_message_add_header_string(msg, OV_HTTP_KEY_HOST, "one.test"));
  testrun(ov_http_message_add_header_string(msg, OV_HTTP_KEY_UPGRADE,
                                            OV_WEBSOCKET_KEY));
  testrun(ov_http_message_add_header_string(msg, OV_HTTP_KEY_CONNECTION,
                                            OV_HTTP_KEY_UPGRADE));
  ...
  msg =
      ov_http_create_request_string(http_config, http_version, "GET", "/chat");
  testrun(msg);
  testrun(ov_http_message_add_header_string(msg, OV_HTTP_KEY_HOST, "one.test"));
  testrun(ov_http_message_add_header_string(msg, OV_HTTP_KEY_UPGRADE,
                                            OV_WEBSOCKET_KEY));
  ...
  msg = ov_http_create_request_string(http_config, http_version, "GET", "/");
  testrun(msg);
  testrun(ov_http_message_add_header_string(msg, OV_HTTP_KEY_HOST, "two.test"));
  testrun(ov_http_message_add_header_string(msg, OV_HTTP_KEY_UPGRADE,
                                            OV_WEBSOCKET_KEY));
  testrun(ov_http_message_add_header_string(msg, OV_HTTP_KEY_CONNECTION,
                                            OV_HTTP_KEY_UPGRADE));
...
int test_ov_webserver_base_config_from_json() {
  ...
  ov_socket_configuration https =
      (ov_socket_configuration){.type = TCP, .host = "localhost", .port = 443};
  ...
  testrun(
      socket_configs_equal(&config.http.redirect, &EMPTY_SOCKET_CONFIGURATION));
  testrun(
  ...
  for (size_t i = 0; i < OV_WEBSERVER_BASE_MAX_STUN_LISTENER; i++) {
    if (i < stun_config_items) {
      testrun(socket_configs_equal(&config.stun.socket[i], &stun_configs[i]));
    } else {
      testrun(socket_configs_equal(&config.stun.socket[i],
                                   &EMPTY_SOCKET_CONFIGURATION));
    }
...
int test_ov_webserver_base_answer_get() {
  ...
  testrun(OV_FILE_SUCCESS == ov_file_write(file, content, content_bytes, "w"));
  ...
  while (retry) {
    ...
    bytes = SSL_read(ssl, response->buffer->start + response->buffer->length,
  ...
  testrun(ov_http_header_get_unique(
      response->header, response->config.header.capacity, OV_HTTP_KEY_SERVER));
  ...
  testrun(header);
  testrun(0 ==
          memcmp(mime_with_charset, header->value.start, header->value.length));
...
int check_wss_events() {
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    case 6:
      testrun(1 ==
              SSL_set_tlsext_host_name(ssl[i], (char *)one->config.name.start));
    ...
    case 9:
      testrun(1 ==
              SSL_set_tlsext_host_name(ssl[i], (char *)two->config.name.start));
  ...
  testrun(
      ov_websocket_set_data(frame, (uint8_t *)content, strlen(content), true));
  ...
  for (size_t i = 0; i < items; i++) {
    ...
    while (bytes == -1) {
      ...
      bytes = SSL_write(ssl[i], frame->buffer->start, frame->buffer->length);
      testrun(loop->run(loop, TEST_RUNTIME_USECS));
    }
  ...
  for (size_t i = 0; i < items; i++) {
    ...
    while (bytes == -1) {
      ...
      bytes = SSL_read(ssl[i], frame->buffer->start, frame->buffer->capacity);
      testrun(loop->run(loop, TEST_RUNTIME_USECS));
    }
    testrun(bytes > 0);
  ...
  testrun(bytes > 0);
  frame->buffer->length = bytes;
  testrun(OV_WEBSOCKET_PARSER_SUCCESS == ov_websocket_parse_frame(frame, NULL));
  ...
  testrun(1 == ov_json_object_count(val));
  testrun(0 ==
          strcmp("json", ov_json_string_get(ov_json_object_get(val, "some"))));
  val = ov_json_value_free(val);
...
int test_ov_webserver_base_send_json() {
  ...
  out = ov_json_value_from_string((const char *)buffer->start, buffer->length);
  testrun(ov_event_api_event_is(out, "test"));
  out = ov_json_value_free(out);
  ov_websocket_frame_clear(frame);
  frame = ov_websocket_frame_free(frame);
...
int test_ov_webserver_base_find_domain() {
  ...
  ov_memory_pointer p = {.start = (uint8_t *)domain, .length = strlen(domain)};
