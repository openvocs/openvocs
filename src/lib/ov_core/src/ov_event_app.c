...
typedef struct Callback {
  ...
  void *userdata;
  void (*callback)(void *userdata, const char *name, int socket,
                   ov_json_value *input);
...
bool ov_event_app_register(ov_event_app *app, const char *name, void *userdata,
  ...
  if (!ov_dict_set(app->registry, key, val, NULL))
    goto error;
  return true;
error:
...
static bool io_accept(void *userdata, int listener, int connection) {
  ...
  if (self->config.callbacks.accept)
    return self->config.callbacks.accept(self->config.callbacks.userdata,
                                         listener, connection);
...
static void io_close_socket(void *userdata, int connection) {
  ...
  if (self->config.callbacks.close)
    self->config.callbacks.close(self->config.callbacks.userdata, connection);
...
static void io_connected(void *userdata, int connection) {
  ...
  if (self->config.callbacks.connected)
    self->config.callbacks.connected(self->config.callbacks.userdata,
                                     connection);
...
bool ov_event_app_send(ov_event_app *self, int socket,
  ...
  bool r = ov_io_send(self->config.io, socket,
