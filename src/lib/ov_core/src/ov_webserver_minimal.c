...
static bool cb_accept(void *userdata, int server, int socket) {
  ...
  if (srv->config.callback.accept)
    return srv->config.callback.accept(srv->config.callback.userdata, server,
                                       socket);
...
static bool process_https_status(ov_webserver_minimal *srv, int socket,
                                 ov_http_message *msg) {
  ...
  if (srv->config.base.debug)
    ov_log_debug("%s unexpected STATUS message received "
                 "at %i %i|%s - closing",
                 srv->config.base.name, socket, msg->status.code,
                 msg->status.phrase);
...
static bool process_https_get(ov_webserver_minimal *srv, int socket,
                              ov_http_message *msg) {
  ...
  if (!ov_webserver_base_uri_file_path(srv->base, socket, msg, PATH_MAX,
                                       path)) {

    ov_log_error("%s URI request rejected |%.*s|", srv->config.base.name,
                 (int)msg->request.uri.length, (char *)msg->request.uri.start);
...
static bool process_https_head(ov_webserver_minimal *srv, int socket,
                               ov_http_message *msg) {
...
static bool process_https_request(ov_webserver_minimal *srv, int socket,
                                  ov_http_message *msg) {
  ...
  ov_log_error("%s METHOD type |%.*s| not implemented - closing",
               srv->config.base.name, (int)msg->request.method.length,
...
static bool cb_https(void *userdata, int socket, ov_http_message *msg) {
  ...
  if (0 != msg->status.code)
    return process_https_status(srv, socket, msg);
  ...
  if (srv->config.base.debug) {
    ...
    if (!value) {
      ...
      ov_log_error("%s wss io at %i for %.*s uri %s not JSON - closing",
                   srv->config.base.name, socket, (int)domain.length,
                   domain.start, uri);
      ...
      ov_log_debug("%s wss JSON io at %i for %.*s uri %s \n%.*s",
                   srv->config.base.name, socket, (int)domain.length,
                   domain.start, uri, content.length, (char *)content.start);
    }
  }
...
ov_webserver_minimal *ov_webserver_minimal_free(ov_webserver_minimal *self) {
  ...
  if (!ov_webserver_minimal_cast(self))
    return self;
...
bool ov_webserver_minimal_configure_uri_event_io(
  ...
  return ov_webserver_base_configure_uri_event_io(self->base, hostname, input,
                                                  &wss_io_handler);
}
