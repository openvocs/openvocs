...
int test_ov_io_base_load_ssl_config() {
  ...
  testrun(
      find_domain(base, (uint8_t *)TEST_DOMAIN_NAME, strlen(TEST_DOMAIN_NAME)));
...
int test_ov_io_base_create_connection() {
  ...
  testrun(0 == memcmp("test", userdata.buffer->start, userdata.buffer->length));
...
int test_ov_io_base_get_statistics() {
  ...
  for (size_t i = 0; i < items; i++) {
    ...
    // check content
    testrun(0 == memcmp(str, userdata.buffer->start, userdata.buffer->length));
    userdata.buffer = ov_buffer_free(userdata.buffer);
  }
