...
timer_reenable:
  ...
  store->invalidate_timer =
      loop->timer.set(loop, store->config.invalidate_check_interval_usec, store,
                      invalidate_run);
...
ov_event_async_store_create(ov_event_async_store_config config) {
  ...
  if (IMPL_INVALIDATE_TIMEOUT_MIN_USEC > config.invalidate_check_interval_usec)
    config.invalidate_check_interval_usec = IMPL_INVALIDATE_TIMEOUT_MIN_USEC;
