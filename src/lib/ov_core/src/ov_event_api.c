...
const char *ov_event_api_get_channel(const ov_json_value *val) {
  return ov_json_string_get(ov_json_object_get(val, OV_EVENT_API_KEY_CHANNEL));
}

/*----------------------------------------------------------------------------*/

double ov_event_api_get_version(const ov_json_value *val) {
  return ov_json_number_get(ov_json_object_get(val, OV_EVENT_API_KEY_VERSION));
}
...
bool ov_event_api_get_error_parameter(const ov_json_value *val, uint64_t *code,
  ...
  if (!error) {
    ...
    *code = ov_json_number_get(ov_json_get(error, "/" OV_EVENT_API_KEY_CODE));
