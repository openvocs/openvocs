...
static bool init_dtls_cookie_keys(size_t quantity, size_t length) {
  ...
  if (dtls_keys) {
    ...
    dtls_keys =
        ov_list_create((ov_list_config){.item = ov_buffer_data_functions()});
  }
...
static bool configure_dtls(ov_dtls *ssl) {
  ...
  if (OV_TIMER_INVALID != ssl->dtls.timer.key_renew) {
    ...
    ssl->config.loop->timer.unset(ssl->config.loop, ssl->dtls.timer.key_renew,
                                  NULL);
  }
...
ov_dtls *ov_dtls_create(ov_dtls_config config) {
  ...
  if (0 == config.srtp.profile[0])
    snprintf(config.srtp.profile, OV_DTLS_PROFILE_MAX, OV_DTLS_SRTP_PROFILES);
