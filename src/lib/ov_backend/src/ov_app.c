...
static bool app_init(DefaultApp *app, ov_app_config config) {
  ...
  if (0 < config.reconnect.max_connections) {
    ...
    app->reconnect =
        ov_reconnect_manager_create(config.loop, config.reconnect.interval_secs,
                                    config.reconnect.max_connections);
    app->reconnect_data =
        calloc(config.reconnect.max_connections, sizeof(ReconnectDataEntry));
  }
...
static AppConnection *app_connection_create(DefaultApp *app,
  ...
  *connection = (AppConnection){
  ...
  if (socket > -1)
    if (!ov_socket_get_data(socket, &connection->local, &connection->remote))
      goto error;
...
void *app_listener_free(void *data) {
  ...
  if (listener->config.callback.close)
    listener->config.callback.close((ov_app *)listener->app, listener->socket,
                                    NULL, listener->config.callback.userdata);
...
bool impl_app_socket_open(ov_app *self, ov_app_socket_config config,
  ...
  if (socket < 0) {
    ov_log_error("%s failed to open socket %s:%i | %s", app->public.config.name,
...
static bool open_client_connection(ov_app *self, ov_app_socket_config config,
  ...
  DefaultApp *app = AS_IMPL_APP(self) if (!app || !config.as_client) goto error;
  ...
  ov_log_info(
      ...
      "%s",
      app->public.config.name, connection->uuid, socket, connection->local.host,
      connection->local.port, connection->remote.host, connection->remote.port,
...
static ov_parser *app_get_send_parser(DefaultApp *app, int socket,
  ...
  AppConnection *connection = connection_from_socket_data(socket_data, remote);
...
static bool cb_io_accept(int socket_fd, uint8_t events, void *data) {
  ...
  if (nfd < 0) {
    ...
    ov_log_error("%s failed to accept at socket %i", app->public.config.name,
                 socket_fd);
    goto error;
  }
  ...
  AppConnection *connection = app_connection_create(app, listener->config, nfd);
  ...
  if ((0 != listener->config.callback.accepted) &&
      (!listener->config.callback.accepted(
          &app->public, socket_fd, nfd, listener->config.callback.userdata))) {
...
bool cb_io_stream(int socket, uint8_t events, void *self) {
  ...
  ov_log_debug("%s io stream at %i | event %i", app->public.config.name, socket,
               events);
...
static bool send_parser_answer(DefaultApp *app, AppConnection *connection,
  ...
  ov_log_debug("%s | %s connection IO parser %s answer "
               "at %s:%i from %s:%i",
               app->public.config.name, connection->uuid,
               connection->parser->name, connection->config.socket_config.host,
...
static bool app_send_tcp(ov_app *app, const ov_buffer *buffer, int socket) {
  ...
  while (total != buffer->length) {
    ...
    bytes =
        send(socket, (char *)buffer->start + total, buffer->length - total, 0);
...
ov_app_parse_command_line_optargs(int argc, char *const argv[],
  ...
  optind = 1; // resets getopt in case of the arguments will scanned later again
...
bool ov_app_connect(ov_app *self, ov_app_socket_config config) {
  ...
  if (0 == app) {
    ...
    if (0 == rd) {
      ...
      return ov_reconnect_manager_connect(app->reconnect, config.socket_config,
                                          events, callbacks, rd);
    }
  }
}
