...
ov_json_value *ov_ice_proxy_vocs_msg_create_session_with_sdp(const char *uuid,
    ...
    if (!ov_json_object_set(params, OV_KEY_SDP, val)) goto error;
...
ov_json_value *ov_ice_proxy_vocs_msg_client_response_from_ice_session_create(
    ...
    if (!client_request || !session_response) goto error;
    ...
    */
    ...
    if (NULL != ov_event_api_get_error(session_response)) goto error;
    ...
    if (!ov_event_api_set_type(
            response, ov_media_type_to_string(OV_MEDIA_OFFER)))
        goto error;

    val = ov_json_string(ice_session_id);
    if (!ov_json_object_set(response, OV_KEY_SESSION, val)) goto error;

    val = ov_json_string(sdp);
    if (!ov_json_object_set(response, OV_KEY_SDP, val)) goto error;
...
ov_json_value *ov_ice_proxy_vocs_msg_register(const char *uuid) {
    ...
    */
    ...
    if (!ov_event_api_set_uuid(params, uuid)) goto error;
...
ov_json_value *ov_ice_proxy_vocs_msg_forward_stream(
    ...
    if (session_uuid) {
        val = ov_json_string(session_uuid);
        if (!ov_json_object_set(params, OV_KEY_SESSION, val)) goto error;
    }

    val = ov_ice_proxy_vocs_stream_forward_data_to_json(data);
    if (!ov_json_object_set(params, OV_KEY_STREAM, val)) goto error;
...
ov_json_value *ov_ice_proxy_vocs_msg_candidate(
    ...
    if (!info->ufrag || !info->candidate) goto error;
    ...
    if (!ov_json_object_set(params, OV_ICE_STRING_SDP_MID, val)) goto error;
    ...
    if (!ov_json_object_set(params, OV_ICE_STRING_UFRAG, val)) goto error;

    val = ov_json_string(info->candidate);
    if (!ov_json_object_set(params, OV_ICE_STRING_CANDIDATE, val)) goto error;
...
ov_json_value *ov_ice_proxy_vocs_msg_media(const char *session_id,
    ...
    if (!ov_json_object_set(params, OV_KEY_TYPE, val)) goto error;
...
ov_json_value *ov_ice_proxy_vocs_msg_session_completed(const char *uuid,
    ...
    if (!ov_json_object_set(params, OV_KEY_STATE, val)) goto error;
...
ov_json_value *ov_ice_proxy_vocs_msg_talk(const char *uuid,
    ...
    if (!uuid || !session_uuid) goto error;
    ...
    if (!ov_json_object_set(params, OV_KEY_LOOP, val)) goto error;
    ...
    if (!ov_json_object_set(params, OV_KEY_ON, val)) goto error;
