...
static ov_json_value *api_message_create(const char *name) {
  ...
  out = ov_event_api_message_create(name, uuid, ov_ice_proxy_vocs_API_VERSION);
...
ov_json_value *ov_ice_proxy_vocs_msg_client_response_from_ice_session_create(
  ...
  */
  ...
  if (!ov_event_api_set_type(response, ov_media_type_to_string(OV_MEDIA_OFFER)))
    goto error;
...
ov_json_value *ov_ice_proxy_vocs_msg_forward_stream(
  ...
  if ((0 == data.socket.host[0]) || (0 == data.socket.port) || (0 == data.ssrc))
    goto error;

  out = ov_event_api_message_create(OV_KEY_ICE_SESSION_FORWARD_STREAM, NULL, 0);
