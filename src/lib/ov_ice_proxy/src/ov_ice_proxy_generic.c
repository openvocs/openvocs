...
ov_ice_proxy_generic *ov_ice_proxy_generic_free(ov_ice_proxy_generic *self) {
  ...
  if (!ov_ice_proxy_generic_cast(self))
    return self;
...
ov_ice_proxy_generic *ov_ice_proxy_generic_cast(const void *data) {
  ...
  if (*(uint16_t *)data != OV_ICE_PROXY_GENERIC_MAGIC_BYTES)
    return NULL;
...
ov_ice_proxy_generic_config_from_json(const ov_json_value *input) {
  ...
  out.dynamic.stun.server = ov_socket_configuration_from_json(
      ov_json_get(config, "/" OV_KEY_DYNAMIC "/" OV_KEY_STUN "/" OV_KEY_SERVER),
      (ov_socket_configuration){0});

  out.dynamic.turn.server = ov_socket_configuration_from_json(
      ov_json_get(config, "/" OV_KEY_DYNAMIC "/" OV_KEY_TURN "/" OV_KEY_SERVER),
      (ov_socket_configuration){0});

  const char *turn_user = ov_json_string_get(
      ov_json_get(config, "/" OV_KEY_DYNAMIC "/" OV_KEY_TURN "/" OV_KEY_USER));

  const char *turn_pass = ov_json_string_get(
      ov_json_get(config, "/" OV_KEY_DYNAMIC "/" OV_KEY_TURN "/" OV_KEY_PASS));

  if (turn_user)
    strncpy(out.dynamic.turn.username, turn_user,
            OV_ICE_PROXY_GENERIC_TURN_USERNAME_MAX);

  if (turn_pass)
    strncpy(out.dynamic.turn.password, turn_pass,
